...
@implementation STIMOpenPlatformCell{
...
+ (CGFloat)getCellHeightWithMessage:(STIMMessageModel *)message{
    NSDictionary *msgDic = [[STIMJSONSerializer sharedInstance] deserializeObject:message.message error:nil];
...
- (void)updateBottomButtonState:(NSNotification *)notify{
    NSDictionary *msgDic = [[STIMJSONSerializer sharedInstance] deserializeObject:self.message.message error:nil];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [_tagLabel setTextColor:[UIColor stimDB_colorWithHex:0x41bbc4 alpha:1]];
        [_tagLabel setText:[NSBundle stimDB_localizedStringForKey:@"#tag#"]];
        ...
        [_timeLabel setTextColor:[UIColor stimDB_colorWithHex:0x999999 alpha:1]];
        [_timeLabel setText:[NSBundle stimDB_localizedStringForKey:@"0 min ago"]];
        ...
        [_contentLabel setTextColor:[UIColor stimDB_colorWithHex:0x333333 alpha:1]];
        ...
        [_bottomButton setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor stimDB_colorWithHex:0x41bbc4 alpha:1]] forState:UIControlStateNormal];
        [_bottomButton.titleLabel setFont:[UIFont systemFontOfSize:14]];
        [_bottomButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Answer now"] forState:UIControlStateNormal];
...
- (void)onBottomClick:(UIButton *)sender{
    if ([self.delegate respondsToSelector:@selector(STIMOpenPlatformCellClick:)]) {
        [self.delegate STIMOpenPlatformCellClick:self];
    }
}

- (void)updateBottomButtonWithDealId:(NSString *)dealId{
    QDDealState state = [[STIMKit sharedInstance] getDealIdState:dealId];
    switch (state) {
        ...
        case QDDealState_True:
        {
            ...
            [_bottomButton setTitleColor:[UIColor stimDB_colorWithHex:0x666666 alpha:1] forState:UIControlStateNormal];
            [_bottomButton setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor stimDB_colorWithHex:0xffffff alpha:1]] forState:UIControlStateNormal];
        }
            break;
        case QDDealState_Faild:
        {
            [_bottomButton setTitle:[NSBundle stimDB_localizedStringForKey:@"It has been answered"] forState:UIControlStateNormal];
            [_bottomButton removeTarget:self action:@selector(onBottomClick:)forControlEvents:UIControlEventTouchUpInside];
            [_bottomButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_bottomButton setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor stimDB_colorWithHex:0x94e0e9 alpha:1]] forState:UIControlStateNormal];
        }
            break;
        case QDDealState_TimeOut:
        {
            [_bottomButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Expired"] forState:UIControlStateNormal];
            [_bottomButton removeTarget:self action:@selector(onBottomClick:)forControlEvents:UIControlEventTouchUpInside];
            [_bottomButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_bottomButton setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor stimDB_colorWithHex:0x94e0e9 alpha:1]] forState:UIControlStateNormal];
        }
...
//    "timeout": "20"
    ...
    [_timeLabel setText:[[NSDate dateWithTimeIntervalSince1970:time] stimDB_timeIntervalDescription]];
