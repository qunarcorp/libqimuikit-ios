...
@interface STIMSingleChatCell() <STIMMenuImageViewDelegate, UIActionSheetDelegate>
{
...
@implementation STIMSingleChatCell
...
- (void)setMessage:(STIMMessageModel *)message {
    _message = message;
    _textContainer = [STIMMessageParser textContainerForMessage:message];
}
...
//        _textLabel.tag = kTextLabelTag;
        ...
        _msgSendFailedImageView = [[UIImageView alloc] initWithImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"tips_message_failed"]];
...
- (void)tapHandle:(UITapGestureRecognizer *)tap{
    if (self.message.messageSendState == STIMMessageSendState_Faild) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kXmppStreamReSendMessage object:self.message];
        //        UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"] destructiveButtonTitle:nil otherButtonTitles:[NSBundle stimDB_localizedStringForKey:@"Resend"],[NSBundle stimDB_localizedStringForKey:@"Delete"], nil];
...
- (void)msgDidSendNotificationHandle:(NSNotification *)notify
{
    ...
    if ([[self.message messageId] isEqualToString:msgID]) {
        if (self.message.messageSendState < STIMMessageSendState_Success) {
            self.message.messageSendState = STIMMessageSendState_Success;
        }
        [self refreshUI];
    }
...
- (void)refreshUI{
    ...
    switch (self.message.messageDirection) {
        case STIMMessageDirection_Received:
        {
            ...
            [_backView setImage:[STIMMsgBaloonBaseCell leftBallocImage]];
        ...
        case STIMMessageDirection_Sent:
        {
            ...
            [_backView setImage:[STIMMsgBaloonBaseCell rightBallcoImage]];
            if (self.message.messageSendState == STIMMessageSendState_Waiting) {
                _actIndView.frame = CGRectMake(_backView.left - 30, _backView.bottom - 35, 30, 30);
                [_actIndView startAnimating];
            }else if(self.message.messageSendState == STIMMessageSendState_Faild){
