...
typedef enum {
    ...
    STIMXMenuViewArrowDirectionNone,
    STIMXMenuViewArrowDirectionUp,
    STIMXMenuViewArrowDirectionDown,
    STIMXMenuViewArrowDirectionLeft,
    STIMXMenuViewArrowDirectionRight,
    
} STIMXMenuViewArrowDirection;


@interface STIMXMenuView : UIView
...
@interface STIMXMenuOverlay : UIView
...
#pragma mark - STIMXMenuOverlay

@implementation STIMXMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[STIMXMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma mark - STIMXMenuView
@implementation STIMXMenuView {
    STIMXMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    STIMXMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([STIMXMenu hasShadow]) {
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [STIMXMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = STIMXMenuViewArrowDirectionUp;
        ...
        _arrowDirection = STIMXMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = STIMXMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = STIMXMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = STIMXMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(STIMXMenuSelectedItem)selectedItem {
    ...
    STIMXMenuOverlay *overlay = [[STIMXMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    if (self.superview) {
        if (animated) {
            ...
            [UIView animateWithDuration:0.2 animations:^(void) {
                ...
                if ([self.superview isKindOfClass:[STIMXMenuOverlay class]])
...
- (void)performAction:(id)sender {
    ...
    STIMXMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView {
    ...
    UIFont *titleFont = [STIMXMenu titleFont];
    
    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    
    for (STIMXMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [STIMXMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [STIMXMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == STIMXMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == STIMXMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == STIMXMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == STIMXMenuViewArrowDirectionRight) {
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [STIMXMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([STIMXMenu backgrounColorEffect] == STIMXMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == STIMXMenuViewArrowDirectionLeft ||
        _arrowDirection == STIMXMenuViewArrowDirectionRight) {
...
#pragma mark - STIMXMenu
/// MenuView
static STIMXMenu                      *gMenu;
/// 背景色
static UIColor                      *gTintColor;
/// 字体
static UIFont                       *gTitleFont;
/// 背景色效果
static STIMXMenuBackgrounColorEffect   gBackgroundColorEffect = STIMXMenuBackgrounColorEffectSolid;
/// 是否显示阴影
static BOOL                          gHasShadow = NO;

@implementation STIMXMenu {
    STIMXMenuView *_menuView;
    BOOL         _observing;
}
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = STIMXMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(STIMXMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[STIMXMenuView alloc] init];
