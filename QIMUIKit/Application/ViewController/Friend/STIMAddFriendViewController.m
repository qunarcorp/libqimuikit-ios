...
#import "STIMAddFriendViewController.h"
#import "STIMChatVC.h"
#import "MBProgressHUD.h"

@interface STIMAddFriendViewController ()<UITableViewDataSource,UITableViewDelegate>{
...
@implementation STIMAddFriendViewController
...
- (void)onReceiveFriendPresence:(NSNotification *)notify{
    ...
    if ([xmppId isEqualToString:notify.object]) {
        ...
        if ([result isEqualToString:@"success"]) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:[NSString stringWithFormat:@"添加好友失败,原因:%@。",reason] delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
            [alertView show];
        }
    }
}
...
- (void)initTableView{ 
    ...
    [_tableView setBackgroundColor:[UIColor stimDB_colorWithHex:0xeaeaea alpha:1]];
...
- (UIView *)getContentView{
    ...
    CGFloat startY = 0;
    {
        ...
        [headerImageView stimDB_setImageWithJid:jid];
        ...
        [arrowImageView setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"arrow_right"]];
    ...
    { // 附加消息 Additional message
        ...
        if (state == 1) {
            ...
            [replyButton setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"AV_Check_start_button_normal"] forState:UIControlStateNormal];
            [replyButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Reply"] forState:UIControlStateNormal];
...
- (void)onHeaderViewClick{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [STIMFastEntrance openUserCardVCByUserId:[weakSelf.userInfoDic objectForKey:@"XmppId"]];
    });
}
...
- (void)openChatSession{
    ...
    /*
    NSString *name = [self.userInfoDic objectForKey:@"Name"];
    [[STIMKit sharedInstance] openChatSessionByUserId:xmppid];
...
- (MBProgressHUD *)progressHUD {
    if (!_progressHUD) {
        ...
        [_progressHUD setDetailsLabelText:[NSBundle stimDB_localizedStringForKey:@"Please wait for a momente"]];
...
- (void)onAgreeClick:(UIButton *)sender{
    ...
    if (xmppId.length > 0) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STIMKit sharedInstance] agreeFriendRequestWithXmppId:xmppId];
        });
    } else {
        ...
        NSString *xmppid = [self.userInfoDic objectForKey:@"XmppId"];
//        NSString *name = [self.userInfoDic objectForKey:@"Name"];
//       STIMMessageModel *msg = [[STIMKit sharedInstance] createMessageWithMsg:@"我通过了你的朋友验证请求，现在我们可以开始聊天了" extenddInfo:nil userId:xmppid userType:ChatType_SingleChat msgType:STIMMessageType_Text];
        [[STIMKit sharedInstance] sendMessage:@"我通过了你的朋友验证请求，现在我们可以开始聊天了" WithInfo:nil ToUserId:xmppid WithMsgType:STIMMessageType_Text];
    }
}

- (void)onRefuesClick:(UIButton *)sender{
    [[STIMKit sharedInstance] refusedFriendRequestWithXmppId:[self.userInfoDic objectForKey:@"XmppId"]];
