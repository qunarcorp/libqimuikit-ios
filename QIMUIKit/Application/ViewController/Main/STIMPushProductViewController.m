...
#import "STIMPushProductCell.h"
#import "STIMJSONSerializer.h"
#import "MBProgressHUD.h"
#import "NSBundle+STIMLibrary.h"

@interface STIMPushProductViewController ()<UITableViewDataSource,UITableViewDelegate,STIMPushProductCellDelegate>
{
...
@implementation STIMPushProductViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    self.navigationItem.title = [NSBundle stimDB_localizedStringForKey:@"send_custom_pruduct"];
...
- (void)setUpHeader{
    ...
    if (_searchTextField == nil) {
        ...
        _searchTextField.placeholder = [NSBundle stimDB_localizedStringForKey:@"check_product_please_input_name"];
    ...
    if (_searchBtn == nil) {
        ...
        [_searchBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"check_product"] forState:UIControlStateNormal];
...
-(void)searchBtnHandle:(id)sender{
    if (_searchTextField.text.length == 0) {
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"产品Id不能为空！" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return [STIMPushProductCell getCellHeight];
}

-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    STIMPushProductCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (cell == nil) {
        cell = [[STIMPushProductCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
        cell.delegate = self;
    }
...
#pragma mark - STIMPushProductCellDelegate
...
- (void)sendBtnClickedForCell:(STIMPushProductCell *)cell{
    if (self.delegate && [self.delegate respondsToSelector:@selector(sendProductInfoStr:productDetailUrl:)]) {
        ...
        [self.delegate sendProductInfoStr:[[STIMJSONSerializer sharedInstance] serializeObject:infoDic] productDetailUrl:[NSString stringWithFormat:@"[obj type=\"url\" value=\"%@\"]",[infoDic[@"data"] objectForKey:@"touchDtlUrl"]]];
    }
