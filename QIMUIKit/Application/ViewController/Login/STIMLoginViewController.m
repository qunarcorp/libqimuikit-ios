...
@interface STIMLoginViewController ()<UIAlertViewDelegate>
{
...
@implementation STIMLoginViewController{
}
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([linkUrl length] > 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"是否更新" message:nil delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:[NSBundle stimDB_localizedStringForKey:@"Cancel"],nil];
        [alertView show];
    }
...
-(void)login:(id)sender
{
    if (([[self.passwordTextField text] length] > 0)&&([[self.usernameTextField text] length] > 0)) {
        ...
        NSString * RSAPassword = [STIMRSACoder RSAForPassword:[self.passwordTextField text]];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ 
            ...
            NSDictionary * loginDic = [[STIMKit sharedInstance] QChatLoginWithUserId:self.usernameTextField.text rsaPassword:RSAPassword type:type];
            if ([[loginDic objectForKey:@"ret"] boolValue]) {
                ...
                [passwordDic setSTIMSafeObject:[cookieDic objectForKey:@"qcookie"] forKey:@"q"];
                [passwordDic setSTIMSafeObject:[cookieDic objectForKey:@"vcookie"] forKey:@"v"];
                [passwordDic setSTIMSafeObject:[cookieDic objectForKey:@"tcookie"] forKey:@"t"];
                [passwordDic setSTIMSafeObject:[cookieDic objectForKey:@"type"] forKey:@"type"];
                NSString *password = [[STIMJSONSerializer sharedInstance] serializeObject:@{@"cookie":passwordDic}];
                
                [[STIMKit sharedInstance] setUserObject:passwordDic forKey:@"QChatCookie"];
                
                //同步http cookie
                [self syncWebviewCookie];
                [[STIMKit sharedInstance] loginWithUserName:userName WithPassWord:password];
                [[STIMKit sharedInstance] saveUserInfoWithName:userName passWord:[self.passwordTextField text]];
            } else {
                dispatch_async(dispatch_get_main_queue(), ^{ 
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:[NSString stringWithFormat:@"[%d] %@",[[loginDic objectForKey:@"errcode"] intValue],[loginDic objectForKey:@"errmsg"]] delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_ok"] otherButtonTitles:nil];
                    [alertView show];
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"用户名密码无效" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
        [alertView show];
    }
}
...
- (void)loginNotify:(NSNotification *)notify{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([notify.object boolValue]) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"] message:[NSBundle stimDB_localizedStringForKey:@"login_faild"] delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_ok"] otherButtonTitles:nil];
            [alert show];
        }
    });
}
...
- (void)registerNotify:(NSNotification *)notify{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"] message:@"暂不开放注册" delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_ok"] otherButtonTitles:nil];
        [alert show];
        return;
        
        if ([notify.object boolValue] == NO) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"] message:@"注册失败！" delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_ok"] otherButtonTitles:nil];
            [alert show];
        }
    });
}
...
- (void)initWithUI
{
    ...
    //设置圆角
    [STIMViewHelper setRadiusToView:self.usernameTextField];
    [STIMViewHelper setRadiusToView:self.passwordTextField];
    [STIMViewHelper setRadiusToView:self.loginButton];
    [STIMViewHelper setRadiusToView:self.registerButton];
    
    //设置textfield左部填充空白
    [STIMViewHelper setTextFieldLeftView:self.usernameTextField];
    [STIMViewHelper setTextFieldLeftView:self.passwordTextField];
    
    //初始化数据
    NSString *username = [STIMKit getLastUserName];
    NSString *password = [[STIMKit sharedInstance] getLastPassword];
    ...
    [agreeBtn setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"checkbox_normal"] forState:UIControlStateNormal];
    [agreeBtn setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"checkbox_click"] forState:UIControlStateSelected];
