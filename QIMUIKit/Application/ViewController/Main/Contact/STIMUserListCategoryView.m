...
@interface STIMUserListCategoryView ()

@property (nonatomic, strong) NSArray *categoryList;

@end

@implementation STIMUserListCategoryView
...
- (void)setupCateoryListView {
    ...
    for (NSInteger i = 0; i < self.categoryList.count; i++) {
        ...
        if (i != self.categoryList.count - 1) {
            ...
            lineView.backgroundColor = [UIColor stimDB_colorWithHex:0xEEEEEE alpha:1.0];
...
- (UIView *)singleCateoryListViewWithType:(UserListCategoryType)type WithLastView:(UIView *)lastView {
    ...
    iconImageView.image = [UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:[self getCategoryIconImageName:type] size:26 color: [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1/1.0]]];
...
//    _nameLabel.font = [UIFont fontWithName:FONT_NAME size:[[STIMCommonFont sharedInstance] currentFontSize] - 4];
    [titleLabel setFont:[UIFont fontWithName:FONT_NAME size:[[STIMCommonFont sharedInstance] currentFontSize] - 4]];
...
- (NSString *)getCategoryTitle:(UserListCategoryType)type {
    switch (type) {
        case UserListCategoryTypeNotRead: {
            return [NSBundle stimDB_localizedStringForKey:@"contact_tab_not_read"];
        }
            break;
        case UserListCategoryTypeFriend: {
            return [NSBundle stimDB_localizedStringForKey:@"contact_tab_friend"];
        }
            break;
        case UserListCategoryTypeGroup: {
            return [NSBundle stimDB_localizedStringForKey:@"contact_tab_group"];
        }
            break;
        case UserListCategoryTypePublicNumber: {
            return [NSBundle stimDB_localizedStringForKey:@"contact_tab_public_number"];
        }
            break;
        case UserListCategoryTypeOrganizational: {
            return [NSBundle stimDB_localizedStringForKey:@"contact_tab_organization"];
        }
            break;
        default:
            break;
