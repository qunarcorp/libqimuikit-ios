...
@implementation STIMGroupViewCell
{
...
+ (float)getCellHeightForGroupName:(NSString *)groupName{
    CGSize size = [groupName stimDB_sizeWithFontCompatible:[UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize]] constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 10 - 60 - 10, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping];
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [_headerView setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"singleHeaderDefault"]];
        ...
        [_notReadNumButton setBackgroundImage:[[UIImage stimDB_imageFromColor:[UIColor qunarRedColor]] stretchableImageWithLeftCapWidth:8 topCapHeight:8]  forState:UIControlStateNormal];
        [_notReadNumButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_notReadNumButton setBackgroundImage:[[UIImage stimDB_imageFromColor:[UIColor qunarRedColor]] stretchableImageWithLeftCapWidth:8 topCapHeight:8]  forState:UIControlStateHighlighted];
...
- (void)updateCell:(NSNotification *)notify {
    ...
    if ([groupIds containsObject:self.groupID]) {
        NSDictionary *cardDic = [[STIMKit sharedInstance] getGroupCardByGroupId:self.groupID];
...
- (void)refresh {
    ...
    _nameLabel.font = [UIFont fontWithName:FONT_NAME size:[[STIMCommonFont sharedInstance] currentFontSize]];
    [_nameLabel setText:(self.userName.length > 0) ? self.userName : [[self.groupID componentsSeparatedByString:@"@"] firstObject]];
//    UIImage * headImage = [[STIMKit sharedInstance] getGroupImageFromLocalByGroupId:self.groupID];
