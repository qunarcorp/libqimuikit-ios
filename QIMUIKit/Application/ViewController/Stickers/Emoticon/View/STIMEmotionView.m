...
@interface STIMEmotionView () <UICollectionViewDelegate, UICollectionViewDataSource>

@property (nonatomic, strong) NSMutableArray *originDataSource;
@property (nonatomic, strong) NSMutableArray *devideDataSource;
@property (nonatomic, weak) id <STIMEmotionTipDelegate> touchView;
...
@implementation STIMEmotionView
    ...
    if (!_originDataSource) {
        ...
        switch (self.emotionType) {
            ...
            case QTalkEmotionTypeNormal: {
                _originDataSource = [NSMutableArray arrayWithArray:[[STIMEmotionManager sharedInstance] getEmotionImagePathListForPackageId:self.Pkid]];
            }
                break;
            case QTalkEmotionTypeCollection: {
                [_originDataSource addObjectsFromArray:[[STIMCollectionFaceManager sharedInstance] getCollectionFaceList]];
            }
                break;
            default:
                break;
...
- (void)reloadCollectionFaceView {
    if (self.emotionType == QTalkEmotionTypeCollection) {
        self.originDataSource = [NSMutableArray arrayWithArray:[[STIMCollectionFaceManager sharedInstance] getCollectionFaceList]];
...
+ (instancetype)qtalkEmotionCollectionViewWithFrame:(CGRect)frame WithPkid:(NSString *)packageId {
    BOOL showAll = [[STIMEmotionManager sharedInstance] isEmotionPackageSupportGraphicMixedForPackageId:packageId];
    ...
    STIMEmotionView *collectionView = [[STIMEmotionView alloc] initWithFrame:frame collectionViewLayout:layout WithPackageId:packageId WithEmotionType:emotionType];
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(UICollectionViewLayout *)layout WithPackageId:(NSString *)Pkid WithEmotionType:(QTalkEmotionType)type {
    STIMVerboseLog(@"QTalkEmotionType : %ld, PKID : %@", type, Pkid);
    ...
    if (self) {
        ...
        [self registerClass:[STIMCollectionViewCell class] forCellWithReuseIdentifier:collectionCellId];
        [self registerClass:[STIMFaceViewCell class] forCellWithReuseIdentifier:showAllEmotionCellId];
        [self registerClass:[STIMFaceViewCell class] forCellWithReuseIdentifier:normalEmotionCellId];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (self.emotionType) {
        case QTalkEmotionTypeShowAll: {
            STIMFaceViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:showAllEmotionCellId forIndexPath:indexPath];
        ...
        case QTalkEmotionTypeNormal: {
            ...
            STIMFaceViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:normalEmotionCellId forIndexPath:indexPath];
            ...
            imageStr = [[STIMEmotionManager sharedInstance] getImageAbsolutePathForRelativePath:imageStr];
            cell.emojiPath = imageStr;
            cell.emojiView.image = [[STIMEmotionManager sharedInstance] getEmotionThumbIconWithImageStr:imageStr BySize:CGSizeMake(kShowAllImageFacePageViewItemWidth, kShowAllImageFacePageViewItemWidth)];
            return cell;
        }
            break;
        case QTalkEmotionTypeCollection: {
            STIMCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:collectionCellId forIndexPath:indexPath];
            /**
             *  计算cell排列方式让collectionView横向滚动，一行一行地去排列
             */
            //行
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (self.emotionType) {
        ...
        case QTalkEmotionTypeNormal: {
            UICollectionViewCell *cell = [collectionView cellForItemAtIndexPath:indexPath];
            NSString *normalFaceImagePath = [[[STIMEmotionManager sharedInstance] getEmotionImagePathListForPackageId:[[STIMEmotionManager sharedInstance] currentPackageId]] objectAtIndex:cell.tag];
            NSString *faceName = [[STIMEmotionManager sharedInstance] getEmotionShortCutForImagePath:normalFaceImagePath withPackageId:[[STIMEmotionManager sharedInstance] currentPackageId]];
            //直接发送表情
            ...
            break;
        case QTalkEmotionTypeCollection: {
            STIMCollectionViewCell *cell = (STIMCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
            NSString *fileUrl = nil;
            if (cell.tag) {
                fileUrl = [[STIMCollectionFaceManager sharedInstance] getCollectionFaceHttpUrlWithIndex:cell.tag - 1];
            } else {
            ...
            NSString *fileName = [[STIMKit sharedInstance] getFileNameFromUrl:fileUrl];
...
- (void)longPressHandler:(UILongPressGestureRecognizer *)longPress {
    ...
    id <STIMEmotionTipDelegate> touchView = (id <STIMEmotionTipDelegate> )[self subViewAtPoint:point];
