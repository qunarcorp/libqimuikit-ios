...
@interface STIMRedPackageView()<UIWebViewDelegate,STIMContactSelectionViewControllerDelegate,UIAlertViewDelegate>
@property (nonatomic,copy) NSString  * rId;
@end
@implementation STIMRedPackageView{
    UIWebView *_webView;
}
...
+ (void)showRedPackagerViewByUrl:(NSString *)url{
    if (__redPackageWindow == nil) {
        ...
        [__redPackageWindow setBackgroundColor:[UIColor stimDB_colorWithHex:0x0 alpha:0.5]];
        ...
        STIMRedPackageView *redPackageView = [[STIMRedPackageView alloc] initWithFrame:CGRectMake(20, 20, __redPackageWindow.width-40, __redPackageWindow.height-40)];
        if ([[STIMKit sharedInstance] getIsIpad]) {
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        if ([STIMKit getSTIMProjectType] != STIMProjectTypeQChat) {
            NSString *ua = [[STIMWebView defaultUserAgent] stringByAppendingString:@" qunartalk-ios-client"];
...
- (void)contactSelectionViewController:(STIMContactSelectionViewController *)contactVC groupChatVC:(STIMGroupChatVC *)vc{
    dispatch_async(dispatch_get_main_queue(), ^{ 
        NSString * jsStr = [NSString stringWithFormat:@"relay_red_env('%@','%@','%@','%@');",[[[NSString stringWithFormat:@"%@00d8c4642c688fd6bfa9a41b523bdb6b",self.rId] stimDB_getMD5] lowercaseString],@"qunartalk-ios",[[contactVC getSelectInfoDic] objectForKey:@"userId"],@""];
