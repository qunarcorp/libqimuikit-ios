...
@interface STIMJoinGroupVC ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate,SearchBarDelgt,UIGestureRecognizerDelegate>{
...
@implementation STIMJoinGroupVC
...
- (void)viewDidLoad {
    ...
    _groupList = [[NSMutableArray alloc] initWithArray:[[STIMKit sharedInstance] getGroupList]];
...
//    
//    BarButton *leftButton = [[BarButton alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Back"] style:eBarButtonStyleBack target:self action:@selector(goBack:)];
...
//    [STIMViewHelper setRadiusToView:_groupNameField];
//    [STIMViewHelper setTextFieldLeftView:_groupNameField];
...
//    [STIMViewHelper setRadiusToView:_joinButton];
...
- (void)searchBar:(SearchBar *)SectionSearchBar textDidChange:(NSString *)searchText
{
    ...
    if (searchText.length>0&&![STIMChineseInclude isIncludeChineseInString:searchText]) {
        NSMutableArray *  searchDictArray  = nil;
        searchDictArray = _groupList;
        for (NSDictionary * dict in searchDictArray) {
            if ([STIMChineseInclude isIncludeChineseInString:[dict objectForKey:keyName]]) {
                NSString *tempPinYinStr  =    [dict objectForKey:@"pinyin"]!=nil?[dict objectForKey:@"pinyin"]:[STIMPinYinForObjc chineseConvertToPinYin:[dict objectForKey:keyName]];
...
- (void)onDoneButtonClick:(UIButton *)sender{
    self.groupId = [NSString stringWithFormat:@"%@@%@.%@",_groupNameField.text,@"conference",[[STIMKit sharedInstance] getDomain]];
    _nickName = [[STIMKit sharedInstance] getMyNickName];
    if ([[STIMKit sharedInstance] isGroupMemberByGroupId:self.groupId]) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"已经是该群成员。" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
        [alertView show];
    } else {
...
- (void)onJoinSuccessNotify:(NSNotification *)notify{
    if ([_groupId isEqualToString:notify.object]) {
        ...
        [[STIMKit sharedInstance] openGroupSessionByGroupId:self.groupId ByName:_groupNameField.text];
