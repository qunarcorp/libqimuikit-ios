...
@interface STIMContactSelectionViewController ()<UITableViewDataSource,UITableViewDelegate,SearchBarDelgt,UIGestureRecognizerDelegate,STIMGroupListVCDelegate,STIMFriendListSelectionVCDelegate>{
...
@implementation STIMContactSelectionViewController
...
- (void)viewDidLoad {
    ...
    _myGroups = [NSMutableArray arrayWithArray:[[STIMKit sharedInstance] getMyGroupList]];
...
-(void)initNavBar {
    ...
    [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"Forward to"]];
}
...
-(void)setupSearchBar {
    ...
    [_searchBarKeyTmp setPlaceHolder:[NSBundle stimDB_localizedStringForKey:@"search_bar_placeholder"]];
...
- (void)initTableViewHeader {
    ...
    friendTitleLabel.text = [NSBundle stimDB_localizedStringForKey:@"Select_Friend"];
...
//    [headerBG addSubview:friendView];
    ...
    groupTitleLabel.text = [NSBundle stimDB_localizedStringForKey:@"Select a Group"];
    ...
    indiorImageView.image = [UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f3c8" size:24 color:[UIColor colorWithRed:158/255.0 green:158/255.0 blue:158/255.0 alpha:1/1.0]]];
    ...
    [titleLabel setText:[NSBundle stimDB_localizedStringForKey:@"Latest Chats"]];
...
- (void)userBtnClick:(UIButton *)btn {
    STIMFriendListSelectionVC *listVC = [[STIMFriendListSelectionVC alloc] init];
...
- (void)searchBar:(SearchBar *)SectionSearchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length > 0) {
        [_searchResults addObjectsFromArray:[[STIMKit sharedInstance] searchUserListBySearchStr:searchText]];
    }
    ...
    for (NSDictionary * dict in searchDictArray) {
        ...
        if (pinyin == nil){
            pinyin = [STIMPinYinForObjc chineseConvertToPinYin:[dict objectForKey:keyName]];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_searchResults.count + _searchGroupResults.count > 0) {
        ...
        return [STIMContactUserCell getCellHeight];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_searchResults.count + _searchGroupResults.count > 0) {
        if (indexPath.row >= _searchResults.count) {
            ...
            STIMGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier2];
            if (cell == nil) {
                cell = [[STIMGroupViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier2];
            }
        ...
        NSString *combineJid = (realJid.length > 0) ? [NSString stringWithFormat:@"%@<>%@", xmppId, realJid] : [NSString stringWithFormat:@"%@<>%@", xmppId, xmppId];
        STIMContactUserCell *cell = [tableView dequeueReusableCellWithIdentifier:combineJid];
        if (cell == nil){
            cell = [[STIMContactUserCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)redeemDidSelectContactWithJid:(NSString *)jid chatType:(ChatType)chatType {
    ...
    NSString *name = nil;
    if (chatType == ChatType_SingleChat) {
        infoDic = [[STIMKit sharedInstance] getUserInfoByUserId:jid];
    } else {
        infoDic = [[STIMKit sharedInstance] getGroupCardByGroupId:jid];
    }
    name = [infoDic objectForKey:@"Name"];
    
    NSString *redeemContactMsg = [NSString stringWithFormat:@"%@%@?", [NSBundle stimDB_localizedStringForKey:@"Forward_chat_history"], name];
    if (self.ExternalForward) {
        redeemContactMsg = [NSString stringWithFormat:@"%@%@?", [NSBundle stimDB_localizedStringForKey:@"Send_to"], name];
    }
    UIAlertController *redeemContactVc = [UIAlertController alertControllerWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:redeemContactMsg preferredStyle:UIAlertControllerStyleAlert];
...
//解析原始消息
- (NSDictionary *)getOriginMessageWithMsg:(STIMMessageModel *)msg {
    if (msg.messageId.length <= 0) {
        return nil;
    }
    STIMVerboseLog(@"msgId : %@", msg.messageId);
    NSDictionary *msgDic = [[STIMKit sharedInstance] getMsgDictByMsgId:msg.messageId];
    if (msgDic.count > 0) {
        ...
        if (msgRaw) {
            NSDictionary * originMsgDic = [[STIMKit sharedInstance] parseMessageByMsgRaw:msgRaw];
        ...
        STIMMessageType msgType = [[msgDic objectForKey:@"MsgType"] intValue];
...
- (void)selectContactWithJid:(NSString *)jid{
    ...
    STIMChatVC *chatVC = (STIMChatVC *)[[STIMFastEntrance sharedInstance] getSingleChatVCByUserId:jid];
    /*
    ...
    */
    ...
    if (self.ExternalForward) {
        ...
        NSDictionary * msgExtendInfo =[[STIMJSONSerializer sharedInstance]deserializeObject:self.message.extendInformation error:nil];
        if ([msgExtendInfo objectForKey:@"HttpUrl"] && [[msgExtendInfo objectForKey:@"HttpUrl"] hasPrefix:@"file"]) {
                STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_SingleChat msgType:self.message.messageType backinfo:nil];
        }
        else{
            STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_SingleChat msgType:self.message.messageType backinfo:nil];
            [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
        }
    } else {
        if (self.message) {
            ...
            if (msgContent.length > 0 && msgExtendInfo.length > 0) {
               STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_SingleChat msgType:self.message.messageType backinfo:self.message.backupInfo];
                [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
            } else {
                ...
                STIMMessageType msgType = [[originMsg objectForKey:@"MsgType"] integerValue];
                if (msgType == STIMMessageType_None && !self.ExternalForward) {
                    [[STIMKit sharedInstance] sendMessage:self.message.message WithInfo:self.message.extendInformation ToUserId:jid WithMsgType:self.message.messageType];
                } else {
                   STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_SingleChat msgType:msgType backinfo:backUpInfo];
                    [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
                }
            }
        } else if (self.messageList.count){
            for (STIMMessageModel * msg in self.messageList) {
                ...
                STIMMessageType msgType = [[originMsg objectForKey:@"MsgType"] integerValue];
                if (msgType == STIMMessageType_None && !self.ExternalForward) {
                    [[STIMKit sharedInstance] sendMessage:msg.message WithInfo:msg.extendInformation ToUserId:jid WithMsgType:msg.messageType];
                } else {
                   STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_SingleChat msgType:msgType backinfo:backUpInfo];
                    [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
                }
    ...
    if (!self.isTransfer) {
        if ([[STIMKit sharedInstance] getIsIpad]) {
            ...
            STIMNavController *nav = (STIMNavController *)[[[[UIApplication sharedApplication] delegate] window] rootViewController];
...
#warning 转发消息 可能出现丢字段的情况，需要从本地数据库取出原始消息拼接
    ...
    NSDictionary *infoDic = [[STIMKit sharedInstance] getGroupCardByGroupId:jid];
    STIMGroupChatVC *chatGroupVC = [[STIMFastEntrance sharedInstance] getGroupChatVCByGroupId:jid];
    /*
    ...
    if (self.ExternalForward) {
        NSDictionary * msgExtendInfo =[[STIMJSONSerializer sharedInstance]deserializeObject:self.message.extendInformation error:nil];
        if ([msgExtendInfo objectForKey:@"HttpUrl"] && [[msgExtendInfo objectForKey:@"HttpUrl"] hasPrefix:@"file"]) {
            STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_GroupChat msgType:self.message.messageType backinfo:nil];
        }
        else{
            STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_GroupChat msgType:self.message.messageType backinfo:nil];
            STIMMessageModel *tempMsg = [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
        ...
        if (self.message) {
            ...
            NSString *msgExtendInfo = self.message.extendInformation;
            if (msgContent.length > 0 && msgExtendInfo.length > 0) {
               STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_GroupChat msgType:self.message.messageType backinfo:self.message.backupInfo];
                [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
            } else {
                NSDictionary *originMsg = [self getOriginMessageWithMsg:self.message];
                NSString *msgContent = [originMsg objectForKey:@"Content"];
                ...
                STIMMessageType msgType = [[originMsg objectForKey:@"MsgType"] integerValue];
                if (msgType == STIMMessageType_None && !self.ExternalForward) {
                    [[STIMKit sharedInstance] sendMessage:self.message.message WithInfo:self.message.extendInformation ToGroupId:jid WithMsgType:self.message.messageType];
                } else {
                   STIMMessageModel *newMsg = [[STIMKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_GroupChat msgType:msgType backinfo:backUpInfo];
                    [[STIMKit sharedInstance] sendMessage:newMsg ToUserId:jid];
                }
            }
        } else if (self.messageList.count){
