...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        UIImage *videoIndicatorImage = [UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/VideoOverlay" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]];
        ...
        [_selectedButton setImage:[UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/ImageSelectedSmallOff" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateNormal];
        [_selectedButton setImage:[UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/ImageSelectedSmallOn" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateSelected];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(setProgressFromNotification:)
                                                     name:STIMMWPHOTO_PROGRESS_NOTIFICATION
                                                   object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)setGridController:(STIMMWGridViewController *)gridController {
    _gridController = gridController;
    // Set custom selection image if required
    if (_gridController.browser.customImageSelectedSmallIconName) {
        [_selectedButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:_gridController.browser.customImageSelectedSmallIconName] forState:UIControlStateSelected];
    }
}
...
- (void)setPhoto:(id <STIMMWPhoto>)photo {
    _photo = photo;
...
- (void)showImageFailure {
    ...
    if (![_photo respondsToSelector:@selector(emptyImage)] || !_photo.emptyImage) {
        if (!_loadingError) {
            ...
            _loadingError.image = [UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/ImageError" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]];
...
- (void)setProgressFromNotification:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        id <STIMMWPhoto> photoWithProgress = [dict objectForKey:@"photo"];
        if (photoWithProgress == _photo) {
//            STIMVerboseLog(@"%f", [[dict valueForKey:@"progress"] floatValue]);
