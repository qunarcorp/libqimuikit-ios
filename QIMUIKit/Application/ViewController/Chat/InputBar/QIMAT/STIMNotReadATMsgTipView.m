...
@implementation STIMNotReadATMsgTipView
{
    UIView *_backView;
    UIView *_leftView;
...
- (CGFloat)getWidthForNotReadCount:(int)notReadCount{
    NSString *titleStr = [NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"%ldn mentioned"],notReadCount];
    _title = titleStr;
...
- (void)updateNotReadAtMsgCount:(int)notReadAtMsgCount {
    _title = [NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"%ldn mentioned"],notReadAtMsgCount];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [_leftArrowImageView setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"up_arrow"]];
        ...
        [_titleLabel setTextColor:stimDB_notReadAtMessageUpArrowTextColor];
