...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [_infoBackView setBackgroundColor:[UIColor stimDB_colorWithHex:0x0 alpha:0.65]];
...
- (void)setInfoStr:(NSString *)infoStr{
    ...
    CGSize size = [_infoStr stimDB_sizeWithFontCompatible:_infoLabel.font constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 20, INT16_MAX) lineBreakMode:NSLineBreakByCharWrapping];
...
@interface STIMPublicNumberRobotVC ()<UITableViewDataSource,UITableViewDelegate,UIGestureRecognizerDelegate,STIMSingleChatCellDelegate,STIMSingleChatVoiceCellDelegate,NSXMLParserDelegate,STIMMWPhotoBrowserDelegate,ASIProgressDelegate,STIMRemoteAudioPlayerDelegate,STIMMsgBaloonBaseCellDelegate,STIMChatBGImageSelectControllerDelegate,STIMPNRichTextCellDelegate,STIMPNActionRichTextCellDelegate,PNNoticeCellDelegate,PNOrderMsgCellDelegate,STIMOpenPlatformCellDelegate, STIMTextBarDelegate>
{
    ...
    STIMRemoteAudioPlayer *_remoteAudioPlayer;
...
@implementation STIMPublicNumberRobotVC
...
- (STIMTextBar *)textBar {
    ...
    if (!_textBar) {
        ...
        _textBar = [STIMTextBar sharedIMTextBarWithBounds:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height) WithExpandViewType:STIMTextBarExpandViewTypePublicNumber];
...
//        [_textBar setPlaceHolder:@"文本信息"];
        ...
        NSDictionary *notSendDic = [[STIMKit sharedInstance] getNotSendTextByJid:self.robotJId];
        [_textBar setSTIMAttributedTextWithItems:notSendDic[@"inputItems"]];
        [_textBar.layer setBorderColor:[UIColor stimDB_colorWithHex:0xadadad alpha:1].CGColor];
        [_textBar setTextViewBackgroundImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"chat_bottom_textfield"] stretchableImageWithLeftCapWidth:5 topCapHeight:5]];
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, self.view.height - (_replayable?49:0) - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT]) style:UITableViewStylePlain];
...
- (STIMVoiceRecordingView *)voiceRecordingView {
    ...
    if (!_voiceRecordingView) {
        ...
        _voiceRecordingView = [[STIMVoiceRecordingView alloc] initWithFrame:CGRectMake(self.view.width/2-75, self.navigationController.navigationBar.height+150, 150, 150)];
...
- (UIButton *)cardButton {
    ...
    if (!_cardButton) {
        ...
        [_cardButton setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f0eb" size:24 color:[UIColor colorWithRed:33/255.0 green:33/255.0 blue:33/255.0 alpha:1/1.0]]] forState:UIControlStateNormal];
...
- (void)initUI {
    ...
    [[STIMKit sharedInstance] clearNotReadMsgByJid:self.robotJId];
    ...
    self.tableView.mj_header = [STIMMessageRefreshHeader messsageHeaderWithRefreshingTarget:self refreshingAction:@selector(loadNewPublicNumberMsgList)];
...
- (void)viewDidLoad {
    ...
    _publicNumberCard = [[STIMKit sharedInstance] getPublicNumberCardByJid:self.robotJId];
    ...
    [[STIMKit sharedInstance] setCurrentSessionUserId:self.robotJId];
...
-(void)loadData
{
    _dataSounce = [[NSMutableArray alloc] initWithArray:[[STIMKit sharedInstance] getPublicNumberMsgListById:self.robotJId WithLimit:kPageCount WithOffset:0]];
...
- (void)onCardClick{
    ...
    if ([[STIMKit sharedInstance] getPublicNumberCardByJid:self.robotJId]) {
...
- (void)selfPopedViewController{
    ...
    [[STIMKit sharedInstance] setNotSendText:[self.textBar getSendAttributedText] inputItems:[self.textBar getAttributedTextItems] ForJid:self.robotJId];
...
- (void)updateBottomView{
    if (_isActionOrTextBar) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [UIView animateWithDuration:0.2 animations:^{
                [_actionBottomView setFrame:CGRectMake(0,self.view.frame.size.height - 49 - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT], self.view.frame.size.width, 49)];
            }];
        }];
    } else {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            [_actionBottomView setFrame:CGRectMake(0,self.view.frame.size.height - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT], self.view.frame.size.width, 49)];
        } completion:^(BOOL finished) {
        }];
    }
}
...
- (void)onActionClick:(ActionButton *)sender{
    ...
    if ([action isEqualToString:@"openurl"]) {
        ...
        if ([value isKindOfClass:[NSString class]] == NO) {
            value = [[STIMJSONSerializer sharedInstance] serializeObject:value];
        }
        ...
        STIMZBarViewController*vc=[[STIMZBarViewController alloc] initWithBlock:^(NSString *str, BOOL isScceed) {
            if (isScceed) {
                ...
                if ([value isKindOfClass:[NSString class]]) {
                    value = [[STIMJSONSerializer sharedInstance] deserializeObject:value error:nil];
                }
                if ([value isKindOfClass:[NSDictionary class]]) {
                    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:value];
                    [dic setObject:str forKey:@"content"];
                    NSString *json = [[STIMJSONSerializer sharedInstance] serializeObject:dic];
        ...
        if ([value isKindOfClass:[NSString class]]) {
            dic = [[STIMJSONSerializer sharedInstance] deserializeObject:value error:nil];
        }
        if ([dic isKindOfClass:[NSDictionary class]]) {
            ...
            if (value) {
                ...
                NSString *json = [[STIMJSONSerializer sharedInstance] serializeObject:resultDic];
                [[STIMKit sharedInstance] sendMessage:json ToPublicNumberId:self.robotJId WithMsgId:[STIMUUIDTools UUID] WithMsgType:PublicNumberMsgType_PostBackCookie];
            } else {
                NSMutableDictionary *resultDic = [NSMutableDictionary dictionaryWithDictionary:dic];
...
- (void)initActionBottomView{
    ...
    _actionBottomView = [[UIView alloc] initWithFrame:CGRectMake(0,self.view.frame.size.height - 49 - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT], self.view.frame.size.width, 49)];
    [_actionBottomView setBackgroundColor:[UIColor stimDB_colorWithHex:0xf9f9f9 alpha:1]];
    ...
    [showTextBarBtn setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_listtotext"] forState:UIControlStateNormal];
    [showTextBarBtn setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_listtotextHL"] forState:UIControlStateHighlighted];
    ...
    for (NSDictionary *actionDic in _actionList) {
        ...
        [mainButton setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor qtalkTableDefaultColor]] forState:UIControlStateHighlighted];
        if (subActions.count > 0) {
            [mainButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_textmenuicon"] forState:UIControlStateNormal];
            ...
            [leftBgView setImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_more_frame_left"] stretchableImageWithLeftCapWidth:15 topCapHeight:32]];
            ...
            [rightBgView setImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_more_frame_right"] stretchableImageWithLeftCapWidth:15 topCapHeight:32]];
            [subActionView addSubview:rightBgView];
            
            UIImageView *middleView = [[UIImageView alloc] initWithFrame:CGRectMake(bianWidth, 0, 18, 0)];;
            [middleView setImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_more_frame_middle"] stretchableImageWithLeftCapWidth:9 topCapHeight:32]];
            ...
            for (NSDictionary *subActionDic in subActions) {
                ...
                [button setBackgroundImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Mode_more_frame_Choose"] stretchableImageWithLeftCapWidth:40 topCapHeight:16] forState:UIControlStateHighlighted];
...
- (void)refreshChatBGImageView {
    ...
    if (chatBGImageDic) {
        ...
        UIImage *image = [UIImage imageWithContentsOfFile:[[STIMDataController getInstance] getSourcePath:[NSString stringWithFormat:@"chatBGImageFor_%@", self.robotJId]]];
        if (!image) {
...
- (void)msgDidSendNotificationHandle:(NSNotification *)notify
{
    ...
    for (STIMMessageModel * msg in _dataSounce) {
   ...
   STIMMessageModel * message = _resendMsg;
    for (STIMMessageModel * msg in _dataSounce) {
        if ([msg isEqual:message]) {
            ...
            [[STIMKit sharedInstance] deleteMsg:message ByJid:self.robotJId];
            break;
        }
    }
}
...
//        NSDictionary *infoDic = [message getMsgInfoDic];
        ...
        if ([self isImageMessage:message.message]) {
            ...
            message = [[STIMKit sharedInstance] createMessageWithMsg:message.message extenddInfo:message.extendInformation userId:self.robotJId userType:ChatType_SingleChat msgType:message.messageType forMsgId:_resendMsg.messageId];
            ...
            message = [[STIMKit sharedInstance] sendMessage:message ToUserId:self.robotJId];
...
//    NSString * content = [notify.userInfo objectForKey:@"Content"];
    for (STIMMessageModel * msg in _dataSounce) {
        if ([msg.messageId isEqualToString:msgID]) {
            ...
            [[STIMKit sharedInstance] updateMsg:msg ByJid:self.robotJId];
            [_tableView reloadData];
            break;
        }
    }
       ...
       STIMMessageModel *msg = [notify.userInfo objectForKey:@"message"];
        ...
        if (msg) {
            if (msg.messageType == PublicNumberMsgType_ClientCookie) {
                NSDictionary *dic = [[STIMJSONSerializer sharedInstance] deserializeObject:msg.message error:nil];
                NSString *key = [dic objectForKey:@"key"];
            ...
            } else if (msg.messageType == STIMMessageType_ConsultResult) {
                NSDictionary *resultContent = [[STIMJSONSerializer sharedInstance] deserializeObject:msg.message error:nil];
                ...
                if ([sessionId rangeOfString:@"@"].location == NSNotFound) {
                    sessionId = [sessionId stringByAppendingFormat:@"@%@",[[STIMKit sharedInstance] getDomain]];
                }
                ...
                if (result) {
                    [[STIMKit sharedInstance] setDealId:dealid ForState:QDDealState_True];
                    ...
                    [[STIMKit sharedInstance] setDealId:dealid ForState:QDDealState_Faild];
...
//    NSString *msgId = [STIMUUIDTools UUID];
   ...
   STIMMessageModel *msg = [[STIMKit sharedInstance] createPublicNumberMessageWithMsg:msgContent extenddInfo:nil publicNumberId:self.robotJId msgType:PublicNumberMsgType_SmallVideo];
    
    [_dataSounce addObject:msg];
    [_tableView reloadData];
    [self scrollToBottomWithCheck:YES];
    
    [[STIMKit sharedInstance] uploadFileForPath:videoPath forMessage:msg withJid:self.robotJId isFile:YES];
}

- (void)sendMessage:(NSString *)message WithInfo:(NSString *)info ForMsgType:(int)msgType{
    if (msgType != PublicNumberMsgType_Action) {
       STIMMessageModel * msg = [[STIMKit sharedInstance] createPublicNumberMessageWithMsg:message extenddInfo:info publicNumberId:[NSString stringWithFormat:@"%@@%@",self.robotJId,[[STIMKit sharedInstance] getDomain]] msgType:msgType];
        [_dataSounce addObject:msg];
        [_tableView reloadData];
        [self scrollToBottomWithCheck:YES];
        [self addImageToImageList];
        msg = [[STIMKit sharedInstance] sendMessage:message ToPublicNumberId:self.robotJId WithMsgId:msg.messageId WithMsgType:msgType];
    } else {
        [[STIMKit sharedInstance] sendMessage:message ToPublicNumberId:self.robotJId WithMsgId:[STIMUUIDTools UUID] WithMsgType:msgType];
    }
...
- (void)sendText:(NSString *)text{
    ...
    for (IMTextBarInputItem * item in _textBar.inputItems) {
        if (item.type == IMTextBarInputItemTypeEmotion) {
            ...
            NSString *shortCut = [[STIMEmotionManager sharedInstance] getEmotionShortCutForTipName:tempText withPackageId:item.emotionPKId];
            if (shortCut) {
    ...
    if ([text length] > 0) {
       STIMMessageModel *msg = nil;
        ...
        msg = [[STIMKit sharedInstance] createPublicNumberMessageWithMsg:text extenddInfo:nil publicNumberId:self.robotJId msgType:PublicNumberMsgType_Text];
        [_dataSounce addObject:msg];
        [_tableView reloadData];
        [self scrollToBottomWithCheck:YES];
        [self addImageToImageList];
        msg = [[STIMKit sharedInstance] sendMessage:msg.message ToPublicNumberId:self.robotJId WithMsgId:msg.messageId WithMsgType:msg.messageType];
    }
}
...
{
    [[STIMEmotionSpirits sharedInstance] setDataCount:(int)_dataSounce.count];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([temp isKindOfClass:[NSString class]]) {
        return [[[STIMKit sharedInstance] getRegisterMsgCellClassForMessageType:STIMMessageType_Time] getCellHeightWithMessage:temp chatType:ChatType_SingleChat];
    } else {
       ...
       STIMMessageModel *message = temp;
        switch ((int)message.messageType) {
            ...
            {
                return [STIMPNRichTextCell getCellHeightByContent:message.message];
            }
                break;
            case PublicNumberMsgType_ActionRichText:
            {
                return [STIMPNActionRichTextCell getCellHeightByContent:message.message];
            }
                break;
            case PublicNumberMsgType_Notice:
            {
                return [STIMPublicNumberNoticeCell getCellHeightByContent:message.message];
            }
                break;
            case PublicNumberMsgType_OrderNotify:
            {
                return [STIMPublicNumberOrderMsgCell getCellHeightByContent:message.message];
            }
                break;
            case STIMMessageType_Consult:
            {
                return [STIMOpenPlatformCell getCellHeightWithMessage:message];
            }
                break;
            default:
            {
   ...
   STIMMessageModel *message = temp;
    switch ((int)message.messageType) {
        ...
        case STIMMessageType_Text:
        {
            ...
            STIMSingleChatCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            
            if (cell == nil) {
                cell = [[STIMSingleChatCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
        ...
        case STIMMessageType_Voice:
        {
            ...
            if (cell == nil) {
                cell = [[STIMSingleChatVoiceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
                [cell setFrameWidth:self.view.frame.size.width];
                [cell setDelegate:self];
            }
            [cell setMessage:message];
        ...
        case PublicNumberMsgType_RichText: {
            ...
            if (cell == nil) {
                cell = [[STIMPNRichTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
                [cell setDelegate:self];
            }
        ...
        case PublicNumberMsgType_ActionRichText:{
            ...
            if (cell == nil) {
                cell = [[STIMPNActionRichTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
                [cell setDelegate:self];
            }
            [cell setContent:message.message];
            [cell refreshUI];
        ...
        case PublicNumberMsgType_Notice:{
            ...
            if (cell == nil) {
                cell = [[STIMPublicNumberNoticeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
                [cell setDelegate:self];
            }
            [cell setContent:message.message];
            [cell refreshUI];
        ...
        case STIMMessageType_Consult:
        {
            ...
            STIMOpenPlatformCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            if (cell == nil) {
                cell = [[STIMOpenPlatformCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
                [cell setDelegate:self];
            }
...
- (void)openPlatformRequest:(NSString *)urlStr ForDealId:(NSString *)dealId{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString *temp = [urlStr stringByAppendingFormat:@"&qchat_id=%@",[[STIMKit sharedInstance] getLastJid]];
        NSURL *url = [NSURL URLWithString:[temp stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        ASIHTTPRequest *request = [[ASIHTTPRequest alloc] initWithURL:url];
        [request startSynchronous];
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([request responseStatusCode] == 200) {
                NSDictionary *resultDic = [[STIMJSONSerializer sharedInstance] deserializeObject:request.responseString error:nil];
                ...
                if (isSuccess) {
                    ...
                    switch (errorCode) {
                        case 5007: 
                            ...
                            break;
                        case 5008:
                            [[STIMKit sharedInstance] setDealId:dealId ForState:QDDealState_TimeOut];
                            break;
                        default:
                            break;
                    }
...
- (void)STIMOpenPlatformCellClick:(STIMOpenPlatformCell *)openPlatformCel{
    NSDictionary *msgDic = [[STIMJSONSerializer sharedInstance] deserializeObject:openPlatformCel.message.message error:nil];
...
- (void)processEvent:(int)event withMessage:(id)message {
    if (event == MA_Repeater) {
        ...
        for (STIMMessageModel * msg in _dataSounce) {
            if ([msg.messageId isEqualToString:[(STIMMessageModel *)message messageId]]) {
                NSInteger index = [_dataSounce indexOfObject:msg];
                [_dataSounce removeObject:msg];
        ...
        for (STIMMessageModel * msg in _dataSounce) {
            if ([msg.messageId isEqualToString:[(STIMMessageModel *)message messageId]]) {
                ...
                //                [_dataSounce removeObject:msg];
                //                [_tableView deleteRowsAtIndexPaths:[NSArray arrayWithObject:[NSIndexPath indexPathForRow:index inSection:0]] withRowAnimation:UITableViewRowAnimationAutomatic];
                [(STIMMessageModel *)message setMessageType:STIMMessageType_Revoke];
                [_dataSounce replaceObjectAtIndex:index withObject:message];
                [[STIMKit sharedInstance] updateMsg:message ByJid:self.robotJId];
                [_tableView reloadData];
                ...
                NSMutableDictionary *dicInfo = [NSMutableDictionary dictionary];
                [dicInfo setObject:[(STIMMessageModel *)message from] forKey:@"fromId"];
                [dicInfo setObject:[(STIMMessageModel *)message messageId] forKey:@"messageId"];
                [dicInfo setObject:[(STIMMessageModel *)message message] forKey:@"message"];
                NSString *msgInfo = [[STIMJSONSerializer sharedInstance] serializeObject:dicInfo];
                
                [[STIMKit sharedInstance] revokeMessageWithMessageId:[(STIMMessageModel *)message messageId] message:msgInfo ToJid:self.robotJId];
                break;
            }
        }
        ...
        for (STIMMessageModel *msg in _dataSounce) {
            ...
            if ([msg.messageId isEqualToString:[(STIMMessageModel *)message messageId]]) {
                ...
                [[STIMMyFavoitesManager sharedMyFavoritesManager] setMyFavoritesArrayWithMsg:message];
...
- (id <STIMMWPhoto>)photoBrowser:(STIMMWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index
{
    ...
    NSString *imagePath;
    for (STIMDisplayImage *image in [_photos allValues]) {
        if (image.imageIndex == index) {
            imagePath = image.imagePath;
        }
    }
    if ([imagePath stimDB_hasPrefixHttpHeader]) {
        ...
        return image?[[STIMMWPhoto alloc] initWithImage:image]:nil;
    }
}
...
- (void)loadNewPublicNumberMsgList {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *list = [[STIMKit sharedInstance] getPublicNumberMsgListById:self.robotJId WithLimit:kPageCount WithOffset:(int)_dataSounce.count];
        dispatch_async(dispatch_get_main_queue(), ^{
...
{
   STIMMessageModel *  msg = nil; 
    msg = [[STIMKit sharedInstance] createPublicNumberMessageWithMsg:@"[obj type=\"image\" value=\"imageData\"]" extenddInfo:nil publicNumberId:self.robotJId msgType:PublicNumberMsgType_Text];
    ...
    [[STIMKit sharedInstance] uploadFileForData:imageData forMessage:msg withJid:self.robotJId isFile:YES];
    return nil;
}
...
//            [self updateTextCache:msg.messageId Cache:cache];
            if (cache) {
                if (cache.images.count > 0) {
                    ...
                    STIMVerboseLog(@"cache.imagescache.imagescache.imagescache.images%@",cache.images);
                    if (imagePath) {
                        STIMDisplayImage *displayImage = [[STIMDisplayImage alloc] init];
