...
- (instancetype)initWithKey:(NSString *)key value:(NSString *)value color:(UIColor *)color {
    if (self = [self init]) {
        ...
        _keyLabel.textColor = [UIColor stimDB_colorWithHex:kContentColor alpha:1.0];
        ...
        _valueLabel.textColor = color?color:[UIColor stimDB_colorWithHex:kContentColor alpha:1.0];
...
@interface STIMExtensibleProductCell () {
...
@implementation STIMExtensibleProductCell

+ (float)getCellHeightForProductInfo:(NSString *)infoStr {

    NSDictionary * proDic = [[STIMJSONSerializer sharedInstance] deserializeObject:infoStr error:nil];
    float height = 0;
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _bgView.layer.borderColor = [UIColor stimDB_colorWithHex:kSeplineColor alpha:1.0].CGColor;
        ...
        _titleLabel.textColor = [UIColor stimDB_colorWithHex:kTitleColor alpha:1.0];
        [_bgView addSubview:_titleLabel];
        
        _sepLine = [[UIView alloc] initWithFrame:CGRectZero];
        _sepLine.backgroundColor = [UIColor stimDB_colorWithHex:kSeplineColor alpha:1.0];
...
- (void)setProDcutInfoDic:(NSDictionary *)infoDic {
    ...
    [_titleIcon stimDB_setImageWithURL:[NSURL URLWithString:infoDic[@"titleimg"]] placeholderImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"vacation"]];
    _titleLabel.text = infoDic[@"titletxt"];
    
    NSString * imageStr = infoDic[@"productimg"];
    if (imageStr.length) {
        [_productImageView stimDB_setImageWithURL:[NSURL URLWithString:imageStr]];
        [_bgView addSubview:_productImageView];
    }else{
    ...
    for (NSDictionary * itemDic in descs) {
        UIView * view =  [[QCKeyValueView alloc] initWithKey:itemDic[@"k"] value:itemDic[@"v"] color:[UIColor stimDB_colorWithHex:[itemDic[@"c"] integerValue] alpha:1.0]];
