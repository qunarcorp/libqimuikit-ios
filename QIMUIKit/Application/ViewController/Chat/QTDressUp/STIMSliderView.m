...
#define kMinFlagText            [NSBundle stimDB_localizedStringForKey:@"font_size_small"];
#define kPlhdrFlagText          [NSBundle stimDB_localizedStringForKey:@"font_size_standard"];
#define kMaxFlagText            [NSBundle stimDB_localizedStringForKey:@"font_size_big"];
...
@implementation STIMSliderView

-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        [self setUpUI];
        float currentFontSize = [[STIMCommonFont sharedInstance] currentFontSize];
        _lastIndex = 0;
...
- (void)setUpUI{
    ...
    if (_tolView == nil) {
        ...
        _tolView.backgroundColor = [UIColor stimDB_colorWithHex:kScaleColorHex alpha:1.0];
        ...
        for (int i = 0; i < kNumOfPoint; i ++) {
            ...
            perView.backgroundColor = [UIColor stimDB_colorWithHex:kScaleColorHex alpha:1.0];
        ...
        if (_iconFlagImgView == nil) {
            ...
            _iconFlagImgView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"dynamicfontprogress"];
    ...
    if (_minFlagLabel == nil) {
        ...
        _minFlagLabel.font = [UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - kFontAmp];
    ...
    if (_plhdrFlagLabel == nil) {
        ...
        _plhdrFlagLabel.font = [UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize]];
    ...
    if (_maxFlagLabel == nil) {
        ...
        _maxFlagLabel.font = [UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] + (kNumOfPoint - 2) * kFontAmp];
...
- (void)setIconFlagToIndex:(NSInteger)index{
    if (index < kNumOfPoint) {
        ...
        [[STIMCommonFont sharedInstance] setCurrentFontSize:FONT_SIZE + (index - 1) * kFontAmp];
