...
@interface STIMMyEmotionsManagerCell ()
...
@implementation STIMMyEmotionsManagerCell
...
- (UIButton *)removeBtn {
    if (!_removeBtn) {
        ...
        [_removeBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Remove"] forState:UIControlStateNormal];
...
- (void)removeEmotionPackage:(UIButton *)sender {
    [[STIMProgressHUD sharedInstance] showProgressHUDWithTest:[NSBundle stimDB_localizedStringForKey:@"Removing"]];
    [[STIMEmotionManager sharedInstance] removeEmotionPkgForPkId:self.pkID];
}
