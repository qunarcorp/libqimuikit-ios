//
//  STIMImagePickerController.m
//  STIMImagePickerController
//
//  Created by 谭真 on 15/12/24.
//  Copyright © 2015年 谭真. All rights reserved.
//  version 3.2.0 - 2019.03.02
//  更多信息，请前往项目的github地址：https://github.com/banchichen/STIMImagePickerController
...
@interface STIMImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
/// Default is 4, Use in photos collectionView in STIMPhotoPickerController
/// 默认4列, STIMPhotoPickerController中的照片collectionView
@property (nonatomic, assign) NSInteger columnNumber;
@end

@implementation STIMImagePickerController
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[STIMImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[STIMImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<STIMImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<STIMImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<STIMImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[STIMImagePickerManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    STIMPhotoPreviewController *previewVc = [[STIMPhotoPreviewController alloc] init];
...
- (void)setTakePictureImageName:(NSString *)takePictureImageName {
    ...
    _takePictureImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:takePictureImageName];
}

- (void)setPhotoSelImageName:(NSString *)photoSelImageName {
    _photoSelImageName = photoSelImageName;
    _photoSelImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:photoSelImageName];
}

- (void)setPhotoDefImageName:(NSString *)photoDefImageName {
    _photoDefImageName = photoDefImageName;
    _photoDefImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:photoDefImageName];
}

- (void)setPhotoNumberIconImageName:(NSString *)photoNumberIconImageName {
    _photoNumberIconImageName = photoNumberIconImageName;
    _photoNumberIconImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:photoNumberIconImageName];
}
...
- (void)setPhotoPreviewOriginDefImageName:(NSString *)photoPreviewOriginDefImageName {
    ...
    _photoPreviewOriginDefImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:photoPreviewOriginDefImageName];
}

- (void)setPhotoOriginDefImageName:(NSString *)photoOriginDefImageName {
    _photoOriginDefImageName = photoOriginDefImageName;
    _photoOriginDefImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:photoOriginDefImageName];
}

- (void)setPhotoOriginSelImageName:(NSString *)photoOriginSelImageName {
    _photoOriginSelImageName = photoOriginSelImageName;
    _photoOriginSelImage = [UIImage qim_imageNamedFromSTIMImagePickerBundle:photoOriginSelImageName];
}
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [STIMImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [STIMImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}

- (void)setNotScaleImage:(BOOL)notScaleImage {
    _notScaleImage = notScaleImage;
    [STIMImagePickerConfig sharedInstance].notScaleImage = notScaleImage;
}

- (void)setNeedFixComposition:(BOOL)needFixComposition {
    _needFixComposition = needFixComposition;
    [STIMImagePickerConfig sharedInstance].needFixComposition = needFixComposition;
}
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        STIMPhotoPickerController *photoPickerVc = [[STIMPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[STIMImagePickerManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage needFetchAssets:NO completion:^(QIMAlbumModel *model) {
...
- (void)setPickerDelegate:(id<STIMImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [STIMImagePickerManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [STIMImagePickerManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [STIMImagePickerManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        STIMAssetModel *model = [STIMAssetModel modelWithAsset:asset type:[[STIMImagePickerManager manager] getAssetType:asset]];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [STIMImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [STIMImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [STIMImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [STIMImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [STIMImagePickerManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)addSelectedModel:(STIMAssetModel *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.asset.localIdentifier];
}

- (void)removeSelectedModel:(STIMAssetModel *)model {
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        STIMImagePickerController *imagePickerVc = (STIMImagePickerController *)self.navigationController;
        [[STIMImagePickerManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<QIMAlbumModel *> *models) {
...
@implementation STIMCommonTools
...
@implementation STIMImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static STIMImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[STIMImagePickerConfig alloc] init];
