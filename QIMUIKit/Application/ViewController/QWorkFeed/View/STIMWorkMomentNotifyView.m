...
@interface STIMWorkMomentNotifyView ()
...
@implementation STIMWorkMomentNotifyView

- (instancetype)initWithNewMsgCount:(NSInteger)msgCount {
    if ([[STIMKit sharedInstance] getIsIpad] == YES) {
        self = [super initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] stimDB_rightWidth], 54)];
    } else {
        self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 54)];
    }
    if (self) {
        self.backgroundColor = [UIColor stimDB_colorWithHex:0xF8F8F9];
        ...
        remindView.backgroundColor = [UIColor stimDB_colorWithHex:0xFF5C28];
        ...
        [self.unreadCountLabel setTextColor:[UIColor stimDB_colorWithHex:0x00CABE]];
        ...
        [disclosureIndicatorView setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e42e" size:30 color:[UIColor stimDB_colorWithHex:0xBABABA]]]];
