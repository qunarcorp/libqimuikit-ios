...
@interface STIMMsgBaloonBaseCell() <STIMMenuImageViewDelegate>

@property (nonatomic, strong) STIMTextContainer *textContainer;

@end

@implementation STIMMsgBaloonBaseCell

+ (UIImage *)leftBallocImage {
    //return nil时，自动用贝塞尔曲线画气泡
    return nil;
    if (__leftBallocImage == nil) {
        NSData *data = UIImagePNGRepresentation([UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"leftBalloon"]);
...
+ (UIImage *)rightBallcoImage{
    ...
    if (__rightBallocImage == nil) {
        NSData *data = UIImagePNGRepresentation([UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"rightBalloon"]);
        UIImage *image = [[UIImage alloc] initWithData:data scale:[[UIScreen mainScreen] scale]];
        CGFloat width = image.size.width / 2.0;
        CGFloat height = image.size.height / 2.0;
...
- (void)setMessage:(STIMMessageModel *)message {
   STIMMessageModel *tempMsg = _message;
    _message = message;
...
- (void)updateMessageSendStateByNotification:(NSNotification *)notify {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSDictionary *notifyDic = notify.object;
        STIMMessageSendState sendState = [[notifyDic objectForKey:@"MsgSendState"] unsignedIntegerValue];
...
- (void)initBackViewAndHeaderName{
    ...
    self.backView = [[STIMMenuImageView alloc] initWithFrame:CGRectZero];
...
- (UIImageView *)HeadView {
    if (!_HeadView) {
        ...
        _HeadView.backgroundColor = [UIColor stimDB_colorWithHex:0x9e9e9e alpha:1.0];
        _HeadView.image = [UIImage imageWithData:[STIMKit defaultUserHeaderImage]];
    }
    return _HeadView;
}
...
- (UIButton *)statusButton {
    if (!_statusButton) {
        ...
        [_statusButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"MessageSendFail"] forState:UIControlStateNormal];
...
- (UILabel *)messgaeStateLabel {
    if (!_messgaeStateLabel) {
        ...
        _messgaeStateLabel.textColor = [UIColor stimDB_colorWithHex:0x15b0f9 alpha:1.0];
    }
...
+ (CGFloat)getCellHeightWithMessage:(STIMMessageModel *)message  chatType:(ChatType)chatType{
    @throw  [NSException exceptionWithName:@"STIMMsgBaloonBaseCell Exception" reason:[NSString stringWithFormat:@"Class %@ \"getCellHeightWithMessage\" method has not realized ",[self class]] userInfo:nil];
}
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated {
    ...
    if (self.editing) {
        ...
        if (self.message.messageDirection == STIMMessageDirection_Sent) {
            ...
            if (self.message.messageType == STIMMessageType_SmallVideo) {
...
- (void)updateNameLabel {
    if (self.message.messageDirection == STIMMessageDirection_Received) {
        ...
        dispatch_async([[STIMKit sharedInstance] getLoadMsgNickNameQueue], ^{
            if (self.chatType != ChatType_CollectionChat) {
                //备注
                NSString * remarkName = [[STIMKit sharedInstance] getUserMarkupNameWithUserId:self.message.from];
                if (remarkName.length > 0) {
                    nickName = remarkName;
                }
            } else {
                NSDictionary *userInfo = [[STIMKit sharedInstance] getCollectionUserInfoByUserId:nickName];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.nameLabel.textColor = [UIColor stimDB_colorWithHex:0x888888];
            });
        });
    }
}
...
- (void)updateMedalListView {
    if (self.message.messageDirection == STIMMessageDirection_Received) {
        dispatch_async([[STIMKit sharedInstance] getLoadMsgMedalListQueue], ^{
           
            NSArray *medals = [[STIMKit sharedInstance] getUserWearMedalSmallIconListByUserid:self.message.from];
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (NSInteger i = 0; i < medals.count; i++) {
                    ...
                    [medalIconView stimDB_setImageWithURL:[NSURL URLWithString:medals[i]] placeholderImage:[UIImage new]];
                    marginLeft += (18+5);
...
- (void)updateMessageReadState {
    if (self.message.messageDirection == STIMMessageDirection_Sent && self.message.messageSendState == STIMMessageSendState_Success) {
        ...
        BOOL readFlag = (self.message.messageReadState & STIMMessageRemoteReadStateDidReaded) == STIMMessageRemoteReadStateDidReaded;
        if (readFlag) {
...
//            self.messgaeStateLabel.textColor = [UIColor lightGrayColor];
            ...
            self.messgaeStateIcon.hidden = NO;
            self.messgaeStateIcon.image = [UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:stimDB_messageReadState_font size:20 color:stimDB_messageReadState_color]];
            
            self.statusButton.hidden = YES;
        } else {
            
            BOOL sentFlag = (self.message.messageReadState & STIMMessageRemoteReadStateDidSent) == STIMMessageRemoteReadStateDidSent;
            if (sentFlag) {
                [self.indicatorView stopAnimating];
                self.indicatorView.hidden = YES;
//                self.messgaeStateLabel.hidden = NO;
                self.messgaeStateIcon.hidden = NO;
                self.messgaeStateIcon.image = [UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:stimDB_messageUnReadState_font size:20 color:stimDB_messageUnReadState_color]];
                self.statusButton.hidden = YES;
//                [_sessionButton setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f0f3" size:32 color:[UIColor stimDB_colorWithHex:0x616161 alpha:1.0]]] forState:UIControlStateNormal];

//                self.messgaeStateLabel.text = @"未读";
//                self.messgaeStateLabel.textColor = [UIColor stimDB_colorWithHex:0x15b0f9 alpha:1.0];
            } else {
                [self.indicatorView stopAnimating];
                self.indicatorView.hidden = YES;
//                self.messgaeStateLabel.hidden = NO;
...
- (void)updateMessageSendState {
    ...
    if (self.message.messageDirection == STIMMessageDirection_Sent) {
        ...
        if (![[STIMKit sharedInstance] isMiddleVirtualAccountWithJid:self.message.to] && self.message.chatType != ChatType_GroupChat && self.message.chatType != ChatType_System && self.chatType != ChatType_GroupChat && self.chatType != ChatType_System && self.chatType != ChatType_CollectionChat) {
            if ([[STIMKit sharedInstance] qimNav_Showmsgstat]) {
//                [self.contentView addSubview:self.messgaeStateLabel];
                [self.contentView addSubview:self.messgaeStateIcon];
            }
        }
        if (self.message.messageSendState == STIMMessageSendState_Waiting || self.message.messageSendState == STIMMessageSendState_Faild) {
            self.message.messageSendState = [[STIMKit sharedInstance] getMessageStateWithMsgId:self.message.messageId];
        }
        if ([[[STIMKit sharedInstance] qimNav_getDebugers] containsObject:[STIMKit getLastUserName]]) {
            self.messgaeRealStateLabel.frame = CGRectMake(self.backView.left - 80, self.backView.top, 70, 24);
            [self.contentView addSubview:self.messgaeRealStateLabel];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            switch (self.message.messageSendState) {
                case STIMMessageSendState_Waiting: {
                    self.indicatorView.center = CGPointMake(self.backView.left - 24, self.backView.centerY);
                    [self.contentView addSubview:self.indicatorView];
                    [self.indicatorView startAnimating];
                    self.messgaeRealStateLabel.text = [NSString stringWithFormat:@" %@-%@", [NSBundle stimDB_localizedStringForKey:@"Sending"], self.message.messageId];
                }
                    break;
                case STIMMessageSendState_Faild: {
                    ...
                    break;
                case STIMMessageSendState_Success: {
...
- (void)onHeaderViewClick:(UITapGestureRecognizer *)tapGesture {
    if (self.message.from.length > 0 && self.chatType != ChatType_CollectionChat && self.chatType != ChatType_Consult) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STIMFastEntrance openUserCardVCByUserId:self.message.from];
        });
    } else if (self.message.xmppId.length > 0 && self.chatType == ChatType_Consult && self.message.messageDirection == STIMMessageDirection_Received) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STIMFastEntrance openUserCardVCByUserId:self.message.xmppId];
        });
    } else {
...
- (void)onMedalViewClick:(UITapGestureRecognizer *)tap {
    if (self.message.from.length > 0 && self.chatType != ChatType_CollectionChat && self.chatType != ChatType_Consult) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STIMFastEntrance openUserMedalFlutterWithUserId:self.message.from];
        });
    } else if (self.message.xmppId.length > 0 && self.chatType == ChatType_Consult && self.message.messageDirection == STIMMessageDirection_Received) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STIMFastEntrance openUserMedalFlutterWithUserId:self.message.xmppId];
        });
    } else {
...
- (void)onMenuActionWithType:(MenuActionType)actionType {
    switch (actionType) {
        case MA_Copy: {
            ...
            for (STIMTextStorage *textStorage in self.textContainer.textStorages) {
                if (![textStorage isKindOfClass:[STIMTextStorage class]]) {
                    continue;
                } else {
            ...
            break;
        case MA_Collection: {
            self.textContainer = [STIMMessageParser textContainerForMessage:self.message fromCache:NO];
            for (STIMImageStorage * imageStorage in self.textContainer.textStorages) {
                
                if (![imageStorage isKindOfClass:[STIMImageStorage class]]) {
                    
                    return;
                } else {
                    NSString *imageUrl = imageStorage.imageURL.absoluteString;
                    STIMVerboseLog(@"收藏表情后的地址为 : %@", imageUrl);
                    if (![imageUrl containsString:@"null"] && imageUrl.length > 0) {
                        ...
                        [[STIMCollectionFaceManager sharedInstance] insertCollectionEmojiWithEmojiUrl:imageUrl];
                    } else {
                        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)refreshHeaderView {
    if (self.message.messageType == STIMMessageType_GroupNotify || self.message.messageType == STIMMessageType_Time || self.message.messageType == STIMMessageType_Revoke || self.message.messageType == STIMMessageTypeRobotQuestionList) {
        self.HeadView.hidden = YES;
        self.nameLabel.hidden = YES;
        self.medalListView.hidden = YES;
        return;
    }
    dispatch_async([[STIMKit sharedInstance] getLoad_msgHeaderImageQueue], ^{
        ...
        if (self.chatType == ChatType_CollectionChat) {
            NSString *collectionUserUrl = [[STIMKit sharedInstance] getCollectionUserHeaderUrlWithXmppId:self.message.from];
            if (![collectionUserUrl stimDB_hasPrefixHttpHeader]) {
                collectionUserUrl = [NSString stringWithFormat:@"%@/%@", [[STIMKit sharedInstance] qimNav_InnerFileHttpHost], collectionUserUrl];
            }
            [self.HeadView stimDB_setImageWithURL:collectionUserUrl placeholderImage:[UIImage imageWithData:[STIMKit defaultUserHeaderImage]]];
        } else if (self.chatType == ChatType_Consult) {
            if (self.message.messageDirection == STIMMessageDirection_Sent) {
                [self.HeadView stimDB_setImageWithJid:[[STIMKit sharedInstance] getLastJid] WithChatType:ChatType_SingleChat];
            } else {
                [self.HeadView stimDB_setImageWithJid:self.message.xmppId];
            }
        } else {
            if (self.message.messageDirection == STIMMessageDirection_Sent) {
                [self.HeadView stimDB_setImageWithJid:[[STIMKit sharedInstance] getLastJid] WithChatType:ChatType_SingleChat];
