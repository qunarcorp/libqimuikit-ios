...
@interface STIMRobotQuestionCell()<STIMMenuImageViewDelegate,UIActionSheetDelegate,STIMAttributedLabelDelegate>
{
    UIView               * _bgView;
...
@implementation STIMRobotQuestionCell
@dynamic delegate;
...
- (void)setMessage:(STIMMessageModel *)message {
    ...
    _textContainer = [STIMMessageParser textContainerForMessageCtnt:infoDic[@"content"] withId:message.messageId direction:message.messageDirection];
}
...
- (void)refreshUI {
    ...
    if (_textContainer) {
        [_textLabel setFrameWithOrign:CGPointMake((STIMMessageDirection_Received == self.message.messageDirection) ? kQCIMMsgCellCtntMargin :kQCIMMsgCellCtntMargin - 3,kQCIMMsgCellCtntMargin) Width:[STIMMessageParser getCellWidth]];
    } else {
        [_textLabel setFrameWithOrign:CGPointMake(0,0) Width:[STIMMessageParser getCellWidth]];
    }
    NSString *jsonStr = self.message.extendInformation.length > 0 ? self.message.extendInformation : self.message.message;
    NSDictionary *infoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:jsonStr error:nil];
    CGFloat maxWidth = screenW - 2*kIMChatMargin;
    ...
    if (listTips.length > 0) {
        ...
        float actionBtnHeight = [listTips stimDB_sizeWithFontCompatible:kIMChatContentFont constrainedToSize:CGSizeMake(backWidth - kIMChatContentLeft - kIMChatContentRight, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping].height + 5;
        ...
        [listTipsLabel setTextColor:[UIColor stimDB_colorWithHex:0x212121]];
    ...
    if ([type.lowercaseString isEqualToString:@"list"]) {
        ...
        if (items && items.count > 0) {
            ...
            for (NSDictionary * item in items) {
                if ([item objectForKey:@"text"]) {
                    float actionBtnHeight = [[item objectForKey:@"text"] stimDB_sizeWithFontCompatible:kIMChatContentFont constrainedToSize:CGSizeMake(backWidth, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping].height + 5;
            ...
            if (items.count > initSize) {
                ...
                [itemAction setTextColor:[UIColor stimDB_colorWithHex:0x616161]];
                ...
                [itemIcon setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f3c7" size:20 color:[UIColor colorWithRed:33/255.0 green:33/255.0 blue:33/255.0 alpha:1/1.0]]]];
