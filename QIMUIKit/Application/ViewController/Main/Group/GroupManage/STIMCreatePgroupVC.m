...
@interface STIMCreatePgroupVC ()<UITableViewDataSource,UITableViewDelegate,SelectionResultDelegate,UITextFieldDelegate>
{
...
@implementation STIMCreatePgroupVC
...
//    
//    [_navBar setTitle:[NSBundle stimDB_localizedStringForKey:@"Create Group"]];
    [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"Create Group"]];
}
...
- (void)goBack:(id)sender{
    ...
    //    [[STIMKit sharedInstance] updateMessageReadStateWithSessionId:self.chatSession.sessionId];
...
-(void)setupTableCellNameSubs:(UIView *)parentView  indexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        [headView setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"singleHeaderDefault" ]];
        ...
        [STIMViewHelper setTextFieldLeftView:groupNameTextField];
    ...
    if (indexPath.section == 1) {
        ...
        if (indexPath.row == 0) {
            ...
            [arrowView setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"icon_arrow_r" ]];
    ...
    else if(indexPath.section == 2)
    {
        ...
        [STIMViewHelper setRadiusToView:createGroupBtn];
...
-(void)createGroup
{
    if ( ( [[groupNameTextField text] length] > 0 )&& ([_selectionArray count] > 0)) {
        ...
        //TODO:no such method
//        if ([[STIMKit sharedInstance] createGroupByGroupName:groupName WithMyNickName:[infoDic objectForKey:@"Name"] WithInviteMember:temp WithSetting:settingDic WithTopic:_groupTopic.stringValue])
//        [[STIMKit sharedInstance] createGroupByGroupName:[groupNameTextField text]
//                                            WithMyNickName:[[STIMKit sharedInstance] getMyNickName]
//                                          WithInviteMember:_selectionArray WithSetting:[[STIMKit sharedInstance] defaultGroupSetting]
//                                                 WithTopic:@""];
        [[STIMKit sharedInstance] createGroupByGroupName:[groupNameTextField text]
                                            WithMyNickName:[[STIMKit sharedInstance] getMyNickName]
...
//        [[STIMKit sharedInstance] createGroupByName:[groupNameTextField text] WithSelectionMember:_selectionArray];
...
//        [VCController popVCAnimated:YES];
    ...
    else
    {
        if ([[groupNameTextField text] length] == 0) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"群名称不能为空" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
            [alertView show];
        }
        else if([_selectionArray count] == 0)
        {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"请选择群成员" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
            [alertView show];
        }
