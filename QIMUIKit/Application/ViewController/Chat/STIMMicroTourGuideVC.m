...
@interface STIMMicroTourGuideVC ()<UIWebViewDelegate>{
...
@implementation STIMMicroTourGuideVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    _dataSource = [NSMutableArray array];
    NSArray *msgList = [[STIMKit sharedInstance] getPublicNumberMsgListById:self.userId WithLimit:50 WithOffset:0];
...
- (void)initMessage:(STIMMessageModel *)msg{
    switch (msg.messageType) {
        case STIMMessageType_Time:
        {
...
//            [self appendTimeStemp:timeStr];
            ...
            break;
        case STIMMessageType_MicroTourGuide:
        {
            NSString *element = [msg message];
            NSDictionary *contentDic = [[STIMJSONSerializer sharedInstance] deserializeObject:element error:nil];
            element = [contentDic objectForKey:@"htmlcontent"];
            if (element.length > 0) {
                NSString *data = [[STIMJSONSerializer sharedInstance] serializeObject:@{@"htmlcontent":element,@"time":@([[NSDate stimDB_dateWithTimeIntervalInMilliSecondSince1970:msg.messageDate] stimDB_timeIntervalSince1970InMilliSecond])}];
...
- (void)initWebView{
    ...
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"STIMMicroTourRoot" ofType:@"html"];
...
- (void)initDataSource{
    for (STIMMessageModel *msg in _dataSource) {
        [self initMessage:msg];
    }
    [self scrollToBottom];
}
