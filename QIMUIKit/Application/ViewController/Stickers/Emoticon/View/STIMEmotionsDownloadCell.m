...
@interface STIMEmotionsDownloadCell ()
...
@implementation STIMEmotionsDownloadCell

- (void)setEmotion:(STIMEmotion *)emotion {
    
    _emotion = emotion;
    
    [self.iconView stimDB_setImageWithURL:[NSURL URLWithString:emotion.thumb] placeholderImage:nil];
...
- (UIButton *)downloadBtn {
    ...
    if (!_downloadBtn) {
        ...
        [_downloadBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Download"] forState:UIControlStateNormal];
...
-(void)layoutSubviews{
    ...
    switch (_state) {
        ...
        {
            [_downloadBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Update"] forState:UIControlStateNormal];
            [_downloadBtn.layer setBorderColor:[UIColor qtalkIconSelectColor].CGColor];
            [_downloadBtn setTitleColor:[UIColor qtalkIconSelectColor] forState:UIControlStateNormal];
            [_downloadBtn setEnabled:YES];
        }
        ...
        {
            [_downloadBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Downloaded"] forState:UIControlStateNormal];
...
//        [[STIMEmotionManager sharedInstance] downloadEmotionForPkId:_pkID fileName:_pkID];
        [[STIMProgressHUD sharedInstance] showProgressHUDWithTest:[NSBundle stimDB_localizedStringForKey:@"Downloading"]];
        [[STIMEmotionManager sharedInstance] downloadEmotionForPkId:_pkID loadUrl:_loadURL];
        [self.downloadBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Downloading"] forState:UIControlStateDisabled];
        [self.downloadBtn setEnabled:NO];
    }
//    else if(_state == EmotionStateDone){
//        [[STIMEmotionManager sharedInstance] removeEmotionPkgForPkId:_pkID];
