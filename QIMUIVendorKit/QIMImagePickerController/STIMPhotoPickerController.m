...
#import "STIMPhotoPickerController.h"
#import "STIMImagePickerController.h"
#import "STIMPhotoPreviewController.h"
#import "STIMAssetCell.h"
#import "STIMAssetModel.h"
#import "UIView+QIMLayout.h"
#import "STIMImagePickerManager.h"
#import "STIMVideoPlayerController.h"
#import "STIMGifPhotoPreviewController.h"
#import "STIMLocationManager.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import "STIMImageRequestOperation.h"

@interface STIMPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@property (nonatomic, strong) QIMCollectionView *collectionView;
...
@implementation STIMPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (@available(iOS 9, *)) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[STIMImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[STIMImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    STIMImagePickerController *tzImagePickerVc = (STIMImagePickerController *)self.navigationController;
    ...
    if (tzImagePickerVc.navLeftBarButtonSettingBlock) {
        ...
        [tzImagePickerVc.childViewControllers firstObject].navigationItem.backBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle qim_localizedStringForKey:@"Back"] style:UIBarButtonItemStylePlain target:nil action:nil];
    }
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[STIMImagePickerManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(QIMAlbumModel *model) {
                self->_model = model;
            ...
            if (self->_showTakePhotoBtn || self->_isFirstAppear) {
                [[STIMImagePickerManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<STIMAssetModel *> *models) {
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    STIMImagePickerController *tzImagePicker = (STIMImagePickerController *)self.navigationController;
    if (tzImagePicker && [tzImagePicker isKindOfClass:[STIMImagePickerController class]]) {
...
- (void)configCollectionView {
    ...
    _collectionView = [[QIMCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
    ...
    if (_showTakePhotoBtn) {
        _collectionView.contentSize = CGSizeMake(self.view.qim_width, ((_model.count + self.columnNumber) / self.columnNumber) * self.view.qim_width);
    } else {
        _collectionView.contentSize = CGSizeMake(self.view.qim_width, ((_model.count + self.columnNumber - 1) / self.columnNumber) * self.view.qim_width);
        if (_models.count == 0) {
            _noDataLabel = [UILabel new];
            _noDataLabel.textAlignment = NSTextAlignmentCenter;
            _noDataLabel.text = [NSBundle qim_localizedStringForKey:@"No Photos or Videos"];
    ...
    [_collectionView registerClass:[STIMAssetCell class] forCellWithReuseIdentifier:@"STIMAssetCell"];
    [_collectionView registerClass:[QIMAssetCameraCell class] forCellWithReuseIdentifier:@"QIMAssetCameraCell"];
}
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, [STIMCommonTools qim_isRightToLeftLayout] ? 10 : -10, 0, 0);
    ...
    if (tzImagePickerVc.photoPickerPageUIConfigBlock) {
        tzImagePickerVc.photoPickerPageUIConfigBlock(_collectionView, _bottomToolBar, _previewButton, nil, _originalPhotoButton, _originalPhotoLabel, _doneButton, _numberImageView, _numberLabel, _divideLine);
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [STIMCommonTools qim_isIPhoneX] ? 50 + (83 - 49) : 50;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden) top += [STIMCommonTools qim_statusBarHeight];
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.qim_height - toolBarHeight - top : self.view.qim_height - top;;
    } else {
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.qim_height - toolBarHeight : self.view.qim_height;
    }
    _collectionView.frame = CGRectMake(0, top, self.view.qim_width, collectionViewHeight);
    _noDataLabel.frame = _collectionView.bounds;
    CGFloat itemWH = (self.view.qim_width - (self.columnNumber + 1) * itemMargin) / self.columnNumber;
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        toolBarTop = self.view.qim_height - toolBarHeight;
    } else {
        CGFloat navigationHeight = naviBarHeight + [STIMCommonTools qim_statusBarHeight];
        toolBarTop = self.view.qim_height - toolBarHeight - navigationHeight;
    }
    _bottomToolBar.frame = CGRectMake(0, toolBarTop, self.view.qim_width, toolBarHeight);
    ...
    _previewButton.qim_width = !tzImagePickerVc.showSelectBtn ? 0 : previewWidth;
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
    ...
    _doneButton.frame = CGRectMake(self.view.qim_width - _doneButton.qim_width - 12, 0, _doneButton.qim_width, 50);
    _numberImageView.frame = CGRectMake(_doneButton.qim_left - 24 - 5, 13, 24, 24);
    _numberLabel.frame = _numberImageView.frame;
    _divideLine.frame = CGRectMake(0, 0, self.view.qim_width, 1);
    
    [STIMImagePickerManager manager].columnNumber = [STIMImagePickerManager manager].columnNumber;
    [STIMImagePickerManager manager].photoWidth = tzImagePickerVc.photoWidth;
    [self.collectionView reloadData];
    
    if (tzImagePickerVc.photoPickerPageDidLayoutSubviewsBlock) {
        tzImagePickerVc.photoPickerPageDidLayoutSubviewsBlock(_collectionView, _bottomToolBar, _previewButton, nil, _originalPhotoButton, _originalPhotoLabel, _doneButton, _numberImageView, _numberLabel, _divideLine);
    }
}
...
- (void)previewButtonClick {
    STIMPhotoPreviewController *photoPreviewVc = [[STIMPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.minImagesCount && tzImagePickerVc.selectedModels.count < tzImagePickerVc.minImagesCount) {
        NSString *title = [NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Select a minimum of %zd photos"], tzImagePickerVc.minImagesCount];
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            STIMAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [STIMImagePickerManager manager].shouldFixOrientation = YES;
        __block UIAlertController *alertView;
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            STIMAssetModel *model = tzImagePickerVc.selectedModels[i];
            STIMImageRequestOperation *operation = [[STIMImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                if (isDegraded) return;
                if (photo) {
                    if (![STIMImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[STIMImagePickerManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }
                ...
                if (progress < 1 && havenotShowAlert && !alertView) {
                    [tzImagePickerVc hideProgressHUD];
                    alertView = [tzImagePickerVc showAlertWithTitle:[NSBundle qim_localizedStringForKey:@"Synchronizing photos from iCloud"]];
                    havenotShowAlert = NO;
                    return;
                }
                if (progress >= 1) {
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.maxImagesCount == 1) {
        if ([[STIMImagePickerManager manager] isVideo:[assets firstObject]]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        QIMAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"QIMAssetCameraCell" forIndexPath:indexPath];
    ...
    STIMAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"STIMAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        STIMImagePickerController *tzImagePickerVc = (STIMImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (STIMAssetModel *model_item in selectedModels) {
            ...
            if (tzImagePickerVc.showSelectedIndex || tzImagePickerVc.showPhotoCannotSelectLayer) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"QIM_PHOTO_PICKER_RELOAD_NOTIFICATION" object:strongSelf.navigationController];
            }
            [UIView showOscillatoryAnimationWithLayer:strongLayer type:QIMOscillatoryAnimationToSmaller];
        } else {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSString *title = [NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Select a maximum of %zd photos"], tzImagePickerVc.maxImagesCount];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        }
    };
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMAssetModel *model = _models[index];
    if (model.type == STIMAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            STIMImagePickerController *imagePickerVc = (STIMImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle qim_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            STIMVideoPlayerController *videoPlayerVc = [[STIMVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == STIMAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            STIMImagePickerController *imagePickerVc = (STIMImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle qim_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            STIMGifPhotoPreviewController *gifPreviewVc = [[STIMGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        NSDictionary *infoDict = [STIMCommonTools qim_getInfoDictionary];
        // 无权限 做一个友好的提示
        NSString *appName = [infoDict valueForKey:@"CFBundleDisplayName"];
        if (!appName) appName = [infoDict valueForKey:@"CFBundleName"];
        
        NSString *message = [NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Please allow %@ to access your camera in \"Settings -> Privacy -> Camera\""],appName];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle qim_localizedStringForKey:@"Can not use camera"] message:message delegate:self cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"Cancel"] otherButtonTitles:[NSBundle qim_localizedStringForKey:@"Setting"], nil];
        [alert show];
...
- (void)pushImagePickerController {
    ...
    if (tzImagePickerVc.allowCameraLocation) {
        __weak typeof(self) weakSelf = self;
        [[STIMLocationManager manager] startLocationWithSuccessBlock:^(NSArray<CLLocation *> *locations) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (void)pushPhotoPrevireViewController:(STIMPhotoPreviewController *)photoPreviewVc {
    [self pushPhotoPrevireViewController:photoPreviewVc needCheckSelectedModels:NO];
}

- (void)pushPhotoPrevireViewController:(STIMPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[STIMImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.qim_width <= 320) {
        return;
    }
    STIMImagePickerController *imagePickerVc = (STIMImagePickerController *)self.navigationController;
    [[STIMImagePickerManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    STIMImagePickerController *tzImagePickerVc = (STIMImagePickerController *)self.navigationController;
    for (STIMAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[STIMImagePickerManager manager] savePhotoWithImage:photo location:self.location completion:^(PHAsset *asset, NSError *error){
                if (!error) {
        ...
        if (videoUrl) {
            [[STIMImagePickerManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                if (!error) {
                    [self addPHAsset:asset];
                }
            }];
...
- (void)addPHAsset:(PHAsset *)asset {
    STIMAssetModel *assetModel = [[STIMImagePickerManager manager] createModelWithAsset:asset];
    STIMImagePickerController *tzImagePickerVc = (STIMImagePickerController *)self.navigationController;
    [tzImagePickerVc hideProgressHUD];
    if (tzImagePickerVc.sortAscendingByModificationDate) {
    ...
    if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
        if (assetModel.type == STIMAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
            // 不能多选视频的情况下，不选中拍摄的视频
        } else {
...
- (void)resetCachedAssets {
    [[STIMImagePickerManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[STIMImagePickerManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[STIMImagePickerManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            STIMAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
...
@implementation QIMCollectionView
