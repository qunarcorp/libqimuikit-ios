...
@interface STIMPGroupSelectionCell() <QRadioButtonDelegate> {
...
@implementation STIMPGroupSelectionCell
...
- (void)refrash {
    ...
    NSDictionary *userInfo = [[STIMKit sharedInstance] getUserInfoByUserId:_jid];
    _descLabel.text = [userInfo valueForKey:@"DescInfo"];
    [_headerView stimDB_setImageWithJid:_jid];
}

- (void)didSelectedRadioButton:(QRadioButton *)radio groupId:(NSString *)groupId {
    STIMVerboseLog(@"did selected radio:%@ groupId:%@", radio.titleLabel.text, groupId);
...
- (void)initSubControls {
    ...
    [_lineView setBackgroundColor:[UIColor stimDB_colorWithHex:0xc7ced4 alpha:1.0]];
