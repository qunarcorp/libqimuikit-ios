...
@protocol STIMAttributedLabelDelegate <NSObject>
@optional
...
/**
 *  STIMAttributedLabel 属性文本 支持图文混排显示，支持添加image和UIView，支持自定义排版
 */
@interface STIMAttributedLabel : UIView

@property (nonatomic, assign) id<STIMAttributedLabelDelegate>     delegate;
 ...
 *
 *  @param textRunArray textRun数组（需遵循协议STIMTextStorageProtocol,否则不会添加）
 */
...
@interface STIMAttributedLabel (AppendAttributedString)
/**
 *  追加(添加到最后) 普通文本
 *
 *  @param text 普通文本
...
@interface STIMAttributedLabel (Link)
...
@interface STIMAttributedLabel (UIImage)
