...
#import "STIMLocalLogTableViewCell.h"
#import "STIMFileIconTools.h"
#import "STIMStringTransformTools.h"

@interface STIMLocalLogTableViewCell ()
{
    BOOL _selected;
}
...
@implementation STIMLocalLogTableViewCell
...
- (void)setLogFileDict:(NSDictionary *)logFileDict {
    if (logFileDict.count > 0) {
        ...
        //文件大小 MB
        NSString *fileSizeStr = [STIMStringTransformTools stimDB_CapacityTransformStrWithSize:theFileSize.longLongValue];
        //文件名
        NSString *fileName = [logFilePath lastPathComponent];        
        [self.nameLabel setText:fileName];
        [self.sizeLabel setText:fileSizeStr];
        [self.fileIcon setImage:[STIMFileIconTools getFileIconWithExtension:fileName.pathExtension]];
    }
}
