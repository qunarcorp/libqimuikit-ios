//
//  STIMQRCodeViewDisplayController.m
//  STChatIphone
//
...
- (NSString *)activityTitle {
    return [NSBundle stimDB_localizedStringForKey:@"common_send_friend"];
}

- (UIImage *)activityImage {
    return [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Action_Share"];
}
...
@interface STIMQRCodeViewDisplayController()<QCActivityToFriendDelegate,STIMContactSelectionViewControllerDelegate>{
    UIView *_backView;
    BOOL    _navBarHidden;
}
...
@implementation STIMQRCodeViewDisplayController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor stimDB_colorWithHex:0xf1f1f1 alpha:1];
    ...
    _backView.backgroundColor = [UIColor stimDB_colorWithHex:0xeaeaea alpha:1];
    [_backView.layer setBorderColor:[UIColor stimDB_colorWithHex:0xd1d1d1 alpha:1].CGColor];
    ...
    [nameLabel setTextColor:[UIColor stimDB_colorWithHex:0x333333 alpha:1]];
    ...
    [infoLabel setTextColor:[UIColor stimDB_colorWithHex:0x999999 alpha:1]];
    [infoView addSubview:infoLabel];
    
    switch (self.QRtype) {
        case QRCodeType_GroupQR:
        {
            [infoLabel setText:[NSBundle stimDB_localizedStringForKey:@"qrcode_tips_group"]];
            nameLabel.text = self.name;
//            icon.image = [[STIMKit sharedInstance] getGroupImageFromLocalByGroupId:self.jid];
            ...
            [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"group_qr_code"]];
        }
            break;
        case QRCodeType_UserQR:
        {
            [infoLabel setText:[NSBundle stimDB_localizedStringForKey:@"qrcode_tips_user"]];
            nameLabel.text = self.name;
            icon.image = [[STIMImageManager sharedInstance] getUserHeaderImageByUserId:[[STIMKit sharedInstance] getLastJid]];
            ...
            [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"myself_tab_qrcode"]];
        }
            break;
        case QRCodeType_RobotQR:
        {
            
            [infoLabel setText:[NSBundle stimDB_localizedStringForKey:@"qrcode_tips_public_number"]];
            nameLabel.text = self.name;
            NSDictionary *cardDic = [[STIMKit sharedInstance] getPublicNumberCardByJid:[NSString stringWithFormat:@"%@@%@",self.jid,[[STIMKit sharedInstance] getDomain]]];
            NSString *headerSrc = [cardDic objectForKey:@"HeaderSrc"];
            icon.image = [[STIMKit sharedInstance] getPublicNumberHeaderImageByFileName:headerSrc];
        ...
        case QRCodeType_ClientNav: {
            [infoLabel setText:[NSBundle stimDB_localizedStringForKey:@"qrcode_tips_client_nav"]];
            nameLabel.text = self.name;
            icon.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"setup_38x38_"];
