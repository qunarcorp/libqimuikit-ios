...
@interface QTImageAssetViewController ()<UITableViewDelegate,UITableViewDataSource,QTImageAssetCellDelegate,UIActionSheetDelegate,STIMImageEditViewControllerDelegate>{
...
- (void)initNavBar{
    ...
    if ([self.title.lowercaseString isEqualToString:@"camera roll"]) {
        self.title = [NSBundle stimDB_localizedStringForKey:@"Camera Roll"];
    }
...
- (void)initBottomView{
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT] - 49, self.view.width, 49)];
...
- (void)initTableView{
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, self.view.height - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT] - _bottomView.height) style:UITableViewStylePlain];
...
- (void)setupAssets
{
    ...
    ALAssetsGroupEnumerationResultsBlock resultsBlock = ^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        ...
        else if (self.assets.count > 0)
        {
            ...
            [label setTextColor:[UIColor stimDB_colorWithHex:0xa1a1a1 alpha:1]];
            [label setTextAlignment:NSTextAlignmentCenter];
            NSString *title = nil;
            if (_numberOfVideos == 0) {
                title = [NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"n Photos"], (long)_numberOfPhotos];
            }
            else if (_numberOfPhotos == 0) {
                title = [NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"n Videos"], (long)_numberOfVideos];
            }
            else {
                title =[NSString stringWithFormat:@"%@,%@",[NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"n Photos"],(long)_numberOfPhotos],[NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"n Videos"],(long)_numberOfVideos]];
            }
            [label setText:title];
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    ...
    if (picker.indexPathsForSelectedItems.count >= picker.maximumNumberOfSelection) {
        if (picker.delegate!=nil&&[picker.delegate respondsToSelector:@selector(qtImagePickerControllerDidMaximum:)]) {
            ...
            [QTalkTipsView showTips:[NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"Select a maximum of %d photos"],(int)picker.maximumNumberOfSelection] InView:self.view];
        }
    }
