...
@interface STIMFriendQNSettingVC ()<UITableViewDataSource,UITableViewDelegate>

@end

@implementation STIMFriendQNSettingVC{
...
- (void)onDoneClick{
    if (_questionTextField.text.length <= 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"请输入问题内容。" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
        [alertView show];
        return;
    }
