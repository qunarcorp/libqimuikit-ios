...
@implementation STIMPublicNumberCell{
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(_iconImageView.right+10, ([STIMPublicNumberCell getCellHeight]-20 - 18)/2.0, self.width - _iconImageView.right - 20, 20)];
        ...
        [_contentLabel setTextColor:[UIColor stimDB_colorWithHex:0x888888 alpha:1]];
        ...
        [_timeLabel setTextColor:[UIColor stimDB_colorWithHex:0xa1a1a1 alpha:1]];
        [_timeLabel setBackgroundColor:[UIColor clearColor]];
        [_timeLabel setTextAlignment:NSTextAlignmentRight];
        [self.contentView addSubview:_timeLabel];
        
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(_titleLabel.left, [STIMPublicNumberCell getCellHeight]-0.5, self.width, 0.5)];
...
- (void)refreshUI{
    ...
    [_iconImageView setImage:[[STIMKit sharedInstance] getPublicNumberHeaderImageByFileName:self.headerSrc]];
    NSString *text = nil;
    switch (self.msgType) {
        ...
        case STIMMessageType_Consult:{
            NSDictionary *msgDic = [[STIMJSONSerializer sharedInstance] deserializeObject:self.content error:nil];
        ...
        default:
        {
            if (self.content.length > 0) {
                text = [[STIMKit sharedInstance] getMsgShowTextForMessageType:(STIMMessageType)self.msgType];
                if (text.length <= 0) {
                    text = @"你收到了一条消息。";
                }
            }
