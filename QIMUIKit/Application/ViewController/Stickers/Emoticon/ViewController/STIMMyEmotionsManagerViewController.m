//
//  STIMMyEmotionsManagerViewController.m
//  qunarChatIphone
//
...
#import "STIMMyEmotionsManagerViewController.h"
#import "STIMEmotion.h"
#import "STIMMyEmotionsManagerCell.h"
#import "STIMEmotionSave.h"
#import "STIMEmotionManager.h"

//排序已在本Vc做好，移除需要发送remove通知，download同理
static NSString *cellID = @"STIMMyEmotionsManagerCell";

@interface STIMMyEmotionsManagerViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation STIMMyEmotionsManagerViewController
...
- (NSMutableArray *)dataList {
    ...
    if (!_dataList) {
        ...
        NSString *path = [[STIMEmotionSave sharedInstance] getEmotionInfoDataPath];
        NSArray *array = [NSArray arrayWithContentsOfFile:path];
        NSArray *EmotionPaclageIdList = [[STIMEmotionManager sharedInstance] getEmotionPackageIdList];
...
- (UILabel *)promptLabel {
    ...
    if (!_promptLabel) {
        ...
        _promptLabel.text = [NSBundle stimDB_localizedStringForKey:@"chat_Stickers"];
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        [_tableView registerClass:[STIMMyEmotionsManagerCell class] forCellReuseIdentifier:cellID];
...
- (UIButton *)editBtn {
    ...
    if (!_editBtn) {
        ...
        [_editBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Reorder"] forState:UIControlStateNormal];
...
- (void)editHandel:(id)sender {
    ...
    NSString *title = btn.selected ? [NSBundle stimDB_localizedStringForKey:@"Done"] : [NSBundle stimDB_localizedStringForKey:@"Reorder"];
    NSString *promptTitle = btn.selected ? [NSBundle stimDB_localizedStringForKey:@"Adjust_Chat_stickers"] : [NSBundle stimDB_localizedStringForKey:@"chat_Stickers"];
...
- (void)initUI {
    ...
    self.title = [NSBundle stimDB_localizedStringForKey:@"My_Stickers"];
    [self setUpNav];
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath {
    ...
    [[STIMEmotionManager sharedInstance] updateEmotions:newCollectionFaceList];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMMyEmotionsManagerCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];

    STIMEmotion *emotion = [[STIMEmotion alloc] initWithDict:self.dataList[indexPath.row]];
