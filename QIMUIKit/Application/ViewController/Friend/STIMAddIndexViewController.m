...
#import "STIMPickerView.h"
#import "STIMJSONSerializer.h"
#import "STIMChatVC.h"
#import "STIMAddSomeCell.h"
...
@interface STIMAddIndexViewController ()<UITableViewDataSource,UITableViewDelegate,SearchBarDelgt>{
...
@implementation STIMAddIndexViewController{
...
- (void)loadEmptyNotReadMsgList {
    ...
    emptyIconView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"EmptyNotReadList"];
...
- (void)getDomainList {
    ...
    [[STIMHttpRequestMonitor sharedInstance] syncRunBlock:^{
...
- (void)selectDomain {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [STIMPickerView dismissWithCompletion:^(NSString *str) {
            
        }];
        
    } completion:^(BOOL finished) {
        __weak typeof(self) weakSelf = self;
        NSDictionary *dict = @{STIMMMbackgroundColor: [UIColor whiteColor],
                               STIMMMtextColor: [UIColor blackColor],
                               STIMMMtoolbarColor: [UIColor whiteColor],
                               STIMMMbuttonColor: [UIColor blueColor],
                               STIMMMfont: [UIFont systemFontOfSize:18],
                               STIMMMvalueY: @3,
                               STIMMMselectedObject:_selectedString};
        
        [STIMPickerView showPickerViewInView:self.view withStrings:_stringsArray withOptions:dict completion:^(NSString *selectedString) {
            ...
            STIMVerboseLog(@"selectDomainUrl === %@", weakSelf.domainURL);
            STIMVerboseLog(@"selectDomainUrlRow === %@", selectedString);
        }];
    }];
}
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    [self.searchBar resignFirstResponder];
    [STIMPickerView dismissWithCompletion:^(NSString *dismissRow) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_searchResults.count > 0) {
        return [STIMAddSomeCell getCellHeight];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_searchResults.count > 0) {
        ...
        STIMAddSomeCell *someCell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (someCell == nil) {
            someCell = [[STIMAddSomeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
        }
...
- (void)searchList {
    ...
    NSArray *searchList = [[STIMKit sharedInstance] searchUserListBySearchStr:searchString Url:self.domainURL id:self.domainId limit:limitCount offset:0];
...
- (void)onReceiveFriendPresence:(NSNotification *)notify{
    if ([_infoDic[@"XmppId"] isEqualToString:notify.object]) {
        ...
        if ([result isEqualToString:@"success"]) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"添加好友失败！" delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
            [alertView show];
        }
    }
}
...
{
    [[STIMKit sharedInstance] openChatSessionByUserId:_infoDic[@"XmppId"]];
    [STIMFastEntrance openSingleChatVCByUserId:_infoDic[@"XmppId"]];
    /*
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (count>0) {
        ...
        if (distanceFromBottom < heigth) {
            ...
            STIMVerboseLog(@"end of tableView");
            limitCount += 12;
            [self searchList];
        }
