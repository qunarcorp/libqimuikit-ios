...
#define key_STIMSelectItem_Title @"key_STIMSelectItem_Title"
#define key_STIMSelectItem_TitleColor @"key_STIMSelectItem_TitleColor"

#define kColorWithHex(c) [UIColor colorWithRed:((c>>16)&0xFF)/255.0f green:((c>>8)&0xFF)/255.0f blue:(c&0xFF)/255.0f alpha:1.0f]

#define Screen_STIM_Width [UIScreen mainScreen].bounds.size.width
#define Screen_STIM_Height [UIScreen mainScreen].bounds.size.height
...
@interface STIMUpdateAlertView ()

@property(nonatomic, strong) STIMUpdateDataModel *updateModel;

//@property(nonatomic, assign) STIMUpgradeType upgradeType;    //更新状态
//
...
@implementation STIMUpdateAlertView

- (UIImageView *)iconView {
    if (!_iconView) {
        _iconView = [[UIImageView alloc] initWithFrame:CGRectZero];
        _iconView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"update_version"];
    }
    return _iconView;
}
...
- (UILabel *)messageLabel {
    if (!_messageLabel) {
        ...
        _messageLabel.textColor = [UIColor stimDB_colorWithHex:0x888888];
...
- (UIButton *)nextButton {
    if (!_nextButton) {
        ...
        _nextButton.layer.borderColor = [UIColor stimDB_colorWithHex:0xBFBFBF].CGColor;
        _nextButton.layer.masksToBounds = YES;
        [_nextButton setTitle:@"下次再说" forState:UIControlStateNormal];
        [_nextButton setTitleColor:[UIColor stimDB_colorWithHex:0x999999] forState:UIControlStateNormal];
...
- (UIButton *)updateButton {
    if (!_updateButton) {
        ...
        [_updateButton setTitleColor:[UIColor stimDB_colorWithHex:0xFFFFFF] forState:UIControlStateNormal];
        [_updateButton setBackgroundColor:[UIColor stimDB_colorWithHex:0x00CABE]];
...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc] initWithFrame:CGRectMake(kAlertView_offsetX, 0, Screen_STIM_Width - kAlertView_offsetX * 2, kAlertView_MAX_Height / 2)];
...
+ (instancetype)stimDB_showUpdateAlertViewWithUpdateDic:(NSDictionary *)updateDic {
    STIMUpdateAlertView *updateView = [[STIMUpdateAlertView alloc] initWithFrame:[[UIApplication sharedApplication] keyWindow].bounds];
...
- (void)showUpdateAlertViewWithUpdateDic:(NSDictionary *)updateDic {
    ...
    STIMUpdateDataModel *updateModel = [STIMUpdateDataModel yy_modelWithDictionary:updateDic];
...
- (void)layoutHCAlertView {
    ...
    CGSize rectTitleSize = [self.title stimDB_sizeWithFontCompatible:self.titleLabel.font forWidth:normalWidth lineBreakMode:NSLineBreakByWordWrapping];
    ...
    CGSize rectMessageSize = [self.updateModel.message stimDB_sizeWithFontCompatible:self.messageLabel.font constrainedToSize:CGSizeMake(normalMessageWidth, MAXFLOAT) lineBreakMode:NSLineBreakByTruncatingTail];
    ...
    [self.alertView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(Screen_STIM_Width - kAlertView_offsetX * 2);
        make.bottom.mas_equalTo(self.updateButton.mas_bottom).mas_offset(30);
        make.left.mas_equalTo(kAlertView_offsetX);
        make.top.mas_equalTo(Screen_STIM_Width - kAlertView_offsetX * 2);
    }];
...
- (void)nextUpdate:(id)sender {
    //下次再说
    if (self.upgradeDelegate && [self.upgradeDelegate respondsToSelector:@selector(stimDB_UpdateAlertViewDidClickWithType:withUpdateDic:)]) {
        [self.upgradeDelegate stimDB_UpdateAlertViewDidClickWithType:STIMUpgradeAfter withUpdateDic:self.updateModel];
    }
    [self dismiss];
}
...
- (void)gotoUpdate:(id)sender {
    //现在去更新
    if (self.upgradeDelegate && [self.upgradeDelegate respondsToSelector:@selector(stimDB_UpdateAlertViewDidClickWithType:withUpdateDic:)]) {
        [self.upgradeDelegate stimDB_UpdateAlertViewDidClickWithType:STIMUpgradeNow withUpdateDic:self.updateModel];
    }
    [self dismiss];
}
