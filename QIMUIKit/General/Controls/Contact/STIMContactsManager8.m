#import "STIMContactsManager8.h"
#import "STIMContactObject.h"
#import "STIMContactObjectManager.h"
@import AddressBook;

typedef void(^ContactDidObatinBlock)(NSArray <STIMContactObject *> *);

@interface STIMContactsManager8 ()
...
@implementation STIMContactsManager8

-(instancetype)init {
    if (self = [super init]) {
        self.addressBook = ABAddressBookCreate();
        
//        STIMVerboseLog(@"%@",@([(id)self.addressBook retainCount]));
...
void addressBookChangeCallBack(ABAddressBookRef addressBook, CFDictionaryRef info, void *context) {
    ...
    STIMVerboseLog(@"通讯录发生变化啦");
    
    //初始化对象
    STIMContactsManager8 * contactManager = CFBridgingRelease(context);
...
+(instancetype)sharedInstance {
    static STIMContactsManager8 * addressBookManager = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        addressBookManager = [[STIMContactsManager8 alloc]init];
    });
...
#pragma mark - 请求通讯录
//请求通讯录
-(void)requestContactsComplete:(void (^)(NSArray<STIMContactObject *> * _Nonnull))completeBlock {
...
//    CFArrayRef contants = ABAddressBookCopyArrayOfAllPeopleInSourceWithSortOrdering(addressBook, recordRef, kABPersonSortByFirstName);
    ...
    //存放所有联系人的数组
    NSMutableArray <STIMContactObject *> * contacts = [NSMutableArray arrayWithCapacity:0];
    ...
    for (NSInteger i = 0; i < CFArrayGetCount(contants); i++) {
        ...
        //获得contact对象
        STIMContactObject * contactObject = [STIMContactObjectManager contantObject:recordRef];
...
@implementation STIMContactsManager8 (QTCodingHandle)
...
#pragma mark - 添加联系人头像属性
    /*添加联系人头像属性*/
    ABPersonSetImageData(person, (__bridge CFDataRef)(UIImagePNGRepresentation([UIImage stimDB_imageNamedFromSTIMUIKitBundle:@""])),&error);//设置联系人头像
