...
#import "STIMDefalutMessageCell.h"
...
@interface STIMDefalutMessageCell () <STIMMenuImageViewDelegate>

@property (nonatomic, strong) STIMAttributedLabel *messageLabel;

@property (nonatomic, strong) STIMTextContainer *textContainer;

@end

@implementation STIMDefalutMessageCell

- (STIMAttributedLabel *)messageLabel {
    if (!_messageLabel) {
        _messageLabel = [[STIMAttributedLabel alloc] init];
...
- (void)layoutSubviews {
    ...
    [self.messageLabel setFrameWithOrign:CGPointMake(kTextLableLeft + (self.message.messageDirection == STIMMessageDirection_Sent ? 0 : 10),10) Width:_textContainer.textWidth];
}
