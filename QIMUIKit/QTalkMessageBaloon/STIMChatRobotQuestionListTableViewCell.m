//
//  STIMChatRobotQuestionListTableViewCell.m
//  STIMUIKit
//
...
@interface STIMChatRobotQuestionListTableViewCell()<STIMMenuImageViewDelegate,UIActionSheetDelegate,STIMAttributedLabelDelegate>
{
    UIView               * _bgView;
...
@property (nonatomic, strong) STIMTextContainer *hintContainer;
...
@implementation STIMChatRobotQuestionListTableViewCell
@dynamic delegate;

+ (CGFloat)getCellHeightWithMessage:(STIMMessageModel *)msg chatType:(ChatType)chatType {
    return [self consultRbtCellHeightForMsg:msg];
}

+ (CGFloat)consultRbtCellHeightForMsg:(STIMMessageModel *)msg {
    ...
    NSDictionary *infoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:jsonStr error:nil];
    ...
    if (infoDic.count) {
        ...
        if (content.length) {
            STIMTextContainer *textContainer = [STIMMessageParser textContainerForMessageCtnt:content withId:msg.messageId direction:msg.messageDirection];
        ...
        if (listTips.length > 0) {
            cellHeight += 20;
            [listTips stimDB_sizeWithFontCompatible:kIMChatContentFont constrainedToSize:CGSizeMake(maxWidth- kIMChatContentLeft - kIMChatContentRight, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping].height + 5;
            cellHeight += 5;
        }
        ...
        if ([type.lowercaseString isEqualToString:@"list"]) {
            ...
            NSDictionary * isUnfoldDic = [[STIMMessageCellCache sharedInstance] getObjectForKey:kMessageIsUnfold];
            BOOL isUnfold = NO;
            if (isUnfoldDic) {
            ...
            if (items && items.count > 0) {
                ...
                //                        cellHeight += [[item objectForKey:@"text"] stimDB_sizeWithFontCompatible:kIMChatContentFont constrainedToSize:CGSizeMake(maxWidth- kIMChatContentLeft - kIMChatContentRight, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping].height + 5;
                //                        if (index > initSize) {
                //                            break;
                //                        }
                //                        index ++;
        ...
        if (hints && [hints isKindOfClass:[NSArray class]] && hints.count) {
            ...
            [[STIMMessageCellCache sharedInstance] setObject:container forKey:[msg.messageId stringByAppendingString:@"_hints"]];
        }
    }
...
//        _textLabel.characterSpacing
        ...
        _hintLabel.backgroundColor = [UIColor stimDB_colorWithHex:0xc1c1c1];
        [_bgView addSubview:_hintLabel];
        
        _resolveBtn = [[UIButton alloc]init];
        [_resolveBtn setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"robotResovlebtnUnSelected"] forState:UIControlStateNormal];
        [_resolveBtn setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"robotResovlebtnSelected"] forState:UIControlStateSelected];
        ...
        [_unResolveBtn setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"robotunResovlebtnUnSelected"] forState:UIControlStateNormal];
        [_unResolveBtn setBackgroundImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"robotunResovelebtnSelected"] forState:UIControlStateSelected];
        ...
        _bottom_tipsLabel.textColor = [UIColor stimDB_colorWithHex:0x999999];
        ...
        _separateLine.backgroundColor = [UIColor stimDB_colorWithHex:0xEAEAEA];
        
        [_bgView addSubview:_separateLine];
        
        _resolveLabel = [[UILabel alloc]init];
        _resolveLabel.textAlignment = NSTextAlignmentLeft;
        _resolveLabel.textColor = [UIColor stimDB_colorWithHex:0x999999];
        ...
        _unResolveLabel.textColor = [UIColor stimDB_colorWithHex:0x999999];
...
- (void)setMessage:(STIMMessageModel *)message {
    ...
    //    _hintContainer = [[STIMMessageCellCache sharedInstance] getObjectForKey:[message.messageId stringByAppendingString:@"_hints"]];
    ...
    _textContainer = [STIMMessageParser textContainerForMessageCtnt:infoDic[@"content"] withId:self.message.messageId direction:self.message.messageDirection];
    _textContainer.textAlignment = kCTTextAlignmentLeft;
}

//- (STIMTextContainer *)getTextContainer{
//
...
//    [_textLabel clearOwnerView];
    if (_textContainer) {
        ...
        [_textLabel setFrameWithOrign:CGPointMake((STIMMessageDirection_Received == self.message.messageDirection) ? 14 :14,15) Width:kHintCelMaxWidth-14-14];
    } else {
    ...
    if (listTips.length > 0) {
        ...
        float actionBtnHeight = [listTips stimDB_sizeWithFontCompatible:kIMChatContentFont constrainedToSize:CGSizeMake(maxWidth - kIMChatContentLeft - kIMChatContentRight, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping].height + 5;
        ...
        [listTipsLabel setTextColor:[UIColor stimDB_colorWithHex:0x999999]];
    ...
    if ([type.lowercaseString isEqualToString:@"list"]) {
        ...
        if (items && items.count > 0) {
            ...
            //            for (NSDictionary * item in items) {
            //                if ([item objectForKey:@"text"] ) {
            //                    float actionBtnHeight = [[item objectForKey:@"text"] stimDB_sizeWithFontCompatible:kIMChatContentFont constrainedToSize:CGSizeMake(maxWidth, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping].height + 5;
            //
            ...
            //                    [itemAction setTextColor:[UIColor stimDB_colorWithHex:0x5CC57F]];
            ...
            //                    view.backgroundColor = [UIColor stimDB_colorWithHex:0x00CABE];
            ...
            ////                        line.backgroundColor = [UIColor stimDB_colorWithHex:0xe7e7e7];
            ...
            for (int i = 0; i<items.count; i++) {
                ...
                if ([item objectForKey:@"text"] ) {
                    ...
                    itemAction.numberOfLines = 0;
                    [itemAction setText:[item objectForKey:@"text"]];
                    [itemAction setTextColor:[UIColor stimDB_colorWithHex:0x333333]];
                    [itemAction setFont:[UIFont systemFontOfSize:15]];
            ...
            if (items.count > initSize) {
                ...
                [itemAction setTextColor:[UIColor stimDB_colorWithHex:0x00CABE]];
                ...
                [itemIcon setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f3c7" size:20 color:[UIColor colorWithRGBHex:0x00CABE]]]];
...
- (void)requestHttpWithRequestUrl:(NSString *)urlStr {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [properties setValue:[[STIMKit sharedInstance] thirdpartKeywithValue]  forKey:NSHTTPCookieValue];
    ...
    if (index >= 0 && index < maxIndex) {
        ...
        if ([[clickType lowercaseString] isEqualToString:@"interface"]) {
            if (url.length > 0) {
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(sendSTIMChatRobotQusetionListTextMessageForText:isSendToServer:userType:)]) {
                    if (afterClickSendMsg.length) {
//                        [self.delegate sendTextMessageForText:afterClickSendMsg isSendToServer:YES userType:@"cRbt"];
                        [self.delegate sendSTIMChatRobotQusetionListTextMessageForText:afterClickSendMsg isSendToServer:YES userType:@"cRbt"];
                    } else{
                       [self.delegate sendSTIMChatRobotQusetionListTextMessageForText:afterClickSendMsg isSendToServer:NO userType:@"cRbt"];
                    }
                }
            }
        ...
        STIMCustomPopViewController *popVc = [[STIMCustomPopViewController alloc] init];
        popVc.popHeaderTitle = listTip;
        popVc.items = items;
        [STIMCustomPopManager showPopVC:popVc withRootVC:[[UIApplication sharedApplication] visibleViewController]];
    }
    /*
     ...
     [[STIMMessageCellCache sharedInstance] setObject:newIsUnfoldDic forKey:kMessageIsUnfold];
...
- (void)resolveBtnClicked:(UIButton *)btn{
    ...
    [[STIMKit sharedInstance] sendTPPOSTRequestWithUrl:_yesBtnUrl withSuccessCallBack:^(NSData *responseData) {
        NSDictionary * dictFromData = [[STIMJSONSerializer sharedInstance] deserializeObject:responseData error:nil];
        ...
        if ([dictFromData objectForKey:@"data"]) {
            ...
            if ([dic objectForKey:@"is_worked"] && weakSelf.message.messageId) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (weakSelf.delegate && [weakSelf respondsToSelector:@selector(refreshSTIMChatRobotListQuestionMessageCell:)]) {
                        [weakSelf.delegate refreshSTIMChatRobotListQuestionMessageCell:self];
                    }
                });
            }
            else{
...
- (void)unResolveBtnClicked:(UIButton *)btn{
    ...
    __weak typeof(self) weakSelf = self;
    [[STIMKit sharedInstance] sendTPPOSTRequestWithUrl:_noBtnUrl withSuccessCallBack:^(NSData *responseData) {
