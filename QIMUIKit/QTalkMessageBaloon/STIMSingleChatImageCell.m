...
#import "STIMSingleChatImageCell.h"
...
@interface STIMSingleChatImageCell()<STIMMenuImageViewDelegate,UIGestureRecognizerDelegate>

@end

@implementation STIMSingleChatImageCell{
...
//        NSDictionary *infoDic = [self.message getMsgInfoDic];
        ...
        if (infoDic) {
            ...
            if (image == nil && httpUrl.length > 0){
                ...
                if (_msgImageView.image == nil) {
                    image = [[STIMSingleChatImageTools sharedInstance] getImageDownloadFaildWithDirect:self.message.messageDirection];
                } else {
            ...
            if ([self.message.message hasPrefix:@"http://"]) {
                ...
                if (_msgImageView.image == nil) {
                    image = [[STIMSingleChatImageTools sharedInstance] getImageDownloading];
                } else {
                    image = _msgImageView.image;
                }
            } else {
        ...
        if (self.message.messageDirection == STIMMessageDirection_Received) {
            ...
            [_backView setImage:[[STIMSingleChatImageTools sharedInstance] getReceivedBg]];
            ...
            [_backView setFrame:frame];
            [_backView setImage:[[STIMSingleChatImageTools sharedInstance] getSentBg]];
