//
//  STIMLocationShareMsgCell.m
//  qunarChatIphone
//
//  Created by xueping on 15/7/9.
//
//
#define kSTIMLocationShareMsgCellHeight    150
...
#import "STIMLocationShareMsgCell.h"
#import "UserLocationViewController.h"
//#import "UIImageView+STIMWebCache.h"
#import "ShapedImageView.h"

@interface STIMLocationShareMsgCell()<STIMMenuImageViewDelegate>
{
...
@implementation STIMLocationShareMsgCell

+ (CGFloat)getCellHeightWithMessage:(STIMMessageModel *)message chatType:(ChatType)chatType
{
    return kSTIMLocationShareMsgCellHeight + 20 + (chatType == ChatType_GroupChat ? 20 : 0);
}
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _imageView = [[ShapedImageView alloc] initWithImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"map_located"]];
...
- (void)refreshUI
{
    ...
    float backWidth = 215;
    float backHeight = kSTIMLocationShareMsgCellHeight;
...
//            [self.backView setFrame:frame];
    ...
    if (localScreenImageExist == YES) {
        ...
        if (![imageUrlStr stimDB_hasPrefixHttpHeader] && imageUrlStr.length > 0) {
            imageUrlStr = [NSString stringWithFormat:@"%@/%@", [[STIMKit sharedInstance] qimNav_InnerFileHttpHost], imageUrlStr];
        }
        [_imageView stimDB_setImageWithURL:[NSURL URLWithString:imageUrlStr] placeholderImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"map_located"] options:SDWebImageDecodeFirstFrameOnly progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            [[STIMKit sharedInstance] saveFileData:UIImageJPEGRepresentation(image, 1.0) withFileName:nil forCacheType:STIMFileCacheTypeColoction];
        }];
    }else{
        _imageView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"map_located"];
    }
}
