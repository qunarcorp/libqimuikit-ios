...
@interface STIMGroupCreateVC ()<UITableViewDataSource,UITableViewDelegate,SelectionResultDelegate,UITextFieldDelegate>{
...
@implementation STIMGroupCreateVC


- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    _groupId = [STIMUUIDTools UUID];
    _memberArrays = [NSMutableArray arrayWithCapacity:1];
    if ([self userId]) { 
        [_memberArrays addObject:[self userId]];
    } else { 
        [_memberArrays addObject:[[STIMKit sharedInstance] getLastJid]];
    }
            ...
            if (memberList.count > 0) {
                ...
                for (int i = 0; i < memberList.count; i++) {
                    ...
                    if ([memDic objectForKey:@"type"] && [[memDic objectForKey:@"type"] isEqualToString:@"add"]) {
                        ...
                        UIImage *headerImage = [[STIMImageManager sharedInstance] getUserHeaderImageByUserId:newMemberJid];
...
- (void)onDoneButtonClick:(UIButton *)sender{
    ...
    [[STIMKit sharedInstance] createGroupByGroupName:_groupId
        ...
        if (finish) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"创建失败" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
            [alertView show];
        }
    }];
}
