...
#define kCommonTrdInfoCellWidth   [UIScreen mainScreen].stimDB_rightWidth  * 3.2 / 5

@implementation STIMCommonTrdInfoCell{
    UILabel         * _titleLabel;
...
+ (CGFloat)getCellHeightWithMessage:(STIMMessageModel *)message chatType:(ChatType)chatType{
    ...
    if (message.messageType == STIMMessageType_CommonTrdInfoPer || showas667) {
        NSString *desc = [infoDic objectForKey:@"desc"];
        CGSize descSize = [desc stimDB_sizeWithFontCompatible:[UIFont systemFontOfSize:12] constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width * 0.60 - 20, MAXFLOAT)];
        return 110 + MAX(descSize.height, 20);
    } else {
        
        NSString *title = [infoDic objectForKey:@"title"];
        CGSize titleSize = [title stimDB_sizeWithFontCompatible:[UIFont systemFontOfSize:16] constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width * 0.60 - 20, 40)];
...
-(void)refreshUI {
    ...
    NSDictionary * infoDic = [[STIMJSONSerializer sharedInstance] deserializeObject:infoStr error:nil];
    CGFloat cellHeight = [STIMCommonTrdInfoCell getCellHeightWithMessage:self.message chatType:self.chatType];
    ...
    CGSize titleSize = [_titleLabel.text stimDB_sizeWithFontCompatible:_titleLabel.font constrainedToSize:CGSizeMake(self.backView.width - titleLeft - 20, 40)];
