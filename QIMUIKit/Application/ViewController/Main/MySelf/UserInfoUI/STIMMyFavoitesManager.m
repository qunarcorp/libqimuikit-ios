...
@interface STIMMyFavoitesManager ()

@end

static NSMutableArray *_myFavoritesArray = nil;
@implementation STIMMyFavoitesManager

+ (instancetype)sharedMyFavoritesManager {
    
    static STIMMyFavoitesManager *__myFavoriteManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __myFavoriteManager = [[STIMMyFavoitesManager alloc] init];
    });
...
- (void)setMyFavoritesArrayWithMsg:(STIMMessageModel *)message {
    ...
    message.messageDirection = STIMMessageDirection_Received;
