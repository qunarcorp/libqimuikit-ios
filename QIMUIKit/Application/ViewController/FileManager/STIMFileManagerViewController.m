//
//  STIMFileManagerViewController.m
//  STChatIphone
//
...
@interface STIMFileManagerViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation STIMFileManagerViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    if ([[STIMKit sharedInstance] getIsIpad]) {
        self.view.width = [[STIMWindowManager shareInstance] getDetailWidth];
    }

    self.navigationItem.title = [NSBundle stimDB_localizedStringForKey:@"explore_tab_my_file"];
    
    self.view.backgroundColor = [UIColor whiteColor];
    
    _selectArr = [NSMutableArray arrayWithCapacity:1];
    
    NSArray * fileMsgs = [[STIMKit sharedInstance] getMsgsForMsgType:STIMMessageType_File];
    STIMVerboseLog(@"file msgs = %@",fileMsgs);
//    _files = [NSMutableArray arrayWithArray:fileMsgs];
    ...
    if (self.isSelect) {
        ...
        UIBarButtonItem * item = [[UIBarButtonItem alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_cancel"] style:UIBarButtonItemStylePlain target:self action:@selector(cancelBtnHandle:)];
...
- (void)parseFiles:(NSArray *)files
{
    ...
    for (STIMMessageModel * msg in files) {
        long long msgDate = msg.messageDate; 
        NSString * key = [[[[NSDate stimDB_dateWithTimeIntervalInMilliSecondSince1970:msgDate] stimDB_formattedDateDescription] componentsSeparatedByString:@" "] firstObject];
...
- (void)showBottomViewEnabled : (BOOL) enabled
{
    if (!_bottomView) {
        ...
        [sendBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"common_send"] forState:UIControlStateNormal];
...
- (void)sendBtnHandle:(UIButton *)sender
{
    ...
    //获取加密状态
    STIMEncryptChatState encryptState = [[STIMEncryptChat sharedInstance] getEncryptChatStateWithUserId:self.userId];
#endif
    for (STIMMessageModel * message in _selectArr) {
       STIMMessageModel *msg = message;
#if __has_include("STIMNoteManager.h")
        if (encryptState == STIMEncryptChatStateEncrypting) {
            NSString *encryptContent = [[STIMEncryptChat sharedInstance] encryptMessageWithMsgType:[msg messageType] WithOriginBody:msg.message WithOriginExtendInfo:msg.extendInformation WithUserId:self.userId];
            msg.message = @"加密文件消息iOS";
            msg.extendInformation = encryptContent;
            msg.messageType = STIMMessageType_Encrypt;
        }
#endif
        if (self.messageSaveType == ChatType_GroupChat) {
         msg = [[STIMKit sharedInstance] sendMessage:[msg message] WithInfo:[msg extendInformation] ToGroupId:self.userId WithMsgType:[msg messageType]];
        }else{
         msg = [[STIMKit sharedInstance] sendMessage:[msg message] WithInfo:[msg extendInformation] ToUserId:self.userId WithMsgType:[msg messageType]];
...
{
   STIMMessageModel * message = [[_filesDic objectForKey:[_fileKeys objectAtIndex:indexPath.section]] objectAtIndex:indexPath.row];
    STIMFileManagerCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (!cell) {
        cell = [[STIMFileManagerCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"cell"];
    }
...
{
    if (self.isSelect) {
        STIMFileManagerCell * cell = (STIMFileManagerCell *)[tableView cellForRowAtIndexPath:indexPath];
        ...
        if ([cell isCellSelected]) {
           ...
           STIMMessageModel * message = [[_filesDic objectForKey:[_fileKeys objectAtIndex:indexPath.section]] objectAtIndex:indexPath.row];
            for (STIMMessageModel * msg in _selectArr) {
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{//请求数据源提交的插入或删除指定行接收者。
    if (editingStyle ==UITableViewCellEditingStyleDelete) {//如果编辑样式为删除样式
       STIMMessageModel * message = [[_filesDic objectForKey:[_fileKeys objectAtIndex:indexPath.section]] objectAtIndex:indexPath.row];
        ...
        NSString *filePath = [[[STIMKit sharedInstance] getDownloadFilePath] stringByAppendingPathComponent:fileName];
        [[NSFileManager defaultManager] removeItemAtPath:filePath error:nil];
        [[STIMKit sharedInstance] deleteMsg:message ByJid:nil];
