...
+ (STIMMWPhoto *)photoWithImage:(STIMImage *)image {
	return [[STIMMWPhoto alloc] initWithImage:image];
}

+ (STIMMWPhoto *)photoWithURL:(NSURL *)url {
    return [[STIMMWPhoto alloc] initWithURL:url];
}

+ (STIMMWPhoto *)photoWithAsset:(PHAsset *)asset targetSize:(CGSize)targetSize {
    return [[STIMMWPhoto alloc] initWithAsset:asset targetSize:targetSize];
}

+ (STIMMWPhoto *)videoWithURL:(NSURL *)url {
    return [[STIMMWPhoto alloc] initWithVideoURL:url];
}
...
#pragma mark - STIMMWPhoto Protocol Methods

- (STIMImage *)underlyingImage {
        ...
        _webImageOperation = [manager loadImageWithURL:url options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
            if (expectedSize > 0) {
                ...
                NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:
                                      ...
                                      self, @"photo", nil];
                [[NSNotificationCenter defaultCenter] postNotificationName:STIMMWPHOTO_PROGRESS_NOTIFICATION object:dict];
            }
        } completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
            if (error) {
                STIMMWLog(@"SDWebImage failed to download image: %@", error);
            }
            _webImageOperation = nil;
            self.underlyingImage = (STIMImage *)image;
//            [[STIMImage alloc] initWithData:data];
            dispatch_async(dispatch_get_main_queue(), ^{
                [self imageLoadingComplete];
            });
        }];
    } @catch (NSException *e) {
        STIMMWLog(@"Photo from web: %@", e);
        _webImageOperation = nil;
        [self imageLoadingComplete];
    }
}
...
- (void)_performLoadUnderlyingImageAndNotifyWithLocalFileURL:(NSURL *)url {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @autoreleasepool {
            @try {
                self.underlyingImage = [STIMImage imageWithContentsOfFile:url.path];
                if (!_underlyingImage) {
                    STIMMWLog(@"Error loading photo from path: %@", url.path);
                }
            } @finally {
            ...
            @try {
                ...
                [assetslibrary assetForURL:url
                               resultBlock:^(ALAsset *asset){
                                   ...
                                   if (iref) {
                                       self.underlyingImage = [STIMImage imageWithCGImage:iref];
                                   }
                                   [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                               }
                              failureBlock:^(NSError *error) {
                                  self.underlyingImage = nil;
                                  STIMMWLog(@"Photo from asset library error: %@",error);
                                  [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                              }];
            } @catch (NSException *e) {
                STIMMWLog(@"Photo from asset library error: %@", e);
                [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
            }
        }
    });
...
- (void)postCompleteNotification {
    [[NSNotificationCenter defaultCenter] postNotificationName:STIMMWPHOTO_LOADING_DID_END_NOTIFICATION
                                                        object:self];
}
