...
@interface STIMMessageBrowserVC ()<STIMRemoteAudioPlayerDelegate,STIMSingleChatVoiceCellDelegate,STIMMsgBaloonBaseCellDelegate,STIMMWPhotoBrowserDelegate>{
    
    UIScrollView *_scrollView;
    
    STIMAttributedLabel *_msgLabel;
    
    STIMSingleChatVoiceCell * _voiceCell;
    ...
    STIMMWPhotoBrowser *_browser;
}
@property (nonatomic, assign) BOOL isVoice;
@end

@implementation STIMMessageBrowserVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    [self.view setBackgroundColor:self.message.messageDirection == STIMMessageDirection_Sent ? [UIColor stimDB_rightBallocColor] : [UIColor stimDB_leftBallocColor]];
    ...
    switch (self.message.messageType) {
        ...
        {
            //初始化图片浏览控件
            _browser = [[STIMMWPhotoBrowser alloc] initWithDelegate:self];
...
- (id <STIMMWPhoto>)photoBrowser:(STIMMWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index
{
    ...
    if (![urlStr stimDB_hasPrefixHttpHeader]) {
        urlStr = [[STIMKit sharedInstance].qimNav_InnerFileHttpHost stringByAppendingPathComponent:urlStr];
    }
...
- (void)timerHandle:(NSTimer *)sender
{
    ...
    int maxTime = 5;
    if (self.message.messageType == STIMMessageType_Image) {
        maxTime = 30;
    }
...
- (void)setUpVoiceCell
{
//    _voiceCell = [[STIMSingleChatVoiceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
...
//    _voiceCell.frame = CGRectMake(0, (self.view.height - [STIMSingleChatVoiceCell getCellHeightWithMessage:self.message chatType:self.message.messageSaveType]) / 2, self.view.width, 100);
//    [self.view addSubview:_voiceCell];
    ...
    _voiceFireBGView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"VoiceFire"];
    ...
    _voiceView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"iconfont-icon_voice"];
//    [self.view insertSubview:_voiceView aboveSubview:_voiceFireBGView];
    
    NSArray * receiveImageArray = [[NSArray alloc] initWithObjects:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Chat_VoiceBubble_Friend_Icon1"],[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Chat_VoiceBubble_Friend_Icon2"],[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Chat_VoiceBubble_Friend_Icon3"],[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Chat_VoiceBubble_Friend_Icon4"], nil];
    _voicePlayView = [[UIImageView alloc] initWithFrame:CGRectMake(20, _voiceFireBGView.bottom + 100, 20, 20)];
    [_voicePlayView setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Chat_VoiceBubble_Friend_Icon1"]];
