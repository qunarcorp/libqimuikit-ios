...
#import "STIMQuickReplySubTableView.h"
#import "STIMQuickReplyGroupCollectionView.h"
#import "SwipeTableView.h"
#import <objc/message.h>

@interface STIMQuickReplyExpandView () <SwipeTableViewDataSource,SwipeTableViewDelegate, STIMQuickReplyGroupCollectionViewDelegate>

@property (nonatomic, strong) STIMQuickReplySubTableView *subQuickReplyView;
@property (nonatomic, strong) STIMQuickReplyGroupCollectionView *groupCollectionView;
...
@implementation STIMQuickReplyExpandView
...
- (STIMQuickReplySubTableView *)subQuickReplyView {
    if (!_subQuickReplyView) {
        _subQuickReplyView = [[STIMQuickReplySubTableView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame) - 50)];
        _subQuickReplyView.backgroundColor = [UIColor qtalkChatBgColor];
    }
    return _subQuickReplyView;
}

- (STIMQuickReplyGroupCollectionView *)groupCollectionView {
    if (!_groupCollectionView) {
        _groupCollectionView = [[STIMQuickReplyGroupCollectionView alloc] initWithFrame:CGRectMake(0, self.swipeTableView.bottom + 15, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame) - self.swipeTableView.bottom - 15)];
...
- (NSMutableArray *)quickReplyGroups {
    if (!_quickReplyGroups) {
        _quickReplyGroups = [NSMutableArray arrayWithArray:[[STIMKit sharedInstance] getQuickReplyGroup]];
    }
...
- (UIScrollView *)swipeTableView:(SwipeTableView *)swipeView viewForItemAtIndex:(NSInteger)index reusingView:(UIScrollView *)view {
    STIMQuickReplySubTableView *tableView = (STIMQuickReplySubTableView *)view;
    // 重用
    if (nil == tableView) {
        tableView = [[STIMQuickReplySubTableView alloc]initWithFrame:swipeView.bounds];
    ...
    if (data == nil) {
        if (groupInfo.count > 0) {
            data = (NSArray *)[[STIMKit sharedInstance] getQuickReplyContentWithGroupId:groupId];
            if (data) {
