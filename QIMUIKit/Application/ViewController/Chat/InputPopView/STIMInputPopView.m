...
//  STChatIphone
//
//  Created by haibin.li on 15/9/22.
//
//
...
@implementation STIMInputPopView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        [_confirmBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] forState:UIControlStateNormal];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(cancelForSTIMInputPopView:)]) {
        [self.delegate cancelForSTIMInputPopView:self];
    }
    [self close];
}
