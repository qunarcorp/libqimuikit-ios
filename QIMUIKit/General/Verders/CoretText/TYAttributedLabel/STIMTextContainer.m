...
@implementation STIMTextContainer
...
- (void)addTextStoragesWithAtrributedString:(NSMutableAttributedString *)attString
{
    if (attString && _textStorageArray.count > 0) {
        ...
        for (id<STIMTextStorageProtocol> textStorage in _textStorageArray) {
            
            // 修正图片替换字符来的误差
            if ([textStorage conformsToProtocol:@protocol(STIMDrawStorageProtocol) ]) {
                continue;
            }
        ...
        for (id<STIMTextStorageProtocol> textStorage in _textStorageArray) {
            ...
            if ([textStorage conformsToProtocol:@protocol(STIMDrawStorageProtocol)]) {
                id<STIMDrawStorageProtocol> drawStorage = (id<STIMDrawStorageProtocol>)textStorage;
...
{
    [textStorageArray sortUsingComparator:^NSComparisonResult(id<STIMTextStorageProtocol> obj1, id<STIMTextStorageProtocol> obj2) {
...
- (void)saveTextStorageRectWithFrame:(CTFrameRef)frame
{
    ...
    for (int i = 0; i < numberOfLines; i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            id<STIMTextStorageProtocol> textStorage = [attributes objectForKey:kQCTextRunAttributedName];
...
- (void)addRunRectDictionary:(NSDictionary *)runRectDictionary
{
    if (runRectDictionary.count < _runRectDictionary.count) {
        ...
        // 遍历不会画出来的
        for (id<STIMTextStorageProtocol>drawStorage in drawStorageArray) {
            if ([drawStorage conformsToProtocol:@protocol(STIMViewStorageProtocol)]) {
                [(id<STIMViewStorageProtocol>)drawStorage didNotDrawRun];
            }
        }
    }
...
- (void)enumerateDrawRectDictionaryUsingBlock:(void (^)(id<STIMDrawStorageProtocol> drawStorage, CGRect rect))block
{
    [_drawRectDictionary enumerateKeysAndObjectsUsingBlock:^(NSValue *rectValue, id<STIMDrawStorageProtocol> drawStorage, BOOL * stop) {
        if (block) {
...
- (BOOL)enumerateRunRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<STIMTextStorageProtocol> textStorage))successBlock
{
...
- (BOOL)enumerateRunRect:(NSDictionary *)runRectDic ContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<STIMTextStorageProtocol> textStorage))successBlock
{
    ...
    // 遍历run位置字典
    [runRectDic enumerateKeysAndObjectsUsingBlock:^(NSValue *keyRectValue, id<STIMTextStorageProtocol> textStorage, BOOL *stop) {
        
        CGRect imgRect = [keyRectValue CGRectValue];
        CGRect rect = CGRectApplyAffineTransform(imgRect, transform);
        
        if ([textStorage conformsToProtocol:@protocol(STIMDrawStorageProtocol) ]) {
            rect = UIEdgeInsetsInsetRect(rect,((id<STIMDrawStorageProtocol>)textStorage).margin);
        }
...
@implementation STIMTextContainer (Add)

- (void)addTextStorage:(id<STIMTextStorageProtocol>)textStorage
{
    if (textStorage) {
...
{
    if (textStorageArray) {
        for (id<STIMTextStorageProtocol> textStorage in textStorageArray) {
            if ([textStorage conformsToProtocol:@protocol(STIMTextStorageProtocol)]) {
...
@implementation STIMTextContainer (Append)
...
- (void)appendTextStorage:(id<QCAppendTextStorageProtocol>)textStorage
{
    if (textStorage) {
        if ([textStorage conformsToProtocol:@protocol(STIMDrawStorageProtocol)]) {
            [(id<STIMDrawStorageProtocol>)textStorage setTextfontAscent:_font.ascender descent:_font.descender];
        } else if ([textStorage conformsToProtocol:@protocol(QCLinkStorageProtocol)]) {
            if (!((id<QCLinkStorageProtocol>)textStorage).textColor) {
                ((id<QCLinkStorageProtocol>)textStorage).textColor = _linkColor;
            }
