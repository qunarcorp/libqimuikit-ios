...
#import "STIMWorkAttachCommentListView.h"
#import <YYModel/YYModel.h>
#import "STIMEmotionManager.h"

#define MaxNumberOfLines 6

@interface STIMWorkMomentCell () <STIMAttributedLabelDelegate, STIMWorkMomentLinkViewTapDelegate, STIMWorkMomentVideoViewTapDelegate> {
    CGFloat _rowHeight;
...
@implementation STIMWorkMomentCell
...
- (void)updateMomentDetail:(NSNotification *)notify {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        STIMWorkMomentModel *momentModel = [STIMWorkMomentModel yy_modelWithDictionary:momentDic];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([postId isEqualToString:self.moment.momentId]) {
            ...
            for (NSDictionary *attachCommentDic in attachCommentList) {
                STIMWorkCommentModel *commentModel = [STIMWorkCommentModel yy_modelWithDictionary:attachCommentDic];
...
- (void)setUPUI {
    ...
    _controlBtn.frame = CGRectMake([[STIMWindowManager shareInstance] getPrimaryWidth] - 15 - 25, _nameLab.top, 28, 30);
    [_controlBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f1cd" size:28 color:[UIColor stimDB_colorWithHex:0x999999]]] forState:UIControlStateNormal];
    ...
    [_showAllBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"moment_Show_More"] forState:UIControlStateNormal];
    [_showAllBtn setTitleColor:[UIColor stimDB_colorWithHex:0xBFBFBF] forState:UIControlStateNormal];
    ...
    _attachCommentListView = [[STIMWorkAttachCommentListView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    ...
    [_likeBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e0e7" size:20 color:[UIColor stimDB_colorWithHex:0x999999]]] forState:UIControlStateNormal];
    [_likeBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e0cd" size:20 color:[UIColor stimDB_colorWithHex:0x00CABE]]] forState:UIControlStateSelected];
    ...
    _likeBtn.layer.borderColor = [UIColor stimDB_colorWithHex:0xDDDDDD].CGColor;
    ...
    [_commentBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f0ef" size:20 color:[UIColor stimDB_colorWithHex:0x999999]]] forState:UIControlStateNormal];
    [_commentBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f0ef" size:20 color:[UIColor stimDB_colorWithHex:0x999999]]] forState:UIControlStateSelected];
    [_commentBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"moment_comment"] forState:UIControlStateNormal];
    [_commentBtn setTitleColor:[UIColor stimDB_colorWithHex:0x999999] forState:UIControlStateNormal];
    [_commentBtn setTitleColor:[UIColor stimDB_colorWithHex:0x999999] forState:UIControlStateSelected];
    ...
    _commentBtn.layer.borderColor = [UIColor stimDB_colorWithHex:0xDDDDDD].CGColor;
    ...
    _showMoreLabel.textColor = [UIColor stimDB_colorWithHex:0x00CABE];
    _showMoreLabel.font = [UIFont systemFontOfSize:14];
}

- (void)setMoment:(STIMWorkMomentModel *)moment {
    _moment = moment;
    [self.lineView removeFromSuperview];
    NSString *userId = [NSString stringWithFormat:@"%@@%@", moment.ownerId, moment.ownerHost];
    if ([userId isEqualToString:[[STIMKit sharedInstance] getLastJid]] && self.notShowControl == NO) {
    ...
    */
    ...
    for (NSInteger i = attay.count - 1; i >= 0; i--) {
        ...
        if ([number integerValue] == 1) {
            [str appendString:[NSString stringWithFormat:@"[obj type=\"topMoment\" value=\"%@\"]", [NSBundle stimDB_localizedStringForKey:@"moment_stick_top"]]];
            [str appendString:@" "];
        } else if ([number integerValue] == 2) {
            [str appendString:[NSString stringWithFormat:@"[obj type=\"hotMoment\" value=\"%@\"]", [NSBundle stimDB_localizedStringForKey:@"moment_hot"]]];
            [str appendString:@" "];
        } else {
//            [cell removeAllSubviews];
    ...
    STIMWorkFeedContentType contentType = moment.content.type;
    ...
    switch (contentType) {
        ...
        case STIMWorkFeedContentTypeImage: {
            ...
            if (exContent) {
                content = exContent;
//                [[STIMEmotionManager sharedInstance] decodeHtmlUrlForText:moment.content.exContent];
            } else {
        ...
        case STIMWorkFeedContentTypeVideo: {
            ...
            break;
        default: {
            content = [[STIMEmotionManager sharedInstance] decodeHtmlUrlForText:moment.content.content];
        }
            break;
    }
    ...
    STIMTextContainer *textContainer = [STIMWorkMomentParser textContainerForMessage:msg fromCache:YES withCellWidth:[[STIMWindowManager shareInstance] getPrimaryWidth] - self.nameLab.left - 20 withFontSize:15 withFontColor:[UIColor stimDB_colorWithHex:0x333333] withNumberOfLines:MaxNumberOfLines];
    ...
    if(self.alwaysFullText) {
        ...
        if (textContainer.totalNumLine > MaxNumberOfLines) {
            if (!self.isFullText) {
                [self.showAllBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"moment_Show_More"] forState:UIControlStateNormal];
            } else {
                textContainer = [STIMWorkMomentParser textContainerForMessage:msg fromCache:NO withCellWidth:[[STIMWindowManager shareInstance] getPrimaryWidth] - self.nameLab.left - 20 withFontSize:15 withFontColor:[UIColor stimDB_colorWithHex:0x333333] withNumberOfLines:0];
                [self.showAllBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"moment_Show_Less"] forState:UIControlStateNormal];
            }
    ...
    if ([[STIMKit sharedInstance] getIsIpad] == YES) {
        self.contentLabel.frame = CGRectMake(self.nameLab.left, bottom + 3, [[STIMWindowManager shareInstance] getPrimaryWidth] - self.nameLab.left - 20, textContainer.textHeight);
    ...
    if (self.isSearch == YES && _moment.likeNum <= 0) {
        NSDictionary *momentDic = [[STIMKit sharedInstance] getWorkMomentWithMomentId:self.moment.momentId];
    ...
    switch (type) {
        ...
        case STIMWorkFeedContentTypeLink: {
            if (self.moment.content.linkContent) {
                _linkView.hidden = NO;
                _linkView.frame = CGRectMake(self.nameLab.left, bottom + 15, [[STIMWindowManager shareInstance] getPrimaryWidth] - self.nameLab.left - 15, 66);
...
- (void)updateAttachCommentList {
    if (self.moment.attachCommentList.count > 0) {
        ...
        _attachCommentListView.backgroundColor = [UIColor stimDB_colorWithHex:0xF3F3F5];
...
- (void)updateLikeUI {
    ...
    if ([[STIMKit sharedInstance] getIsIpad] == YES) {
        _likeBtn.frame = CGRectMake([[STIMWindowManager shareInstance] getPrimaryWidth] - 15 - 70, _rowHeight + 15, 70, 27);
    } else {
        _likeBtn.frame = CGRectMake([[STIMWindowManager shareInstance] getPrimaryWidth] - 15 - 70, _rowHeight + 15, 70, 27);
    }
...
- (void)didLikeMoment:(UIButton *)sender {
    ...
    [[STIMKit sharedInstance] likeRemoteMomentWithMomentId:self.moment.momentId withLikeFlag:likeFlag withCallBack:^(NSDictionary *responseDic) {
...
#pragma mark - STIMWorkMomentVideoViewTapDelegate
