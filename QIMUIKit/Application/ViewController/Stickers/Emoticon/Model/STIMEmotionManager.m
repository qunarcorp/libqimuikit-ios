...
@implementation STIMEmotionManager {
    dispatch_queue_t _emotionManagerQueue;
}

+ (instancetype)sharedInstance {

    static STIMEmotionManager *__global_emotion_manger = nil;

    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __global_emotion_manger = [[STIMEmotionManager alloc] init];
    });
...
- (instancetype)init {
    ...
    if (self) {
        ...
        dispatch_async(_emotionManagerQueue, ^{
            NSNumber *emotionUpdateKey = [[STIMKit sharedInstance] userObjectForKey:kEmotionUpdateKey];
            if (emotionUpdateKey == nil) {
                [[STIMKit sharedInstance] setUserObject:@(YES) forKey:kEmotionUpdateKey];
            }

            emotionUpdateKey = [[STIMKit sharedInstance] userObjectForKey:kEmotionUpdateKey];
            if ([emotionUpdateKey boolValue] == YES) {
                [[STIMKit sharedInstance] removeUserObjectForKey:@"EmojiOne"];
                // 判断cache文件夹
            ...
            BOOL reloadfromUrl = [[[STIMKit sharedInstance] userObjectForKey:@"emotion_check"] boolValue];
            if (reloadfromUrl) {
                [self getAndCheckEmotionsInfoFromHttp];
            }

            if (emotionUpdateKey.boolValue) {
                [[STIMKit sharedInstance] setUserObject:@(NO) forKey:kEmotionUpdateKey];
            }
...
- (UIImage *)getEmotionThumbIconWithImageStr:(NSString *)imageStr BySize:(CGSize)size {
    ...
    if (image == nil) {
        ...
        [_emotionThumbIconDic setSTIMSafeObject:image forKey:imageStr];
    }
    return image;
}
...
- (void)expandNormalZipFileNamed:(NSString *)fileName {
    ...
    if (![[STIMKit sharedInstance] userObjectForKey:fileName] || ![[NSFileManager defaultManager] fileExistsAtPath:[outputPath stringByAppendingString:[[[STIMKit sharedInstance] userObjectForKey:fileName] objectForKey:@"xmlFilePath"]] isDirectory:nil]) {
        ...
        [[STIMKit sharedInstance] setUserObject:[NSDictionary dictionaryWithObjectsAndKeys:xmlPath, @"xmlFilePath", [NSString stringWithFormat:@"/%@", [[zip getZipFileContents] firstObject]], @"imagePreFix", nil] forKey:fileName];
    } else {
        NSString *xmlPath = [outputPath stringByAppendingString:[[[STIMKit sharedInstance] userObjectForKey:fileName] objectForKey:@"xmlFilePath"]];
        NSData *fileData = [NSData dataWithContentsOfFile:xmlPath];
        if (fileData) {
//            STIMVerboseLog(@"expandNormalZipFileNamed : %@", self.currentPrefixPath);
//            STIMVerboseLog(@"expandNormalZipFileNamed FileName : %@ ===  %@", fileName, [[STIMKit sharedInstance] userObjectForKey:fileName]);
            self.currentPrefixPath = [[NSString stringWithFormat:@"%@", fileName] stringByAppendingPathComponent:[[[STIMKit sharedInstance] userObjectForKey:fileName] objectForKey:@"imagePreFix"]];
//            STIMVerboseLog(@"expandNormalZipFileNamed currentPrefixPath : %@", self.currentPrefixPath);
            [self parseXMLData:fileData];
        }
    }
}
...
//     STIMVerboseLog(@"开始解析文档");
    [self.tempArray removeAllObjects];
//    STIMVerboseLog(@"开始解析文档 : %@", self.tempArray);
}

- (void)parserDidEndDocument:(NSXMLParser *)parser {
//    STIMVerboseLog(@"结束解析文档");
}
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict {
    ...
    if ([elementName isEqualToString:@"FACE"] && [attributeDict count] > 0) {
        ...
        if ([shortCut length] > 0) {
            ...
            [self.defaultEmotionShorCutDic setSTIMSafeObject:shortCut forKey:tip];
        }
    }
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    if (self.bNeedStore == TRUE) {
        ...
        if ([self.tempArray count] > 0) {
            ...
            [self.defaultEmotionDic setSTIMSafeObject:key forKey:value];
            [self.shortcutForImagePathDic setSTIMSafeObject:value forKey:key];
...
//                STIMVerboseLog(@"Emotion Name Error {%@}",string);
            }
...
- (void)loadEmotionInfoDic {
    ...
    for (NSString *packageId in self.emotionDispPkIdList) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:resourcePath]) {
            [_emotionsInfoDic setSTIMSafeObject:[NSMutableDictionary dictionaryWithContentsOfFile:resourcePath] forKey:packageId];
        }
    }
}
...
- (void)initializeDisplayPkIdList:(NSArray *)pkidList {
    ...
    for (NSString *pkId in pkidList) {
        if (pkId.length > 0) {
            ...
            [self.emotionsInfoDic setSTIMSafeObject:emotionInfoDic forKey:pkId];
...
//    STIMVerboseLog(@"表情AbsolutePat : %@", cachePath);
    if ([[NSFileManager defaultManager] fileExistsAtPath:cachePath]) {
        return cachePath;
    }
    return nil;
...
- (void)getAndCheckEmotionsInfoFromHttp {
    ...
    NSString *destUrl = [NSString stringWithFormat:@"%@/file/v1/emo/d/e/config?p=%@", [[STIMKit sharedInstance] qimNav_InnerFileHttpHost], [STIMKit getSTIMProjectType] == STIMProjectTypeQChat ? @"qchat" : @"qtalk"];
    ...
    [STIMHTTPClient sendRequest:request complete:^(STIMHTTPResponse *response) {
        if (response.code == 200) {
            NSError *errol = nil;
            NSArray *result = [[STIMJSONSerializer sharedInstance] deserializeObject:response.data error:&errol];
//            "name": "牛驼表情",
...
- (void)getEmotionImageFromHttpForPkId:(NSString *)pkId shortcut:(NSString *)shortcut signKey:(NSString *)signKey {
    ...
    dispatch_async(_emotionManagerQueue, ^{
        ...
        [paramDic setSTIMSafeObject:[STIMKit getLastUserName] forKey:@"u"];
        [paramDic setSTIMSafeObject:[[STIMKit sharedInstance] myRemotelogginKey] forKey:@"k"];
        NSURL *url = nil;
        if (pkId.length) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:cachePath]) {
            ...
            for (NSString *xmlFileName in [zip getZipFileContents]) {
                if ([xmlFileName hasSuffix:@".xml"]) {
                    ...
                    [_emotionsInfoDic setSTIMSafeObject:emotionInfoDic forKey:pkId];
...
@implementation STIMEmotionManager (DecodeHTMLURL)
...
- (NSString *)decodeHtmlUrlForText:(NSString *)text WithFilterAppendArray:(NSArray *)paramDict {
    ...
    for (NSString *match in matchList) {
        ...
        [tiHuan setSTIMSafeObject:tiStr forKey:temp];
        i++;
    }
