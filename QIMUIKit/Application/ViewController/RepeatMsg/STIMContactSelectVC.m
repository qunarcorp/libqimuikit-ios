...
@interface STIMContactSelectVC () <UIGestureRecognizerDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,STIMFriendListSelectionVCDelegate,STIMGroupListVCDelegate>
{
...
@implementation STIMContactSelectVC
...
- (UITextField *)searchBar {
    if (_searchBar == nil) {
        ...
        _searchBar.placeholder = [NSBundle stimDB_localizedStringForKey:@"Search_contacts/groups"];
...
- (UIView *)tableViewHeaderView {
    ...
    [friendListJump setTitle:[NSBundle stimDB_localizedStringForKey:@"Select_from_friends"] forState:UIControlStateNormal];
    ...
    [groupListJump setTitle:[NSBundle stimDB_localizedStringForKey:@"Select_from_groups"] forState:UIControlStateNormal];
...
- (void)getRecentUserList {
    _recentlyUserList = [[STIMKit sharedInstance] getSessionList];
...
- (void)refreshDisplayUsers {
    ...
    for (NSDictionary * infoDic in _selectedUsers) {
        ...
        if (isGroup) {
            [heaerImgView stimDB_setImageWithJid:jid WithChatType:ChatType_GroupChat];
        } else {
            [heaerImgView stimDB_setImageWithJid:jid];
        }
    ...
    if (self.defaultSelectIds.count) {
        for (NSString * userId in self.defaultSelectIds) {
            ...
            if ([userId rangeOfString:@"conference"].location != NSNotFound) {
                infoDic = [[STIMKit sharedInstance] getGroupCardByGroupId:userId];
                isGroup = YES;
            }else{
                infoDic = [[STIMKit sharedInstance] getUserInfoByUserId:userId];
            }
...
- (void)complete:(id)sender{
    if (self.delegate && [self.delegate respondsToSelector:@selector(STIMContactSelectVC:completeWithUsersInfo:)]) {
        [self.delegate STIMContactSelectVC:self completeWithUsersInfo:[self selectedUsers]];
    }
    [self goBack:sender];
}
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (textField.text.length + string.length - range.length > 0) {
        [[self searchUserList] addObjectsFromArray:[[STIMKit sharedInstance] searchUserListBySearchStr:searchText]];
    }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    label.text = [NSBundle stimDB_localizedStringForKey:@"Recent_contacts"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMContactUserCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[STIMContactUserCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.allowMulSelect) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(STIMContactSelectVC:completeWithUsersInfo:)]) {
            [self.delegate STIMContactSelectVC:self completeWithUsersInfo:@[@{@"userId":jid,@"nick":nickName,@"isGroup":@(isGroup)}]];
        }
        [self dismissViewControllerAnimated:YES completion:^{
...
#pragma mark - STIMFriendListSelectionVCDelegate,STIMGroupListVCDelegate
