...
@implementation STIMAddSomeCell{
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        CGFloat headerWidth = [STIMAddSomeCell getCellHeight] - 10 * 2;
        ...
        _lineView= [[UIView alloc] initWithFrame:CGRectMake(_titleLabel.left, [STIMAddSomeCell getCellHeight]-0.5, [UIScreen mainScreen].bounds.size.width-_titleLabel.left, 0.5)];
...
- (void)refreshUI{
    ...
    [_headerImageView stimDB_setImageWithJid:jid];
    /*
    __block UIImage * headImage = [[STIMKit sharedInstance] getUserHeaderImageByUserId:jid];
    if (!headImage) {
        NSString *iconUrl = [self.userInfoDic objectForKey:@"icon"];
        if (![iconUrl isKindOfClass:[NSNull class]]) {
            BOOL isHTTPRequest = [iconUrl stimDB_hasPrefixHttpHeader];
            if (!isHTTPRequest) {
                iconUrl = [NSString stringWithFormat:@"%@/%@",[STIMKit sharedInstance].qimNav_InnerFileHttpHost, iconUrl];
            }
            __block NSData *headImageData = [[STIMKit sharedInstance] getFileDataFromUrl:iconUrl width:headerWidth height:headerWidth  forCacheType:STIMFileCacheTypeColoction];
            if (!headImageData.length) {
                [[STIMKit sharedInstance] downloadImage:iconUrl width:headerWidth height:headerWidth forCacheType:STIMFileCacheTypeColoction complation:^(NSData *data) {
                    headImageData = data;
