...
#import "QIMProgressView.h"
#import "UIImage+QIMImagePicker.h"

@interface QIMAssetCell ()
...
- (void)setModel:(QIMAssetModel *)model {
    _model = model;
    self.representedAssetIdentifier = model.asset.localIdentifier;
    int32_t imageRequestID = [[QIMImagePickerManager manager] getPhotoWithAsset:model.asset photoWidth:self.qim_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[QIMImagePickerManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(QIMAssetCellType)type {
    _type = type;
    if (type == QIMAssetCellTypePhoto || type == QIMAssetCellTypeLivePhoto || (type == QIMAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    } else if (type == QIMAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[QIMImagePickerManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        [self hideProgressView];
...
- (void)reload:(NSNotification *)noti {
    QIMImagePickerController *tzImagePickerVc = (QIMImagePickerController *)noti.object;
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage qim_imageNamedFromQIMImagePickerBundle:@"VideoSendIcon"]];
...
- (QIMProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[QIMProgressView alloc] init];
...
- (void)setModel:(QIMAlbumModel *)model {
    ...
    [[QIMImagePickerManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
