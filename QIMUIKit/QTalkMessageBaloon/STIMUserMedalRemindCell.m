...
#define kCommonUserMedalRemindCellWidth       IS_Ipad ? ([UIScreen mainScreen].stimDB_rightWidth  * 240 / 320) : ([UIScreen mainScreen].bounds.size.width * 5/7)

@interface STIMUserMedalRemindCell () <STIMMenuImageViewDelegate>
...
@implementation STIMUserMedalRemindCell
...
- (void)onClick {
    if (self.message.messageType == STIMMessageTypeUserMedalRemind) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STIMFastEntrance openUserMedalFlutterWithUserId:[[STIMKit sharedInstance] getLastJid]];
        });
    } else {
...
- (void)refreshUI {
    ...
    CGFloat cellHeight = [STIMUserMedalRemindCell getCellHeightWithMessage:self.message chatType:self.chatType];
    ...
    [allmessageAttriStr addAttribute:NSForegroundColorAttributeName value:[UIColor stimDB_colorWithHex:0x009ad6 alpha:1.0] range:replaceRange];
