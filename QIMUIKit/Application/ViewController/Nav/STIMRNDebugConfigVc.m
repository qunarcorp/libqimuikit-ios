//
//  STIMRNDebugConfigVc.m
//  STIMUIKit
//
//  Created by lihaibin.li on 2019/5/27.
//  Copyright © 2019 STIM. All rights reserved.
//

#import "STIMRNDebugConfigVc.h"

@interface STIMRNDebugConfigVc ()
...
@implementation STIMRNDebugConfigVc
...
- (UITextView *)opsRNTextView {
    if (!_opsRNTextView) {
        ...
        NSString *opsFoundRNDebugUrl = [[STIMKit sharedInstance] userObjectForKey:@"opsFoundRNDebugUrl"];
...
- (UITextView *)searchRNTextView {
    if (!_searchRNTextView) {
        ...
        NSString *qtalkSearchRNDebugUrl = [[STIMKit sharedInstance] userObjectForKey:@"qtalkSearchRNDebugUrl"];
...
- (UILabel *)qtalkRNLabel {
    if (!_qtalkRNLabel) {
        ...
        [_qtalkRNLabel setText:@"STIMRN调试地址"];
...
- (UITextView *)qtalkRNTextView {
    if (!_qtalkRNTextView) {
        ...
        NSString *qtalkFoundRNDebugUrl = [[STIMKit sharedInstance] userObjectForKey:@"qtalkFoundRNDebugUrl"];
...
- (void)initNav {
    self.title = @"RN调试配置页面";
    UIBarButtonItem *save = [[UIBarButtonItem alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Save"] style:UIBarButtonItemStylePlain target:self action:@selector(saverRNDebugConfig)];
...
- (void)saverRNDebugConfig {
    [[STIMKit sharedInstance] setUserObject:self.opsRNTextView.text forKey:@"opsFoundRNDebugUrl"];
    [[STIMKit sharedInstance] setUserObject:self.searchRNTextView.text forKey:@"qtalkSearchRNDebugUrl"];
    [[STIMKit sharedInstance] setUserObject:self.qtalkRNTextView.text forKey:@"qtalkFoundRNDebugUrl"];
