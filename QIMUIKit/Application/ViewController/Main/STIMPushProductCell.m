...
@implementation STIMPushProductCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _typeBgView.backgroundColor = [UIColor stimDB_colorWithHex:0x00000000 alpha:0.3];
        ...
        _bottomBgView.backgroundColor = [UIColor stimDB_colorWithHex:0x000000 alpha:0.5];
        ...
        [_sendBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Send"] forState:UIControlStateNormal];
...
-(void)layoutSubviews{
    ...
    //set values
    [_productImageView stimDB_setImageWithURL:[NSURL URLWithString:_infoDic[@"imageUrl"]] placeholderImage:nil];
    ...
    //rect frame
    CGSize size = [_typeLabel.text stimDB_sizeWithFontCompatible:_typeLabel.font];
    ...
    size = [_tagLabel.text stimDB_sizeWithFontCompatible:_tagLabel.font];
    _tagLabel.frame = CGRectMake(0, _productImageView.height - 30, size.width, 30);
    
    size = [_priceLabel.text stimDB_sizeWithFontCompatible:_priceLabel.font];
    
    _priceLabel.frame = CGRectMake(_titleLabel.left, _titleLabel.bottom + 5, size.width, 20);
    
    size = [_marketPriceLabel.text stimDB_sizeWithFontCompatible:_marketPriceLabel.font];
    ...
    [attri addAttribute:NSStrikethroughColorAttributeName value:[UIColor stimDB_colorWithHex:0x999999 alpha:1] range:NSMakeRange(0, _marketPriceLabel.text.length)];
    [_marketPriceLabel setAttributedText:attri];
    
    size = [_shopNameLabel.text stimDB_sizeWithFontCompatible:_shopNameLabel.font];
