...
#import "STIMMWCommon.h"
#import "STIMMWPhotoBrowser.h"
#import "STIMMWPhotoBrowserPrivate.h"
...
static void * STIMMWVideoPlayerObservation = &STIMMWVideoPlayerObservation;

@interface STIMMWPhotoBrowser () <LCActionSheetDelegate>

@end

@implementation STIMMWPhotoBrowser
...
- (void)_initialisation {
    ...
    // Listen for STIMMWPhoto notifications
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleSTIMMWPhotoLoadingDidEndNotification:)
                                                 name:STIMMWPHOTO_LOADING_DID_END_NOTIFICATION
                                               object:nil];
...
- (void)dealloc {
    ...
    [[STIMImageManager sharedInstance] clearMemory]; // clear memory
}
...
- (void)viewDidLoad {
    ...
    if (self.displayNavArrows) {
        NSString *arrowPathFormat = @"STIMMWPhotoBrowser.bundle/UIBarButtonItemArrow%@";
...
- (void)performLayout {
    ...
    if (_enableGrid) {
        hasItems = YES;
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/UIBarButtonItemGrid" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)]];
    } else {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_viewHasAppearedInitially) {
        if (_autoPlayOnAppear) {
            STIMMWPhoto *photo = [self photoAtIndex:_currentPageIndex];
...
- (void)willMoveToParentViewController:(UIViewController *)parent {
    if (parent && _hasBelongedToViewController) {
        [NSException raise:@"STIMMWPhotoBrowser Instance Reuse" format:@"STIMMWPhotoBrowser instances cannot be reused."];
    }
}
...
- (void)layoutVisiblePages {
	...
	for (STIMMWZoomingScrollView *page in _visiblePages) {
...
- (id<STIMMWPhoto>)photoAtIndex:(NSUInteger)index {
    id <STIMMWPhoto> photo = nil;
...
- (id<STIMMWPhoto>)thumbPhotoAtIndex:(NSUInteger)index {
    id <STIMMWPhoto> photo = nil;
...
- (STIMMWCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    STIMMWCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <STIMMWPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[STIMMWCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (STIMImage *)imageForPhoto:(id<STIMMWPhoto>)photo {
	if (photo) {
...
- (void)loadAdjacentPhotosIfNecessary:(id<STIMMWPhoto>)photo {
    STIMMWZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <STIMMWPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
                    STIMMWLog(@"Pre-loading image at index %lu", (unsigned long)pageIndex-1);
                }
            }
...
#pragma mark - STIMMWPhoto Loading Notification

- (void)handleSTIMMWPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <STIMMWPhoto> photo = [notification object];
    STIMMWZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImage]) {
            // Successful load
...
- (void)tilePages {
	...
	for (STIMMWZoomingScrollView *page in _visiblePages) {
		...
		if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
			...
			STIMMWLog(@"Removed page at index %lu", (unsigned long)pageIndex);
		}
	}
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
			STIMMWZoomingScrollView *page = [self dequeueRecycledPage];
			if (!page) {
				page = [[STIMMWZoomingScrollView alloc] initWithPhotoBrowser:self];
			}
			[_visiblePages addObject:page];
			[self configurePage:page forIndex:index];

			[_pagingScrollView addSubview:page];
			STIMMWLog(@"Added page at index %lu", (unsigned long)index);
            
            // Add caption
            STIMMWCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
            ...
            if (page.displayingVideo) {
                ...
                [playButton setImage:[UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/PlayButtonOverlayLarge" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateNormal];
                [playButton setImage:[UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/PlayButtonOverlayLargeTap" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateHighlighted];
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/ImageSelectedOff" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateNormal];
                UIImage *selectedOnImage;
                if (self.customImageSelectedIconName) {
                    selectedOnImage = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:self.customImageSelectedIconName];
                } else {
                    selectedOnImage = [UIImage imageForResourcePath:@"STIMMWPhotoBrowser.bundle/ImageSelectedOn" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]];
                }
...
- (void)updateVisiblePageStates {
    ...
    for (STIMMWZoomingScrollView *page in copy) {
...
- (STIMMWZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
	STIMMWZoomingScrollView *thePage = nil;
	for (STIMMWZoomingScrollView *page in _visiblePages) {
		if (page.index == index) {
			thePage = page; break;
		}
...
- (STIMMWZoomingScrollView *)pageDisplayingPhoto:(id<STIMMWPhoto>)photo {
	STIMMWZoomingScrollView *thePage = nil;
	for (STIMMWZoomingScrollView *page in _visiblePages) {
		if (page.photo == photo) {
			thePage = page; break;
...
- (void)configurePage:(STIMMWZoomingScrollView *)page forIndex:(NSUInteger)index {
	page.frame = [self frameForPageAtIndex:index];
    page.index = index;
    page.photo = [self photoAtIndex:index];
}

- (STIMMWZoomingScrollView *)dequeueRecycledPage {
	STIMMWZoomingScrollView *page = [_recycledPages anyObject];
	if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) { 
            ...
            if (photo != [NSNull null]) {
                ...
                STIMMWLog(@"Released underlying image at index %lu", (unsigned long)i);
            }
        }
    }
    ...
    id <STIMMWPhoto> currentPhoto = [self photoAtIndex:index];
...
- (CGRect)frameForCaptionView:(STIMMWCaptionView *)captionView atIndex:(NSUInteger)index {
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController = [[STIMMWGridViewController alloc] init];
...
- (void)hideGrid {
    ...
    STIMMWGridViewController *tmpGridController = _gridController;
    _gridController = nil;
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        for (STIMMWZoomingScrollView *page in _visiblePages) {
            if (page.captionView) {
                STIMMWCaptionView *v = page.captionView;
...
- (void)actionButtonPressed:(id)sender {
    ...
    id <STIMMWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            if (![[STIMMWQRCodeActivity sharedInstance] canPerformQRCodeWithImage:[photo underlyingImage]]) {
                
                LCActionSheet *actionSheet = [LCActionSheet sheetWithTitle:nil
                                                                  delegate:self
                                                         cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"]
                                                         otherButtonTitles: [NSBundle stimDB_localizedStringForKey:@"Send to Friends"], [NSBundle stimDB_localizedStringForKey:@"Add to Stickers"], [NSBundle stimDB_localizedStringForKey:@"Q_Save"], [NSBundle stimDB_localizedStringForKey:@"More"], nil];
                ...
                LCActionSheet *actionSheet = [LCActionSheet sheetWithTitle:nil
                                                                  delegate:self
                                                         cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"]
                                                         otherButtonTitles:[NSBundle stimDB_localizedStringForKey:@"Send to Friends"], [NSBundle stimDB_localizedStringForKey:@"Add to Stickers"], [NSBundle stimDB_localizedStringForKey:@"Q_Save"], [NSBundle stimDB_localizedStringForKey:@"Extract QR Code"], [NSBundle stimDB_localizedStringForKey:@"More"], nil];
                self.activityViewController = nil;
                [self hideControlsAfterDelay];
                [self hideProgressHUD:YES];
                [actionSheet show];
    ...
    switch (buttonIndex) {
        case 1: {
            ...
            if (photoUrl.length > 0) {
                ...
                NSString *fileName = [STIMKit updateLoadFile:photo.photoData WithMsgId:nil WithMsgType:STIMMessageType_ImageNew WithPathExtension:nil];
                NSString *fileUrl = @"";
                ...
                UIImage *image = [STIMImage imageWithData:photo.photoData];
        ...
        case 3: {
            ...
            if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
                ...
                if (photoData.length > 0) {
                    ...
                    NSString *localPhotoPath = [[STIMImageManager sharedInstance] defaultCachePathForKey:photo.photoURL.absoluteString];
                    NSData *imageData = [NSData dataWithContentsOfFile:localPhotoPath];
                    if (imageData.length > 0) {
                        NSInteger format = [[STIMImageManager sharedInstance] stimDB_imageFormatForImageData:imageData];
                        if (format == 4) {
        ...
        case 4: {
            if ([[STIMMWQRCodeActivity sharedInstance] canPerformQRCodeWithImage:[photo underlyingImage]]) {
                [[STIMMWQRCodeActivity sharedInstance] setFromPhotoBrowser:self];
                [[STIMMWQRCodeActivity sharedInstance] performActivity];
            } else {
                id <STIMMWPhoto> photo = [self photoAtIndex:_currentPageIndex];
