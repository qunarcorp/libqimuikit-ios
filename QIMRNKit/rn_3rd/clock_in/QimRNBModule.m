...
//    return [NSURL URLWithString:@"http://100.80.128.180:8081/index.ios.bundle?platform=ios&dev=true"];
        ...
        [[QIMKit sharedInstance] getPhoneNumberWithUserId:userId withCallBack:^(NSString *phoneNumberStr) {
            if (phoneNumberStr.length > 0) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:[NSBundle qim_localizedStringForKey:@"common_prompt"] message:message preferredStyle:UIAlertControllerStyleAlert];
                ...
                showNumberId = nil;
                dispatch_async(dispatch_get_main_queue(), ^{
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle qim_localizedStringForKey:@"common_prompt"] message:@"没有查询到相应记录" delegate:self cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"common_ok"] otherButtonTitles:nil, nil];
                    [alert show];
                });
            }
        }];
...
//        UINavigationController *navVC = (UINavigationController *)[[[UIApplication sharedApplication] keyWindow] rootViewController];
        ...
        switch (mode) {
            ...
            {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle qim_localizedStringForKey:@"common_prompt"] message:@"对方拒绝添加好友。" delegate:nil cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"common_ok"] otherButtonTitles:nil];
                [alertView show];
            }
                break;
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (isSuccess) {
                        ...
                        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle qim_localizedStringForKey:@"common_prompt"] message:@"删除好友失败。" delegate:nil cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"common_ok"] otherButtonTitles:nil];
                        [alertView show];
                    }
                });
            });
