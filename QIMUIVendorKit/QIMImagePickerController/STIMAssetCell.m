...
@property (nonatomic, strong) STIMProgressView *progressView;
@property (nonatomic, assign) int32_t bigImageRequestID;
@end

@implementation STIMAssetCell

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reload:) name:@"QIM_PHOTO_PICKER_RELOAD_NOTIFICATION" object:nil];
    return self;
}
...
- (void)setModel:(STIMAssetModel *)model {
    ...
    int32_t imageRequestID = [[STIMImagePickerManager manager] getPhotoWithAsset:model.asset photoWidth:self.qim_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[STIMImagePickerManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(STIMAssetCellType)type {
    _type = type;
    if (type == STIMAssetCellTypePhoto || type == STIMAssetCellTypeLivePhoto || (type == STIMAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == STIMAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.qim_left = self.videoImgView.qim_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == STIMAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.qim_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:QIMOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[STIMImagePickerManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        [self hideProgressView];
...
- (void)reload:(NSNotification *)noti {
    STIMImagePickerController *tzImagePickerVc = (STIMImagePickerController *)noti.object;
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage qim_imageNamedFromSTIMImagePickerBundle:@"VideoSendIcon"]];
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.qim_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.qim_width - 27, 3, 24, 24);
    ...
    _imageView.frame = CGRectMake(0, 0, self.qim_width, self.qim_height);
    
    static CGFloat progressWH = 20;
    CGFloat progressXY = (self.qim_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    _bottomView.frame = CGRectMake(0, self.qim_height - 17, self.qim_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.qim_right, 0, self.qim_width - self.videoImgView.qim_right - 5, 17);
...
@interface QIMAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation QIMAlbumCell
...
- (void)setModel:(QIMAlbumModel *)model {
    _model = model;
    ...
    [[STIMImagePickerManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.qim_width - 24, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.qim_height - titleHeight) / 2, self.qim_width - 80 - 50, titleHeight);
...
@implementation QIMAssetCameraCell
