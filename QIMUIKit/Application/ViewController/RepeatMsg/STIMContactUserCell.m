...
@implementation STIMContactUserCell{
...
- (void)refreshUI{
    ...
    [_prefrenceImageView setHidden:YES];
    if (self.isGroup) {
        [_headerView stimDB_setImageWithJid:self.jid WithChatType:ChatType_GroupChat];
        if (!self.name) {
        ...
        [_headerView stimDB_setImageWithJid:self.jid WithChatType:ChatType_System];
        [_nameLabel setText:[NSBundle stimDB_localizedStringForKey:@"System Messages"]];//@"系统消息"];
    } else {
         [_headerView stimDB_setImageWithJid:self.jid WithChatType:ChatType_SingleChat];
        NSDictionary *userInfo = [[STIMKit sharedInstance] getUserInfoByUserId:self.jid];
