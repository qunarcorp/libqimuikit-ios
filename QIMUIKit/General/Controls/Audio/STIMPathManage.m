...
+ (NSString*)getCacheDirectory
{
    ...
    if (! [fileManager fileExistsAtPath:[storeUrl path]]) {
        ...
        if (!bCreateDir) {
            STIMVerboseLog(@"create Voices directory failed");
        }
    }
...
//{
//    NSString* fileDirectory = [[[STIMPathManage getReceiveCacheDirectory]stringByAppendingPathComponent:_fileName] stringByAppendingPathExtension:_type];
...
{
    NSString* fileDirectory = [[[STIMPathManage getCacheDirectory]stringByAppendingPathComponent:_fileName] stringByAppendingPathExtension:_type];
    return fileDirectory;
}
...
{
    NSString *fileDirectory  = [[[STIMPathManage getCacheDirectory] stringByAppendingPathComponent:fileName] stringByAppendingPathExtension:type];
    [dataToSave writeToFile:fileDirectory atomically:YES];
    STIMVerboseLog(@"保存到了： %@",fileDirectory);
...
+ (NSString*)getPathByFileName:(NSString *)_fileName{
    NSString* fileDirectory = [[STIMPathManage getCacheDirectory]stringByAppendingPathComponent:_fileName];
    return fileDirectory;
}
