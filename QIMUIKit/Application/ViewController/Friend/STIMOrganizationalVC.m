...
@interface STIMOrganizationalVC () <UITableViewDelegate, UITableViewDataSource>
...
@implementation STIMOrganizationalVC
...
- (void)initWithNav{
    [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"contact_tab_organization"]];
    if ([STIMKit getSTIMProjectType] == STIMProjectTypeStartalk) {
        ...
        [button setTitleColor:[UIColor stimDB_colorWithHex:0x00CABE] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor stimDB_colorWithHex:0x00CABE] forState:UIControlStateSelected];
...
- (void)inviteMemberToCompany:(id)sender {
    NSString *inviteStr = [NSString stringWithFormat:@"https://im.qunar.com/new/#/user_list?domain=%@", [[STIMKit sharedInstance] getDomain]];
    [STIMFastEntrance openWebViewForUrl:inviteStr showNavBar:YES];
}

#pragma mark - table delegate
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    
    STIMVerboseLog(@"numberOfSectionsInTableView getSTIMMergedRootBranch");
    return 1;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (item.childNodesDict.count > 0) {
        ...
        if (self.shareCard == YES) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [STIMFastEntrance openUserCardVCByUserId:xmppId];
            });
        }
    }
}
