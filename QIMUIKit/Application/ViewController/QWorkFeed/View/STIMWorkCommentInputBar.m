...
@interface STIMWorkCommentInputBar () <UITextViewDelegate>

@property (nonatomic, strong) UIImageView *iconView;

@property (nonatomic, strong) UIImageView *headerImageView;

@property (nonatomic, strong) STIMWorkCommentTextView *commentTextView;
...
@implementation STIMWorkCommentInputBar
...
- (UIImageView *)iconView {
    if (!_iconView) {
        ...
        _iconView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"q_work_triangle"];
    }
    return _iconView;
}
...
//        [_headerImageView stimDB_setImageWithJid:[[STIMKit sharedInstance] getLastJid]];
...
- (UIButton *)likeBtn {
    if (!_likeBtn) {
        ...
        [_likeBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e0e7" size:26 color:[UIColor stimDB_colorWithHex:0x999999]]] forState:UIControlStateNormal];
        [_likeBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e0cd" size:26 color:[UIColor stimDB_colorWithHex:0x00CABE]]] forState:UIControlStateSelected];
...
- (UIButton *)sendBtn {
    if (!_sendBtn) {
        ...
        [_sendBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e644" size:36 color:[UIColor stimDB_colorWithHex:0xBFBFBF]]] forState:UIControlStateDisabled];
        [_sendBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e644" size:36 color:[UIColor stimDB_colorWithHex:0x00CABE]]] forState:UIControlStateNormal];
...
- (UILabel *)placeholderLabel {
    if (!_placeholderLabel) {
        ...
        _placeholderLabel.textColor = [UIColor stimDB_colorWithHex:0x999999];
    }
    return _placeholderLabel;
}

- (STIMWorkCommentTextView *)commentTextView {
    if (!_commentTextView) {
        _commentTextView = [[STIMWorkCommentTextView alloc] initWithFrame:CGRectMake(self.headerImageView.right + 6, 10, CGRectGetWidth(self.frame) - self.headerImageView.right - 6 - 16 - self.likeBtn.width - 6, 36)];
        _commentTextView.textAlignment = NSTextAlignmentLeft;
        _commentTextView.delegate = self;
        _commentTextView.backgroundColor = [UIColor stimDB_colorWithHex:0xF0F0F0];
        _commentTextView.font = [UIFont systemFontOfSize:15];
        _commentTextView.textColor = [UIColor stimDB_colorWithHex:0x333333];
...
- (void)reloadUserIdentifier {
    ...
    if (isAnonymous == NO) {
        ...
        [_headerImageView stimDB_setImageWithURL:[NSURL URLWithString:anonymousPhoto]];
    }
    
    /*
     Mark by 匿名
    if ([[STIMWorkMomentUserIdentityManager sharedInstance] isAnonymous] == NO) {
        [self.headerImageView stimDB_setImageWithJid:[[STIMKit sharedInstance] getLastJid]];
    } else {
        ...
        [self.headerImageView stimDB_setImageWithURL:[NSURL URLWithString:anonymousPhoto]];
    }
    */
}
...
- (void)didLikeComment:(UIButton *)sender {
    ...
    [[STIMKit sharedInstance] likeRemoteMomentWithMomentId:self.momentId withLikeFlag:likeFlag withCallBack:^(NSDictionary *responseDic) {
            ...
            for (NSString *userXmppJid in selectUsers) {
                if (userXmppJid.length > 0) {
                    ...
                    if (userInfo.count > 0) {
                        ...
                        CGSize size = [memberName stimDB_sizeWithFontCompatible:self.commentTextView.font];
                        atTextAttachment.image = [UIImage stimDB_imageWithColor:[UIColor stimDB_colorWithHex:0xF0F0F0] size:CGSizeMake(size.width, self.commentTextView.font.lineHeight) text:memberName textAttributes:@{NSFontAttributeName:self.commentTextView.font} circular:NO];
...
- (void)sendComment {
    ...
    if (self.commentTextView.text.length > 0 && ![self isEmpty:self.commentTextView.attributedText.string]) {
        ...
        NSString *finallyContent = [[STIMMessageTextAttachment sharedInstance] getStringFromAttributedString:self.commentTextView.attributedText WithOutAtInfo:&outATInfoArray];
