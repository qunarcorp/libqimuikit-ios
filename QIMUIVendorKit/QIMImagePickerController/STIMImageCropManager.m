...
@implementation STIMImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.qim_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[STIMImagePickerManager manager] fixOrientation:cropedImage];
...
+ (UIImage *)sd_qim_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [STIMImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
