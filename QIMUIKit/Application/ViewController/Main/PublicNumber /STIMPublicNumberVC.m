...
#import "STIMMicroTourGuideVC.h"
#import "STIMNavBackBtn.h"
#if __has_include("STIMIPadWindowManager.h")
#import "STIMIPadWindowManager.h"
#endif

@interface STIMPublicNumberVC ()<UITableViewDataSource,UITableViewDelegate,UISearchControllerDelegate,UISearchDisplayDelegate,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UIAlertViewDelegate>{
...
@implementation STIMPublicNumberVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    _dataSource = [NSMutableArray arrayWithArray:[[STIMKit sharedInstance] getPublicNumberList]];
    [self initWithNav];
...
- (void)initWithNav{
    ...
    [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"contact_tab_public_number"]];
    
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"header_icon_more"] style:UIBarButtonItemStylePlain target:self action:@selector(onMoreClick)];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        ...
        NSDictionary *cardDic = [[STIMKit sharedInstance] getPublicNumberCardByJId:[NSString stringWithFormat:@"%@@%@",publicNumberId,[[STIMKit sharedInstance] getDomain]]];
        if (cardDic.count > 0) {
...
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"查找公众号" message:@"输入公众号ID打开名片，查看关注！" delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"] otherButtonTitles:[NSBundle stimDB_localizedStringForKey:@"Confirm"],nil];
...
- (void)initMoreView{
    ...
    NSString *qrCodeStr = [NSBundle stimDB_localizedStringForKey:@"explore_tab_qrcode"];
    NSString *searchPNStr = [NSBundle stimDB_localizedStringForKey:@"common_search_public_number"];
    NSString *moreStr = [NSBundle stimDB_localizedStringForKey:@"common_more"];
    ...
    [_moreView setBackgroundColor:[UIColor stimDB_colorWithHex:0x0 alpha:0.3]];
    ...
    [imageBgView setImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"mailapp_Attach_list"] stretchableImageWithLeftCapWidth:15 topCapHeight:20]];
    ...
    [button setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor qtalkTableDefaultColor]] forState:UIControlStateHighlighted];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (publicNumberCardInfoDic) {
        ...
        if (isMicroTourGuide) {
            STIMMicroTourGuideVC *microTourGuideVC = [[STIMMicroTourGuideVC alloc] init];
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar { 
    _searchDataSource = [[STIMKit sharedInstance] searchPublicNumberListByKeyStr:[searchBar text]];
...
{
    _searchDataSource = [[STIMKit sharedInstance] searchPublicNumberListByKeyStr:searchString];
    return YES;
}
