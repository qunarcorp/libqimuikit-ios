//
//  STIMSelectComponyViewController.m
//  STIMUIKit
//
...
@interface STIMSelectComponyViewController () <UITableViewDelegate, UITableViewDataSource, YYKeyboardObserver, UITextFieldDelegate>
...
@implementation STIMSelectComponyViewController
...
- (UITextField *)companyTextField {
    if (!_companyTextField) {
        ...
        _companyTextField.placeholder = [NSBundle stimDB_localizedStringForKey:@"input_company_name"];
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = [UIColor stimDB_colorWithHex:0xDDDDDD];
    }
    return _lineView;
}
...
- (UITableView *)companyListView {
    if (!_companyListView) {
        ...
        _companyListView.separatorColor = [UIColor stimDB_colorWithHex:0xE4E4E4];
        _companyListView.layer.cornerRadius = 3.0f;
        _companyListView.layer.borderColor = [UIColor stimDB_colorWithHex:0xE4E4E4].CGColor;
        _companyListView.layer.borderWidth = 1.0f;
        
        _companyListView.layer.shadowColor = [UIColor stimDB_colorWithHex:0x000000 alpha:0.08].CGColor;
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = [NSBundle stimDB_localizedStringForKey:@"Select_company"];
    ...
    UIBarButtonItem *barItem = [[UIBarButtonItem alloc] initWithImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f3cd" size:20 color:[UIColor stimDB_colorWithHex:0x333333]]] style:UIBarButtonItemStylePlain target:self action:@selector(backBtnClick:)];
    [barItem setTintColor:[UIColor stimDB_colorWithHex:0x333333]];
...
- (STIMPublicCompanyModel *)getCompanyModelWithDic:(NSDictionary *)modelDic {
    STIMPublicCompanyModel *model = [STIMPublicCompanyModel yy_modelWithDictionary:modelDic];
    return model;
}
...
- (void)textFieldDidChange:(UITextField *)textField {
    ...
    if (textField.text.length > 0) {
        self.lineView.backgroundColor = [UIColor stimDB_colorWithHex:0x00CABE];
    } else {
        self.lineView.backgroundColor = [UIColor stimDB_colorWithHex:0xDDDDDD];
    }
    if (textField.text.length > 0) {
        [[STIMKit sharedInstance] getPublicNavCompanyWithKeyword:textField.text withCallBack:^(NSArray *companies) {
            dispatch_async(dispatch_get_main_queue(), ^{
                STIMVerboseLog(@"companies : %@", companies);
                [self.companies removeAllObjects];
                for (NSDictionary *companyDic in companies) {
                    if ([companyDic isKindOfClass:[NSDictionary class]]) {
                        STIMPublicCompanyModel *model = [self getCompanyModelWithDic:companyDic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMPublicCompanyModel *companyModel = [self.companies objectAtIndex:indexPath.row];
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cellId"];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cellId"];
    }
    cell.textLabel.text = (companyModel.name.length > 0) ? companyModel.name : companyModel.domain;
    cell.textLabel.textColor = [UIColor stimDB_colorWithHex:0x333333];
