...
@implementation STIMChatBgManager
...
+ (void)getChatBgById:(NSString *)userId ByName:(NSString *)name WithReset:(BOOL)reset Complete:(void(^)(UIImage *bgImage)) complete {
    NSString *fileName = [self getMD5ByData:[[NSString stringWithFormat:@"%@_%@_%@",userId,name?name:userId, [[STIMKit sharedInstance] AppBuildVersion]] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        UIColor *bgColor = stimDB_chatWaterMaskBgColor;
        UIColor *textColor = stimDB_chatWaterMaskTextColor;
