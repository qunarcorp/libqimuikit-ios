...
#import "STIMCreatePgroupVC.h"
#import "STIMGroupCardVC.h"
#import "STIMJoinGroupVC.h"
...
@interface STIMGroupListVC ()<UITableViewDataSource,UITableViewDelegate,SearchBarDelgt,UIGestureRecognizerDelegate>

@end 

@implementation STIMGroupListVC{
...
- (void)viewDidLoad {
    ...
    [bgView setImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"MoreFunctionFrame"] stretchableImageWithLeftCapWidth:10 topCapHeight:20]];
    ...
    [createGroupButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Create Group"] forState:UIControlStateNormal];
...
- (void)initWithNav{
    ...
    [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"contact_tab_group"]];
    
    UIButton *createGrouButton = [UIButton buttonWithType:UIButtonTypeCustom];
    createGrouButton.frame = CGRectMake(0, 0, 28, 28);
    [createGrouButton setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f3e1" size:24 color:[UIColor colorWithRed:33/255.0 green:33/255.0 blue:33/255.0 alpha:1/1.0]]] forState:UIControlStateNormal];
    [createGrouButton setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f3e1" size:24 color:[UIColor colorWithRed:33/255.0 green:33/255.0 blue:33/255.0 alpha:1/1.0]]] forState:UIControlStateSelected];
...
{
    NSArray *groups = [[STIMKit sharedInstance] getGroupList];
...
- (void)getGroupList {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if ([STIMKit getSTIMProjectType] != STIMProjectTypeQChat) {
            [[STIMKit sharedInstance] getIncrementMucList:0];
        } else {
            [[STIMKit sharedInstance] quickJoinAllGroup];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            _myGroupListArray = [NSMutableArray arrayWithArray:[[STIMKit sharedInstance] getMyGroupList]];
            [_tableView reloadData];
        });
    });
}
...
{
    NSArray *groups = [[STIMKit sharedInstance] getMyGroupList];
    if (groups) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [STIMGroupViewCell getCellHeightForGroupName:[info objectForKey:@"Name" ]];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_searchBarKeyTmp.text.length > 0) {
        ...
        STIMGroupViewCell *cell = (STIMGroupViewCell *)[tableView cellForRowAtIndexPath:indexPath];
        if ( [[STIMKit sharedInstance] isGroupMemberByGroupId:cell.groupID]) {
            [[STIMKit sharedInstance] clearNotReadMsgByGroupId:cell.groupID];
            [cell refresh];
            [self openGroupChatVC:cell bAddGroupSession:YES];
        }else{
            //群名片页面
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSString *text = [NSString stringWithFormat:@"%@(%d)",(section == 0) ? [NSBundle stimDB_localizedStringForKey:@"common_my_group"] : [NSBundle stimDB_localizedStringForKey:@"common_all_group"],(int)(section==0?_myGroupListArray.count:_groupListArray.count)];
    title.text      = text;
    title.font      = [UIFont fontWithName:FONT_NAME size:[[STIMCommonFont sharedInstance] currentFontSize] - 4];
...
- (void)onJoinGroup:(id)sender{
    STIMJoinGroupVC *joinVC = [[STIMJoinGroupVC alloc] init];
    [self.navigationController pushViewController:joinVC animated:YES];
    _actionMenuView.alpha = 0;
}

-(void)CreateGroup:(id)sender{
    STIMCreatePgroupVC * createGroupVC  =  [[STIMCreatePgroupVC alloc] init];
    [self.navigationController pushViewController:createGroupVC animated:YES];
    _actionMenuView.alpha = 0;
}

-(void)openGroupChatVC:(STIMGroupViewCell *)groupViewCell bAddGroupSession:(BOOL)bAddGroupSession {
    ...
    if ([self.delegate respondsToSelector:@selector(selectGroupWithJid:)]) {
        ...
        if (bAddGroupSession == YES) {
            [[STIMKit sharedInstance] openGroupSessionByGroupId:groupViewCell.groupID ByName:groupViewCell.userName];
        }
        [STIMFastEntrance openGroupChatVCByGroupId:groupViewCell.groupID];
    }
}
