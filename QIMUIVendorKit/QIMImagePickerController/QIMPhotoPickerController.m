...
#import "QIMLocationManager.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import "QIMImageRequestOperation.h"

@interface QIMPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation QIMPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (@available(iOS 9, *)) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[QIMImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[QIMImagePickerController class], nil];
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[QIMImagePickerManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(QIMAlbumModel *model) {
                self->_model = model;
            ...
            if (self->_showTakePhotoBtn || self->_isFirstAppear) {
                [[QIMImagePickerManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<QIMAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[QIMAssetCell class] forCellWithReuseIdentifier:@"QIMAssetCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [QIMCommonTools qim_isIPhoneX] ? 50 + (83 - 49) : 50;
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        ...
        CGFloat navigationHeight = naviBarHeight + [QIMCommonTools qim_statusBarHeight];
    ...
    [QIMImagePickerManager manager].columnNumber = [QIMImagePickerManager manager].columnNumber;
    [QIMImagePickerManager manager].photoWidth = tzImagePickerVc.photoWidth;
...
- (void)previewButtonClick {
    QIMPhotoPreviewController *photoPreviewVc = [[QIMPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            QIMAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            QIMAssetModel *model = tzImagePickerVc.selectedModels[i];
            QIMImageRequestOperation *operation = [[QIMImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                if (isDegraded) return;
                if (photo) {
                    if (![QIMImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[QIMImagePickerManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.maxImagesCount == 1) {
        if ([[QIMImagePickerManager manager] isVideo:[assets firstObject]]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QIMAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"QIMAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        QIMImagePickerController *tzImagePickerVc = (QIMImagePickerController *)strongSelf.navigationController;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QIMAssetModel *model = _models[index];
    if (model.type == QIMAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            QIMVideoPlayerController *videoPlayerVc = [[QIMVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == QIMAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            QIMImagePickerController *imagePickerVc = (QIMImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle qim_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            QIMGifPhotoPreviewController *gifPreviewVc = [[QIMGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    ...
    if (tzImagePickerVc.allowCameraLocation) {
        __weak typeof(self) weakSelf = self;
        [[QIMLocationManager manager] startLocationWithSuccessBlock:^(NSArray<CLLocation *> *locations) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (void)pushPhotoPrevireViewController:(QIMPhotoPreviewController *)photoPreviewVc {
    [self pushPhotoPrevireViewController:photoPreviewVc needCheckSelectedModels:NO];
}

- (void)pushPhotoPrevireViewController:(QIMPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[QIMImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.qim_width <= 320) {
        return;
    }
    QIMImagePickerController *imagePickerVc = (QIMImagePickerController *)self.navigationController;
    [[QIMImagePickerManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    QIMImagePickerController *tzImagePickerVc = (QIMImagePickerController *)self.navigationController;
    for (QIMAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[QIMImagePickerManager manager] savePhotoWithImage:photo location:self.location completion:^(PHAsset *asset, NSError *error){
                if (!error) {
        ...
        if (videoUrl) {
            [[QIMImagePickerManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                if (!error) {
                    [self addPHAsset:asset];
                }
            }];
...
- (void)addPHAsset:(PHAsset *)asset {
    QIMAssetModel *assetModel = [[QIMImagePickerManager manager] createModelWithAsset:asset];
    QIMImagePickerController *tzImagePickerVc = (QIMImagePickerController *)self.navigationController;
    [tzImagePickerVc hideProgressHUD];
    if (tzImagePickerVc.sortAscendingByModificationDate) {
    ...
    if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
        if (assetModel.type == QIMAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
            // 不能多选视频的情况下，不选中拍摄的视频
        } else {
...
- (void)resetCachedAssets {
    [[QIMImagePickerManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[QIMImagePickerManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[QIMImagePickerManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
