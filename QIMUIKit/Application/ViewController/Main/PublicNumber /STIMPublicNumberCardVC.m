...
#import "STIMPublicNumberCardCommonCell.h"

#import "STIMPublicNumberRobotVC.h"

#import "STIMQRCodeCell.h"

@interface STIMPublicNumberCardVC ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate>{
...
@implementation STIMPublicNumberCardVC
...
- (void)viewDidLoad {
    ...
    if (self.publicRobotDic) {
        ...
        [tempDic setSTIMSafeObject:[NSString stringWithFormat:@"%@@%@",self.publicNumberId,[[STIMKit sharedInstance] getDomain]] forKey:@"XmppId"];
        [tempDic setSTIMSafeObject:self.publicNumberId forKey:@"PublicNumberId"];
        [tempDic setSTIMSafeObject:name forKey:@"Name"];
        [tempDic setSTIMSafeObject:headerSrc forKey:@"HeaderSrc"];
        [tempDic setSTIMSafeObject:publicNumberInfo forKey:@"PublicNumberInfo"];
        _publicNumberCardDic = tempDic;
        [self initUI];
    } else if (self.notConcern) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            _resultList = [[STIMKit sharedInstance] updatePublicNumberCardByIds:@[@{@"robot_name":self.publicNumberId,@"version":@(-1)}] WithNeedUpdate:NO];
            dispatch_async(dispatch_get_main_queue(), ^{
                if (_resultList.count > 0) {
                    ...
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"该公众号不存在！" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
                    [alertView show];
                }
            });
        });
    } else {
        _publicNumberCardDic = [[STIMKit sharedInstance] getPublicNumberCardByJid:self.jid];
...
- (void)initWithTableView{
    ...
    [iconImageView setImage:[[STIMKit sharedInstance] getPublicNumberHeaderImageByFileName:headerSrc]];
    ...
    if (self.notConcern) {
        ...
        [button setBackgroundImage:[[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"GreenBigBtn"] stretchableImageWithLeftCapWidth:12 topCapHeight:12] forState:UIControlStateNormal];
...
- (void)onConcernClick:(UIButton *)sender{
    BOOL isSuccess = [[STIMKit sharedInstance] focusOnPublicNumberId:self.publicNumberId];
    if (isSuccess) {
        [[STIMKit sharedInstance] bulkInsertPublicNumbers:_resultList];
        [self onOpenPublicNumberClick:sender];
    } else {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"关注失败！" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
        [alertView show];
    }
}
...
//    @"举报",@"清空内容",@"推荐给朋友",
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"] destructiveButtonTitle:nil otherButtonTitles:@"不再关注", nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0) {
        BOOL isSuccess = [[STIMKit sharedInstance] cancelFocusOnPublicNumberId:self.publicNumberId];
        if (isSuccess) {
            [self.navigationController popToRootViewControllerAnimated:YES];
        } else {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"取消关注失败!" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
            [alertView show];
        }
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([value isEqualToString:@"robotDesc"]) {
        return [STIMPublicNumberCardCommonCell getCellHeightByInfo:_descStr];
    } else if ([value isEqualToString:@"fromsource"]) {
        return [STIMPublicNumberCardCommonCell getCellHeightByInfo:_fromSource];
    } else if ([value isEqualToString:@"tel"]) {
        return [STIMPublicNumberCardCommonCell getCellHeightByInfo:_telStr];
    } else if ([value isEqualToString:@"qcode"]) {
        return [STIMQRCodeCell getCellHeight];
    }
    return 20;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([value isEqualToString:@"robotDesc"]) {
        ...
        STIMPublicNumberCardCommonCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[STIMPublicNumberCardCommonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([value isEqualToString:@"qcode"]) {
        [STIMFastEntrance showQRCodeWithQRId:self.publicNumberId withType:QRCodeType_RobotQR];
    }
}
