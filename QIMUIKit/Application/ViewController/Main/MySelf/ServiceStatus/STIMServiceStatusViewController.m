//
//  STIMServiceStatusViewController.m
//  qunarChatIphone
//
...
#import "STIMServiceStatusTableViewCell.h"
#import "NSBundle+STIMLibrary.h"
@interface STIMServiceStatusViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation STIMServiceStatusViewController

- (NSArray *)serviceStatus {
    if (!_serviceStatus) {
        _serviceStatus = [[STIMKit sharedInstance] availableUserSeatStatus];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMServiceStatusTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:statusCellId];
    if (!cell) {
        cell = [[STIMServiceStatusTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:statusCellId];
    }
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    return [NSString stringWithFormat:@"%@ : %@",[NSBundle stimDB_localizedStringForKey:@"Store Name"],sname];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger shopServiceStatus = [[serviceShop objectForKey:@"st"] integerValue];
    if (status != shopServiceStatus) {
        BOOL updateShopServiceSuccess = [[STIMKit sharedInstance] updateSeatSeStatusWithShopId:shopId WithStatus:status];
        if (updateShopServiceSuccess) {
            self.serviceShops = [[STIMKit sharedInstance] getSeatSeStatus];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [STIMServiceStatusTableViewCell getCellHeight];
}

- (void)setUpNavBar {
    
    self.title = [NSBundle stimDB_localizedStringForKey:@"myself_tab_service"];
}
...
- (void)viewDidLoad {
    ...
    [self setUpNavBar];
    self.serviceShops = [[STIMKit sharedInstance] getSeatSeStatus];
    STIMVerboseLog(@"self.serviceShops : %@", self.serviceShops);
