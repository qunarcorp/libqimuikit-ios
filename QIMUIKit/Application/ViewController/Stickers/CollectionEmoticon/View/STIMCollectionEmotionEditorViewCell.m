//
//  STIMCollectionEmotionEditorViewCell.m
//  STChatIphone
//
...
@interface STIMEmotionEditorImageView : UIView

@end

@interface STIMEmotionEditorImageView ()
{
...
@implementation STIMEmotionEditorImageView

-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        UIView * bgView = [[UIView alloc] initWithFrame:CGRectMake(-1, -1, frame.size.width + 1, frame.size.height + 1)];
        bgView.backgroundColor = [UIColor whiteColor];
        bgView.layer.borderColor = [UIColor stimDB_colorWithHex:0xd6d6d4 alpha:1].CGColor;
...
- (void)setImageUrl:(NSString *)imageUrl {
    ...
    [_imageView stimDB_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:nil options:SDWebImageFromLoaderOnly progress:nil completed:nil];
}
...
- (void)setUpMaskViews{
    ...
    if (_selectFlagView == nil) {
        ...
        _selectFlagView.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"AlbumCellRedSelected"];
        _selectFlagView.hidden = YES;
...
@implementation STIMCollectionEmotionEditorViewCell
...
- (void)setUpEmotionViews {
    ...
    STIMEmotionEditorImageView *itemView = [[STIMEmotionEditorImageView alloc] initWithFrame:self.bounds];
    ...
    if ([item isEqualToString:kImageFacePageViewAddFlagName]) {
        ...
        //添加按钮
        itemImage = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"EmotionEditorAdd"];
        [itemView setImage:itemImage];
    } else {
        
        [itemView setIsAdd:NO];
        NSString *emojiUrl = [[STIMCollectionFaceManager sharedInstance] getCollectionFaceHttpUrlWithIndex:self.tag];
        ...
        /*
         Mark by SD
        [[STIMCollectionFaceManager sharedInstance] showSmallImage:^(UIImage *image) {
...
- (void)itemTapHandle:(UITapGestureRecognizer *)tap{
    ...
    STIMEmotionEditorImageView * view = (STIMEmotionEditorImageView *)tap.view;
    if (self.canSelect) {
