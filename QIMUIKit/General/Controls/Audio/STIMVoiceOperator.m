...
@interface STIMVoiceOperator ()<AVAudioRecorderDelegate> {
...
@implementation STIMVoiceOperator
...
- (void)doVoiceRecordByFilename:(NSString *)filename
{
    if (![self canRecord]) {
        [[[UIAlertView alloc] initWithTitle:nil
                                    message:[NSString stringWithFormat:@"%@需要访问您的麦克风。\n请启用麦克风-设置/隐私/麦克风", [STIMKit getSTIMProjectTitleName]]
                                   delegate:nil
                          cancelButtonTitle:@"好"
    ...
    _filePath = [STIMPathManage getPathByFileName:_fileName ofType:@"wav"];
    _recoder = [[AVAudioRecorder alloc] initWithURL:[NSURL URLWithString:_filePath]
                                           settings:[STIMVoiceOperator getAudioRecorderSettingDic]
                                              error:&error];
//    STIMVerboseLog(@"recoder path == %@",_filePath);
    if (error) {
        STIMVerboseLog(@"recoder init error: %@", error);
    }
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag
{
    ...
    if (audioDurationSeconds < MIN_TIMEOUT || !_ifSave) {
        ...
        [STIMPathManage deleteFileAtPath:_filePath];
