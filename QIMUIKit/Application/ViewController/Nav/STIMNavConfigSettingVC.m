...
@interface STIMNavConfigSettingVC()<UIAlertViewDelegate,NSURLSessionTaskDelegate>{
...
@implementation STIMNavConfigSettingVC

- (NSMutableArray *)navConfigUrls {
    if (!_navConfigUrls) {
        _navConfigUrls = [[STIMKit sharedInstance] qimNav_localNavConfigs];  
    }
...
- (void)setAddedNavDict:(NSDictionary *)navDict {
    ...
    self.navTitle = [navDict objectForKey:STIMNavNameKey];
    self.navUrl = [navDict objectForKey:STIMNavUrlKey];
    self.added = YES;
}
...
- (void)setupUI {
    ...
    label.text = [NSBundle stimDB_localizedStringForKey:@"Configure_Navigation_Manually"];
    label.font = [UIFont systemFontOfSize:14];
    label.textAlignment = NSTextAlignmentLeft;
    label.textColor = [UIColor stimDB_colorWithHex:0xBFBFBF];
    [self.view addSubview:label];
    
    _navNickNameLable = [[UILabel alloc] initWithFrame:CGRectMake(20, label.bottom + 16, self.view.width - 40, 20)];
    [_navNickNameLable setText:[NSBundle stimDB_localizedStringForKey:@"Navigation_Name"]];
    ...
    navNickNameTextBgView.layer.borderColor = [UIColor stimDB_colorWithHex:0xDDDDDD].CGColor;
    ...
    [_navNickNameTextField setPlaceholder:[NSBundle stimDB_localizedStringForKey:@"company_name"]];
    ...
    [_navAddressLabel setText:[NSBundle stimDB_localizedStringForKey:@"nav_required_domain_name"]];
    ...
    navAddressTextBgView.layer.borderColor = [UIColor stimDB_colorWithHex:0xDDDDDD].CGColor;
    ...
    if (self.navUrl) {
        ...
        [_navAddressTextField setPlaceholder:[NSBundle stimDB_localizedStringForKey:@"company_domain"]];
    }
...
//    [_qrcodeNavBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f0f5" size:20 color:[UIColor colorWithRed:97/255.0 green:97/255.0 blue:97/255.0 alpha:1/1.0]]] forState:UIControlStateNormal];
...
- (void)setupNav {
    ...
    self.title = self.navTitle.length > 0 ? self.navTitle : [NSBundle stimDB_localizedStringForKey:@"nav_new_Navigation"];
    ...
    UIBarButtonItem *saveItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_save"] style:UIBarButtonItemStylePlain target:self action:@selector(onSave)];
...
- (void)onSave{
    ...
    if (self.navUrl.length > 0) {
        ...
        __block NSDictionary *userWillsaveNavDict = @{STIMNavNameKey:(navHttpName.length > 0) ? navHttpName : [[self.navUrl.lastPathComponent componentsSeparatedByString:@"="] lastObject], STIMNavUrlKey:self.navUrl};
