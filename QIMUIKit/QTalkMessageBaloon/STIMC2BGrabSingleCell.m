//
//  STIMC2BGrabSingleCell.m
//  STChatIphone
//
//  Created by 李海彬 on 2017/10/25.
//
...
@implementation STIMC2BGrabSingleCell {
...
- (void)updateC2BGrabSingleFeedBack:(NSNotification *)notify {
    ...
    if ([msgId isEqualToString:_c2BMessage.messageId] && [notify.userInfo objectForKey:@"message"]) {
       ...
       STIMMessageModel *msg = [notify.userInfo objectForKey:@"message"];
        ...
        NSDictionary *c2BMsgDict = [[STIMJSONSerializer sharedInstance] deserializeObject:c2BExtendInfo error:nil];
...
- (void)setMessage:(STIMMessageModel *)message {
    ...
    if (message.extendInformation.length > 0) {
        grabSingleDic = [[STIMJSONSerializer sharedInstance] deserializeObject:message.extendInformation error:nil];
    } else {
        grabSingleDic = [[STIMJSONSerializer sharedInstance] deserializeObject:message.message error:nil];
    }
    ...
    NSString *getC2BMessageFeedBackStr = [[STIMKit sharedInstance] getC2BMessageFeedBackWithMsgId:message.messageId];
    NSDictionary *c2BMsgDict = nil;
    if (getC2BMessageFeedBackStr) {
        c2BMsgDict = [[STIMJSONSerializer sharedInstance] deserializeObject:getC2BMessageFeedBackStr error:nil];
        if (c2BMsgDict.count) {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _bgContentView = [[UIView alloc] initWithFrame:CGRectMake(20, 10, [UIScreen mainScreen].bounds.size.width - 40, [STIMC2BGrabSingleCell getCellHeight] - 20)];
