...
@interface STIMGroupMembersCell ()
{
    UILabel         * _titleLabel;
    UILabel         * _countLabel;
    NSArray         * _items;
...
@implementation STIMGroupMembersCell
...
- (void)layoutSubviews
{
    ...
    _titleLabel.text = [NSBundle stimDB_localizedStringForKey:@"group_member"];
    ...
    _countLabel.font = [UIFont fontWithName:FONT_NAME size:[[STIMCommonFont sharedInstance] currentFontSize]-4];
    ...
    for (NSDictionary * itemDic in _items) {
        ...
        [imageView stimDB_setImageWithJid:xmppId];
    ...
    //add
    UIImageView * imageView = [[UIImageView alloc] initWithImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"mqz_add_picture"]];
    imageView.frame = CGRectMake(startX, _titleLabel.bottom + 10, kImageWidth, kImageWidth);
    imageView.layer.cornerRadius = kImageWidth / 2.0;
