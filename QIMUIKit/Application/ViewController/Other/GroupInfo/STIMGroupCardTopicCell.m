...
@implementation STIMGroupCardTopicCell{
    UIView *_rootView;
    UILabel *_titleLabel;
...
+ (CGFloat)getCellHeightWithTopic:(NSString *)topic{
    CGSize size = [topic sizeWithFont:[UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - 6] constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 30, INT32_MAX) lineBreakMode:NSLineBreakByCharWrapping];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [_titleLabel setText:[NSBundle stimDB_localizedStringForKey:@"group_topic"]];
        ...
        _menuView = [[STIMMenuView alloc] initWithFrame:CGRectZero];
...
- (void)refreshUI{
    ...
    _groupTopicLabel.font = [UIFont boldSystemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - 6];
    CGSize size = [self.topic sizeWithFont:[UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - 6] constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 30, INT32_MAX) lineBreakMode:NSLineBreakByCharWrapping];
