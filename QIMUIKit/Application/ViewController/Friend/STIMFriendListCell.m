...
@implementation STIMFriendListCell
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView= [[UIView alloc] initWithFrame:CGRectMake(_nameLabel.left, [STIMFriendListCell getCellHeightForDesc:[self.userInfoDic objectForKey:@"DescInfo"]]-0.5, [UIScreen mainScreen].bounds.size.width-_nameLabel.left, 0.5)];
...
- (void)refreshUI{
    ...
    self.nameLabel.frame = CGRectMake(self.headerView.right + 10, 10, [UIScreen mainScreen].bounds.size.width - 10 - self.headerView.right - 10, [[STIMCommonFont sharedInstance] currentFontSize] + 2);
    self.nameLabel.font = [UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - 2];
    
    CGSize size = [_descInfo stimDB_sizeWithFontCompatible:[UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - 6] constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 10 - 50 - 10, MAXFLOAT) lineBreakMode:NSLineBreakByCharWrapping];
    ...
    self.descInfoLabel.font = [UIFont systemFontOfSize:[[STIMCommonFont sharedInstance] currentFontSize] - 6];
    
    self.lineView.frame = CGRectMake(self.nameLabel.left, [STIMFriendListCell getCellHeightForDesc:[self.userInfoDic objectForKey:@"DescInfo"]]-1, [UIScreen mainScreen].bounds.size.width-self.nameLabel.left, 0.5);
