...
@interface STIMAddSomeViewController()<UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchControllerDelegate,UISearchDisplayDelegate>{
...
@implementation STIMAddSomeViewController{
...
- (UISearchBar *)getSearchBar
{
    ...
    [searchBar sizeToFit];
    searchBar.placeholder = [NSBundle stimDB_localizedStringForKey:@"search_bar_placeholder"];
...
#pragma mark - init UI
- (void)initWithNav{
    [self.navigationItem setTitle:[NSBundle stimDB_localizedStringForKey:@"common_add"]];
}
...
- (void)searchList{
    if (self.fromQChat) {
        ...
        NSArray *searchList = [[STIMKit sharedInstance] searchQunarUserBySearchStr:searchString];
        ...
        if (searchString.length > 0) {
            searchList = [[STIMKit sharedInstance] searchUserListBySearchStr:searchString];
        }
        [_searchResults removeAllObjects];
        if (searchList) {
            [_searchResults addObjectsFromArray:searchList];
...
- (BOOL)searchDisplayController:(UISearchDisplayController *)controller shouldReloadTableForSearchString:(NSString *)searchString
{
    if (self.fromQChat) {
        if (!_justDoItLabel) {
            ...
            addFrientIcon.image = [UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"findPeople"];
...
- (void)addFriend {
    ...
    if (infoStrArr.count > 1) {
        ...
        if (self.fromQChat) {
            ...
            userXmppId = [NSString stringWithFormat:@"%@@%@",rtxID, [[STIMKit sharedInstance] getDomain]];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [STIMFastEntrance openUserCardVCByUserId:userXmppId];
        });
    }
}
