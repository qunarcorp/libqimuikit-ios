...
#define STIMProductInfoWidth 245
#define STIMProductInfoHeight 150
#define STIMProductInfoImageWidth 40

@implementation STIMProductInfoCell {
...
+ (CGFloat)getCellHeight{
    return STIMProductInfoHeight + 25;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _lineView.backgroundColor = [UIColor stimDB_colorWithHex:0x9E9E9E];
        ...
        _platFormIconView.image = [UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e189" size:15 color:[UIColor qunarBlueColor]]];
        ...
        _platFormLabel.textColor = [UIColor stimDB_colorWithHex:0x9E9E9E];
...
- (void)onClick{
    if (self.touchUrl.length > 0) {
        [STIMFastEntrance openWebViewForUrl:self.touchUrl showNavBar:YES];
    }
}
...
- (void)refreshUI{
    ...
    [self setBackViewWithWidth:STIMProductInfoWidth WithHeight:STIMProductInfoHeight];
    [_headerImageView stimDB_setImageWithURL:[NSURL URLWithString:self.headerUrl] placeholderImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"v_aroundTravel_default"]];
    ...
    CGFloat leftOffset = (self.message.messageDirection == STIMMessageDirection_Sent) ? 15 : 20;
    _titleLabel.frame = CGRectMake(leftOffset, 10, self.backView.width - leftOffset - 10, 60);
    _headerImageView.frame = CGRectMake(_titleLabel.left, _titleLabel.bottom + 5, STIMProductInfoImageWidth, STIMProductInfoImageWidth);
    ...
    _lineView.frame =  (self.message.messageDirection == STIMMessageDirection_Sent) ? CGRectMake(leftOffset - 15, self.backView.height - 18, STIMProductInfoWidth - leftOffset + 5.0f, 0.5f) : CGRectMake(leftOffset - 10, self.backView.height - 18, STIMProductInfoWidth - leftOffset + 10.0f, 0.5f);
    _platFormIconView.frame = CGRectMake(_lineView.left + 10, _lineView.bottom + 1.5f, 15, 15);
    _platFormLabel.frame = CGRectMake(_platFormIconView.right + 5, _lineView.bottom, STIMProductInfoWidth, 18);
