...
@interface STIMRemoteAudioPlayer ()<ASIProgressDelegate>
{
    BOOL _ready;
...
@implementation STIMRemoteAudioPlayer
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        if (!success) {
            STIMVerboseLog(@"AVAudioSession error setting category:%@", error);
        }
        //set the audioSession override
        success = [session overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker error:&error];
        if (!success) {
            STIMVerboseLog(@"AVAudioSession error overrideOutputAudioPort:%@", error);
        }
...
//        success = [session setActive:NO error:&error];
        if (!success)  {
            STIMVerboseLog(@"AVAudioSession error activating: %@",error);
        } else {
            STIMVerboseLog(@"audioSession active");
        }
...
//用于QTalk的语音播放。因为使用STIMPathManage类来管理音频文件的存放路径，所以播放音频时将文件名和网络url传输过来。
...
- (void)prepareForFileName:(NSString *)fileName andVoiceUrl:(NSString *)voiceUrl playAfterReady:(BOOL)playAfterReady
{
    ...
    NSString *voicePath = [STIMPathManage getPathByFileName:_fileName ofType:@"amr"];
    if ([STIMPathManage fileExistsAtPath:voicePath]) {
...
- (void)prepareForVoiceURL:(NSString *)voiceUrl playAfterReady:(BOOL)playAfterReady
{
    ...
    [self.request clearDelegatesAndCancel];
    self.request = nil;
    if (![voiceUrl stimDB_hasPrefixHttpHeader]) {
        voiceUrl =  [[STIMKit sharedInstance].qimNav_InnerFileHttpHost stringByAppendingFormat:@"/%@", voiceUrl];
    } 
    if ([voiceUrl rangeOfString:@"?"].location == NSNotFound) {
        voiceUrl = [voiceUrl stringByAppendingFormat:@"?u=%@&k=%@",[[STIMKit getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[[STIMKit sharedInstance] myRemotelogginKey]];
    } else {
        voiceUrl = [voiceUrl stringByAppendingFormat:@"&u=%@&k=%@",[[STIMKit getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[[STIMKit sharedInstance] myRemotelogginKey]];
    }
...
- (void)preparefromWaveData:(NSData *)data playAfterReady:(BOOL)playAfterReady{
    ...
    if (successful) {
        ...
        if ([UIDevice currentDevice].proximityMonitoringEnabled == YES) {
            ...
            STIMVerboseLog(@"The device does not have a proximity sensor");
        }
        if (_playAfterReady) {
            ...
            if (successful) {
                ...
                [_delegate remoteAudioPlayerErrorOccured:self withErrorCode:STIMRemoteAudioPlayerPlayingFailure];
            }
        }
    }
    else {
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    //保存数据到本地
    [STIMPathManage getPathToSaveWithSaveData:amrData ToFileName:_fileName ofType:@"amr"];
...
{
    [_delegate remoteAudioPlayerErrorOccured:self withErrorCode:STIMRemoteAudioPlayerLoadingFailure];
    [self setPlayer:nil];
}
