...
- (BOOL)enumerateRunRectContainPoint:(CGPoint)point
                        ...
                        successBlock:(void (^)(id<STIMTextStorageProtocol> textStorage))successBlock;
...
@interface STIMAttributedLabel ()<UIGestureRecognizerDelegate>
{
    struct {
...
@implementation STIMAttributedLabel
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        [self setupProperty];
        _textContainer = [[STIMTextContainer alloc]init];
    }
    return self;
}
...
- (instancetype)initWithTextContainer:(STIMTextContainer *)textContainer
{
    if (self = [super init]) {
        [self setupProperty];
...
- (void)setTextContainer:(STIMTextContainer *)attStringCreater
{
...
- (void)setDelegate:(id<STIMAttributedLabelDelegate>)delegate
{
...
- (void)clearOwnerView {
    for (id storage in self.textContainer.textStorages) {
        if ([storage respondsToSelector:@selector(setOwnerView:)]) {
            [(id<STIMViewStorageProtocol>)storage setOwnerView:nil];
        }
    }
}
...
{
    // draw storage
    [_textContainer enumerateDrawRectDictionaryUsingBlock:^(id<STIMDrawStorageProtocol> drawStorage, CGRect rect) {
        if ([drawStorage conformsToProtocol:@protocol(STIMViewStorageProtocol) ]) {
            [(id<STIMViewStorageProtocol>)drawStorage setOwnerView:self];
        }
...
- (void)singleTap:(UITapGestureRecognizer *)sender
{
    ...
    [_textContainer enumerateRunRectContainPoint:point viewHeight:CGRectGetHeight(self.frame) successBlock:^(id<STIMTextStorageProtocol> textStorage){
        if (_delegateFlags.textStorageClickedAtPoint) {
...
@implementation STIMAttributedLabel (AppendAttributedString)
...
@implementation STIMAttributedLabel (Link)
...
@implementation STIMAttributedLabel (UIImage)
...
@implementation STIMAttributedLabel (UIView)
