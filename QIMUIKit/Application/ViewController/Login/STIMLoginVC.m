...
#define kValidCodeDisplayString      [NSBundle stimDB_localizedStringForKey:@"login_get_code"]
#define kValidCodeSendingString      [NSBundle stimDB_localizedStringForKey:@"login_sending"]
...
#import "STIMActivityIndicatorView.h"
...
@interface STIMLoginVC () <UITextFieldDelegate,UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource> {
    UIView          * _loginBgView;
    UIView          * _signView;
    ...
    STIMActivityIndicatorView     * _indicatorView;
...
@implementation STIMLoginVC

- (NSMutableArray *)loginUsers {
    if (!_loginUsers) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:[[STIMKit sharedInstance] userObjectForKey:@"Users"]];
...
//            [[STIMKit sharedInstance] setUserObject:@"qtalktest123" forKey:@"kTempUserToken"];
            ...
            if ([lastUserName length] > 0 && [token length] > 0 && self.loginType == QTLoginTypeSms) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    NSString *pwd = [NSString stringWithFormat:@"%@@%@",[STIMUUIDTools deviceUUID],token];
...
- (void)loadLoginType {
    self.loginType = [[STIMKit sharedInstance] qimNav_LoginType];
    [self initUI];
}
...
- (UIView *)loginBgView {
    if (_loginBgView == nil) {
        ...
        _loginBgView.backgroundColor = [UIColor stimDB_colorWithHex:0xFFFFFF alpha:1.0];
...
- (UIView *)signView {
    if (_signView == nil) {
        ...
        UIImageView * iConImg = [[UIImageView alloc]initWithImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"loginPageLogo"]];
...
- (UIView *)userNameSepline {
    if (_userNameSepline == nil) {
        ...
        _userNameSepline.backgroundColor = [UIColor stimDB_colorWithHex:kSepLineColorHex alpha:1.0];
...
- (UILabel *)userNamePlaceholder {
    if (_userNamePlaceholder == nil) {
        ...
        _userNamePlaceholder.textColor = [UIColor stimDB_colorWithHex:kPlaceholderColorHex alpha:1.0];
        _userNamePlaceholder.font = [UIFont systemFontOfSize:kNomalFontSize];
        _userNamePlaceholder.text = [NSBundle stimDB_localizedStringForKey:@"Login_name"];
...
- (UITextField *)userNameInputView {
    if (_userNameInputView == nil) {
        ...
        if (self.loginUsers.count > 0) {
            ...
            [_userNameDropBtn setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"chat_bottom_arrowdown_nor"] forState:UIControlStateNormal];
            [_userNameDropBtn setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"chat_bottom_arrowup_nor"] forState:UIControlStateSelected];
        ...
        _userNameInputView.textColor = [UIColor stimDB_colorWithHex:kNomalInputColorHex alpha:1.0];
        _userNameInputView.font = [UIFont systemFontOfSize:kNomalFontSize];
        _userNameInputView.delegate = self;
        _userNameInputView.tintColor = [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0];
...
- (UIView *)validCodeSepline {
    if (_validCodeSepline == nil) {
        ...
        _validCodeSepline.backgroundColor = [UIColor stimDB_colorWithHex:kSepLineColorHex alpha:1.0];
...
- (UILabel *)validCodePlaceholder {
    if (_validCodePlaceholder == nil) {
        ...
        _validCodePlaceholder.textColor = [UIColor stimDB_colorWithHex:kPlaceholderColorHex alpha:1.0];
        _validCodePlaceholder.font = [UIFont systemFontOfSize:kNomalFontSize];
        [_loginBgView addSubview:_validCodePlaceholder];
    }
    if (self.loginType == QTLoginTypePwd || self.loginType == QTLoginTypeNewPwd) {
        _validCodePlaceholder.text = [NSBundle stimDB_localizedStringForKey:@"Login_password"];
    } else {
        _validCodePlaceholder.text = [NSBundle stimDB_localizedStringForKey:@"login_code"];
    }
...
- (UITextField *)validCodeInputView {
    if (_validCodeInputView == nil) {
        ...
        _validCodeInputView.textColor = [UIColor stimDB_colorWithHex:kNomalInputColorHex alpha:1.0];
        _validCodeInputView.font = [UIFont systemFontOfSize:kNomalFontSize];
        _validCodeInputView.delegate = self;
        _validCodeInputView.tintColor = [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0];
...
- (UILabel *)commitBtn {
    if (_commitBtn == nil) {
        ...
        _commitBtn.backgroundColor = [UIColor stimDB_colorWithHex:0xABE9E5];
        _commitBtn.layer.cornerRadius = 24;
        _commitBtn.clipsToBounds = YES;
        _commitBtn.textColor = [UIColor stimDB_colorWithHex:0xFFFFFF];
        _commitBtn.textAlignment = NSTextAlignmentCenter;
        _commitBtn.font = [UIFont boldSystemFontOfSize:17];
        _commitBtn.text = [NSBundle stimDB_localizedStringForKey:@"login"];
...
- (UIView *)highlightedSepline {
    if (_highlightedSepline == nil) {
        ...
        _highlightedSepline.backgroundColor = [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0];
    }
...
- (UILabel *)getValidCodeBtn {
    if (_getValidCodeBtn == nil) {
        ...
        _getValidCodeBtn.backgroundColor = [UIColor clearColor];
//        _getValidCodeBtn.layer.borderColor = [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0].CGColor;
...
- (STIMActivityIndicatorView *)indicatorView {
    if (_indicatorView == nil) {
        _indicatorView = [[STIMActivityIndicatorView alloc] initWithTintColor:[UIColor whiteColor] size:[self commitBtn].height - 4];
        _indicatorView.frame = [self commitBtn].bounds;
        _indicatorView.backgroundColor = [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0];
...
- (UIButton *)agreeBtn {
    if (_agreeBtn == nil) {
        ...
        [_agreeBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e337" size:17 color:[UIColor stimDB_colorWithHex:0xE4E4E4]]] forState:UIControlStateNormal];
        [_agreeBtn setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000e337" size:17 color:[UIColor stimDB_colorWithHex:0x00CABE]]] forState:UIControlStateSelected];
        ...
        NSString *login_agree = [NSBundle stimDB_localizedStringForKey:@"login_agree"];
        ...
        agreeLabel.textColor = [UIColor stimDB_colorWithHex:0x999999];
        ...
        [agreementBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"login_privacy_policy"] forState:UIControlStateNormal];
        ...
        [agreementBtn setTitleColor:[UIColor stimDB_colorWithHex:0x999999] forState:UIControlStateNormal];
        [agreementBtn setTitleColor:[UIColor stimDB_colorWithHex:0x999999] forState:UIControlStateHighlighted];
...
- (UIButton *)settingBtn {
    if (_settingBtn == nil) {
        ...
        [_settingBtn setTitleColor:[UIColor stimDB_colorWithHex:0x00CABE] forState:UIControlStateNormal];
...
//        if (inputView.text.length == 0) {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                ...
                if (inputView == _userNameInputView) {
                    inputView.placeholder = [NSBundle stimDB_localizedStringForKey:@"login_userNameInputView_placeholder"];
                }
            }];
//        }
...
- (void)setValidCodeBtnEnabled:(BOOL)enabled {
    ...
    _getValidCodeBtn.layer.borderColor = enabled ? [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0].CGColor : [UIColor stimDB_colorWithHex:kPlaceholderColorHex alpha:1.0].CGColor;
    _getValidCodeBtn.textColor = enabled ? [UIColor stimDB_colorWithHex:kHighlightedColorHex alpha:1.0] : [UIColor stimDB_colorWithHex:kPlaceholderColorHex alpha:1.0];
}
...
- (void)commitTapHandle:(UITapGestureRecognizer *)sender {
    ...
    if (!_userNameInputView.text.length) {
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"] message:[NSBundle stimDB_localizedStringForKey:@"common_input_username"] delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_got_it"] otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
    if (!_validCodeInputView.text.length) {
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"] message:[NSBundle stimDB_localizedStringForKey:@"login_input_code"] delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_got_it"] otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
...
#warning 报错即将登陆的用户名 并请求导航
    ...
    [[STIMKit sharedInstance] qimNav_updateNavigationConfigWithCheck:YES];
...
//        [[STIMKit sharedInstance] setUserObject:@"qtalktest123" forKey:@"kTempUserToken"];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            if (self.loginType == QTLoginTypeSms) {
                __block NSString *token = [[STIMKit sharedInstance] getLastUserToken];
//                [[STIMKit sharedInstance] userObjectForKey:@"userToken"];
                if (token.length <= 0) {
                    [[STIMKit sharedInstance] getUserTokenWithUserName:userName WithVerifyCode:validCode withCallback:^(NSDictionary *tokenDic) {
                        ...
                        if (statusId == 0) {
                            ...
                            [[STIMKit sharedInstance] updateLastTempUserToken:token];
//                            [[STIMKit sharedInstance] setUserObject:token forKey:@"kTempUserToken"];
                            NSString *pwd = [NSString stringWithFormat:@"%@@%@",[STIMUUIDTools deviceUUID],token];
                            ...
                            dispatch_async(dispatch_get_main_queue(), ^{
                                ...
                                if (error) {
                                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"]
                                                                                        message:error ? error : @"发生未知错误"
                                                                                       delegate:nil
                                                                              cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_got_it"]
...
//                    [[STIMKit sharedInstance] setUserObject:token forKey:@"kTempUserToken"];
                ...
                [[STIMKit sharedInstance] getNewUserTokenWithUserName:userName WithPassword:validCode withCallback:^(NSDictionary *result) {
                    if (result) {
...
//                                [[STIMKit sharedInstance] setUserObject:newToken forKey:@"kTempUserToken"];
                ...
                [[STIMKit sharedInstance] updateLastTempUserToken:validCode];
//                [[STIMKit sharedInstance] setUserObject:validCode forKey:@"kTempUserToken"];
                [[STIMKit sharedInstance] loginWithUserName:userName WithPassWord:validCode];
            }
        });
    }
}
...
- (void)getValidCodeTapHandle:(UITapGestureRecognizer *)sender {
    if (_userNameInputView.text.length <= 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:[NSBundle stimDB_localizedStringForKey:@"common_input_username"] delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"common_got_it"] otherButtonTitles:nil];
        [alertView show];
        return;
    }
    [_getValidCodeBtn setUserInteractionEnabled:NO];
    ...
    __weak id weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[STIMKit sharedInstance] getVerifyCodeWithUserName:userName withCallback:^(NSDictionary *result) {
            if (!result) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakSelf showNetWorkUnableAlert];
                    [_getValidCodeBtn setUserInteractionEnabled:YES];
        ...
        /*
        NSDictionary *dic = [STIMKit getVerifyCodeWithUserName:userName];
        STIMVerboseLog(@"登录用户名：%@，验证Dict：%@", userName, dic);
        if (!dic) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
//                [alertView show];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (statusId == 501 && [[userName lowercaseString] isEqualToString:@"appstore"]) {
                    ...
                    _waiting = 59;
                    [self performSelector:@selector(updateIndentifyText) withObject:nil afterDelay:1];
                    [[STIMKit sharedInstance] removeUserObjectForKey:@"userToken"];
                    [[STIMKit sharedInstance] removeUserObjectForKey:@"kTempUserToken"];
                }
            });
        }
        */
...
- (void)agreeBtnHandle:(UIButton *)sender
{
    ...
    if (sender.selected == NO) {
        UIAlertView * alert = [[UIAlertView alloc]initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"亲，请查看并同意\n《使用条款和隐私政策》\n才可以登录哟～" delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil, nil];
        [alert show];
    }
}
...
- (void)loginNotify:(NSNotification *)notify{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([notify.object boolValue]) {
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_prompt"] message:[NSBundle stimDB_localizedStringForKey:@"login_faild"] preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *action = [UIAlertAction actionWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_ok"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
//    [cell.imageView setImage:[[STIMKit sharedInstance] getUserHeaderImageByUserId:userFullJid]];
    [cell.imageView stimDB_setImageWithJid:userFullJid];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *userFullJid = [accountDict objectForKey:@"userFullJid"];
    NSString *userId = [[userFullJid componentsSeparatedByString:@"@"] firstObject];
    [[STIMKit sharedInstance] setUserObject:userId forKey:@"currentLoginUserName"];
