//
//  STIMEmotionsDownloadViewController.m
//  qunarChatIphone
//
...
#import "STIMEmotionsDownloadCell.h"
...
static NSString *cellID = @"STIMEmotionsDownloadCell";

@interface STIMEmotionsDownloadViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation STIMEmotionsDownloadViewController
...
- (NSMutableArray *)dataList {
    ...
    if (!_dataList) {
        ...
        _dataList = [NSMutableArray arrayWithArray:[[STIMEmotionManager sharedInstance] getHttpEmotions]];
        
        [[STIMEmotionSave sharedInstance] saveEmotionDownloadData:_dataList];
    }
    return _dataList;
}
...
- (UITableView *)mainTableView {
    ...
    if (!_mainTableView) {
        ...
        [_mainTableView registerClass:[STIMEmotionsDownloadCell class] forCellReuseIdentifier:cellID];
...
- (UIButton *)managerBtn {
    ...
    if (!_managerBtn) {
        ...
        [_managerBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"My_Stickers"] forState:UIControlStateNormal];
...
- (void)setUpNav {
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem createBarButtonItemWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_close"] imageName:nil target:self action:@selector(quitItemHandle:)];
...
- (void)myEmotionsHandel:(id)sender {
    ...
    STIMMyEmotionsManagerViewController *myEmotionVC = [[STIMMyEmotionsManagerViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMEmotionsDownloadCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    STIMEmotion *emotion = [[STIMEmotion alloc] initWithDict:self.dataList[indexPath.row]];
    cell.emotion = emotion;
    
    if ([[[STIMEmotionManager sharedInstance] getEmotionPackageIdList] containsObject:emotion.pkgid]) {
