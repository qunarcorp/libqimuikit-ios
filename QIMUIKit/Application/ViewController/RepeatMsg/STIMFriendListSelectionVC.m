...
@interface STIMFriendListSelectionVC ()<UITableViewDataSource,UITableViewDelegate,SearchBarDelgt,UIGestureRecognizerDelegate>

@end

@implementation STIMFriendListSelectionVC{
...
- (void)updateFriendDataSource{
    ...
    NSArray *temp = [[STIMKit sharedInstance] selectFriendList];
    if (temp) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [STIMFriendListCell getCellHeightForDesc:[info  objectForKey:@"DescInfo"]];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *cellIdentifier2 = @"GROUPCONTACT_LIST";
    if (_searchBarKeyTmp.text.length > 0) {
        STIMFriendListCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier2];
        if (cell == nil) {
            cell = [[STIMFriendListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier2];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    STIMFriendListCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (cell) {
