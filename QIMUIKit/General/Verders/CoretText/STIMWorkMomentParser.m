...
@implementation STIMWorkMomentParser

+ (instancetype) sharedInstance {
    static STIMWorkMomentParser * parser = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        parser = [[STIMWorkMomentParser alloc] init];
    });
    return parser;
}
...
+ (NSArray *)storagesWithContent:(NSString *)content WithMsgId:(NSString *)msgId WithDirection:(STIMMessageDirection)direction {
    ...
    for (NSTextCheckingResult *match in arrayOfAllMatches)
    {
        ...
        if ([type hasPrefix:@"image"]) {
            ...
            if (replyStr.length) {
                UIColor *replyColor = [UIColor stimDB_colorWithHex:0x999999];
            ...
            if (topMoment.length) {
                UIColor *topColor = [UIColor stimDB_colorWithHex:0x389CFE];
          ...
          //置热
            ...
            if (hotMoment.length) {
                UIColor *hotColor = [UIColor stimDB_colorWithHex:0xFF6916];
            ...
            NSString * url  = value;
            if (url.length) {
                UIColor *linkColor = [UIColor stimDB_colorWithHex:0x336cd5];
...
+ (NSArray *)getStoragesForTextString:(NSString *)tStr msgDirection:(STIMMessageDirection) direction {
    ...
    UIColor * textColor = [UIColor stimDB_colorWithHex:0x333333];
    NSString *content = @"";
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        if ([match resultType] == NSTextCheckingTypeLink) {
            ...
            if (urlRange.location + urlRange.length <= tStr.length && urlRange.length + urlRange.location > 0) {
                UIColor *linkTextColor = [UIColor stimDB_colorWithHex:0x336cd5];
