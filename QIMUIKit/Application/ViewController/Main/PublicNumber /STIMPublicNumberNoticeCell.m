...
@interface STIMPublicNumberNoticeCell ()

@property (nonatomic, assign) CGFloat cellWidth;
@property (nonatomic, strong) STIMMenuImageView *bgView;
...
@implementation STIMPublicNumberNoticeCell

- (CGFloat)cellWidth {
    if ([[STIMKit sharedInstance] getIsIpad]) {
        _cellWidth = [[UIScreen mainScreen] stimDB_rightWidth];
    } else {
...
- (STIMMenuImageView *)bgView {
    if (!_bgView) {
        _bgView = [[STIMMenuImageView alloc] initWithFrame:CGRectMake(kBackgroundCap, kCellCap, self.cellWidth - kBackgroundCap * 2, 0)];
        [_bgView setUserInteractionEnabled:YES];
        [_bgView setBackgroundColor:[UIColor stimDB_colorWithHex:0xFEFEFE]];
...
- (UILabel *)introduceLabel {
    if (!_introduceLabel) {
        ...
        [_introduceLabel setTextColor:[UIColor stimDB_colorWithHex:0x888888]];
...
- (UILabel *)readedAllLabel {
    if (!_readedAllLabel) {
        ...
        [_readedAllLabel setTextColor:[UIColor stimDB_colorWithHex:0x00CABE]];
...
- (UIImageView *)arrowImageView {
    if (!_arrowImageView) {
        _arrowImageView = [[UIImageView alloc] initWithImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"Arrow"]];
    }
...
- (void)refreshUI{
    ...
    NSDictionary *dic = [[STIMJSONSerializer sharedInstance] deserializeObject:self.content error:nil];
    NSString *title = [dic objectForKey:@"title"];
    NSString *introduce = [dic objectForKey:@"content"];
    NSString *linkUrl = [dic objectForKey:@"linkurl"];
