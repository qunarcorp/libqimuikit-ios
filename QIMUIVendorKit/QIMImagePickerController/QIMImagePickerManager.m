...
@interface QIMImagePickerManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation QIMImagePickerManager

CGSize AssetGridThumbnailSize;
CGFloat QIMScreenWidth;
CGFloat QIMScreenScale;

static QIMImagePickerManager *manager;
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<QIMAssetModel *> *))completion {
    QIMImagePickerConfig *config = [QIMImagePickerConfig sharedInstance];
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<QIMAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        QIMAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(QIMAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (QIMAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    QIMAssetModel *model;
    QIMAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == QIMAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == QIMAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == QIMAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == QIMAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [QIMAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (QIMAssetModelMediaType)getAssetType:(PHAsset *)asset {
    QIMAssetModelMediaType type = QIMAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = QIMAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = QIMAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = QIMAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = QIMAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        QIMAssetModel *model = photos[i];
        PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
        options.resizeMode = PHImageRequestOptionsResizeModeFast;
        options.networkAccessAllowed = YES;
        if (model.type == QIMAssetModelMediaTypePhotoGif) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != QIMAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPostImageWithAlbumModel:(QIMAlbumModel *)model completion:(void (^)(UIImage *))completion {
    ...
    return [[QIMImagePickerManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([QIMImagePickerConfig sharedInstance].needFixComposition) {
...
- (QIMAssetModel *)createModelWithAsset:(PHAsset *)asset {
    QIMAssetModelMediaType type = [[QIMImagePickerManager manager] getAssetType:asset];
    NSString *timeLength = type == QIMAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[QIMImagePickerManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    QIMAssetModel *model = [QIMAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//    return [NSNumber numberWithBool:![QIMImagePickerManager manager].sortAscendingByModificationDate];
//}
//
//@end
