...
@protocol STIMEmotionSpiritsProtocol <NSObject>
@optional
- (CGRect)getCellBackViewFrame;
@end

static STIMEmotionSpirits *__emotion_spirits = nil;
...
@implementation STIMEmotionSpirits{
...
+ (STIMEmotionSpirits *)sharedInstance{
    if (__emotion_spirits == nil) {
        __emotion_spirits = [[STIMEmotionSpirits alloc] init];
    }
    ...
    dispatch_async(_play_emotion, ^{
        for (NSString *key in _dictionary.allKeys) {
            if ([message rangeOfString:key].location != NSNotFound) {
                ...
                NSString * imageName  = [[STIMEmotionManager sharedInstance] getEmotionImagePathForShortCut:value withPackageId:@"EmojiOne"];
...
//                    [_emotionSpiritsList addObject:[STIMEmotionSpirits imageFromText:value width:16 height:16]];
                }
            }
        }
...
- (void)playAnimation:(NSDictionary*)animation{
    ...
    for (int row = (int)indexPath.row + 1 ; row < self.dataCount ; row ++) {
        ...
        if (![cell isKindOfClass:[STIMSingleChatTimestampCell class]] && [cell respondsToSelector:@selector(getCellBackViewFrame)]) {
