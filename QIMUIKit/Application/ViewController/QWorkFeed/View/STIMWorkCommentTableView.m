...
@interface STIMWorkCommentTableView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation STIMWorkCommentTableView
...
- (UITableView *)commentTableView {
    if (!_commentTableView) {
        ...
        _commentTableView.backgroundColor = [UIColor stimDB_colorWithHex:0xf8f8f8 alpha:1.0];
...
- (void)reloadUploadCommentWithModel:(STIMWorkCommentModel *)commentModel {
    ...
    for (NSInteger i = 0; i < self.hotCommentModels.count; i++) {
        STIMWorkCommentModel *hotCommentModel = [self.hotCommentModels objectAtIndex:i];
    ...
    for (NSInteger i = 0; i < self.commentModels.count; i++) {
        STIMWorkCommentModel *normalCommentModel = [self.commentModels objectAtIndex:i];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (isHotCommentModel == NO) {
            ...
            if (isNormalCommentModel == YES) {
                //评论的是普通评论
                STIMVerboseLog(@"评论的是普通评论 : %@", commentModel);
...
- (void)removeCommentWithIndexPath:(NSIndexPath *)indexPath withIsHotComment:(BOOL)isHotComment withSuperStatus:(NSInteger)superParentStatus {
    ...
    if (superParentStatus == 2) {
        ...
        //更新主评论为“该评论已被删除”
        if (isHotComment) {
            STIMWorkCommentModel *commentModel = [self.hotCommentModels objectAtIndex:indexPath.row];
            ...
            STIMWorkCommentModel *commentModel = [self.commentModels objectAtIndex:indexPath.row];
            commentModel.isDelete = YES;
            [self.commentModels replaceObjectAtIndex:indexPath.row withObject:commentModel];
            [self.commentTableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    STIMWorkCommentModel *commentModel = nil;
    if (self.hotCommentModels.count > 0) {
    ...
    STIMWorkCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[STIMWorkCommentCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:cellId];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMVerboseLog(@"self.selectedIndexPath : %d", self.selectedIndexPath);
    STIMWorkCommentModel *commentModel = nil;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    titleLabel.textColor = [UIColor stimDB_colorWithHex:0x333333];
    titleLabel.font = [UIFont boldSystemFontOfSize:19];
    
    if (self.hotCommentModels.count > 0) {
        if (section == 0) {
            [titleLabel setText:[NSBundle stimDB_localizedStringForKey:@"moment_Hot_Comments"]];
        } else {
            if (self.commentNum > 0 && self.commentModels.count) {
                NSString *commentNumStr = [NSString stringWithFormat:@"（%ld）", self.commentNum];
                NSString *titleText = [NSString stringWithFormat:[NSBundle stimDB_localizedStringForKey:@"moment_comment"]];
                NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:titleText];
                [attributedText setAttributes:@{NSForegroundColorAttributeName:[UIColor stimDB_colorWithHex:0x999999], NSFontAttributeName:[UIFont systemFontOfSize:15]}
