...
@interface STIMMessageManagerFaceView () {
    STIMFaceView * _faceView;
}
...
@implementation STIMMessageManagerFaceView
...
- (void)initUI {
    ...
    _faceView = [STIMFaceView FaceViewWithFrame:CGRectMake(0, 0.0f, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - FaceSectionBarHeight - FacePageControlHeight) WithShowAll:self.showAll WithPKId:self.packageId];
    ...
    self.pageControl.pageIndicatorTintColor = [UIColor stimDB_colorWithHex:0xD8D8D8];
    self.pageControl.currentPageIndicatorTintColor = [UIColor stimDB_colorWithHex:0x84AEBF];
