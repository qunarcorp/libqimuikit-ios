//
//  STIMChatBubbleFontChangeViewController.m
//  qunarChatIphone
//
...
@interface STIMChatBubbleFontChangeViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@interface STIMChatBubbleFontChangeViewController ()<UIAlertViewDelegate>

@end

@implementation STIMChatBubbleFontChangeViewController
...
- (void)viewDidLoad {
    ...
    NSDictionary *dic = [[STIMKit sharedInstance] userObjectForKey:kChatColorInfo];
    if (dic) {
    ...
    if (_chatColorInfo.count == 0) {
        ...
        [infoDic setSTIMSafeObject:@{@"colorHex":@(0x3cc97c),@"alpha":@(1.0)} forKey:kMyBubbleColor];
        [infoDic setSTIMSafeObject:@{@"colorHex":@(0xffffff),@"alpha":@(1.0)} forKey:kMyFontColor];
        [infoDic setSTIMSafeObject:@{@"colorHex":@(0xffffff),@"alpha":@(1.0)} forKey:kOtherBubbleColor];
        [infoDic setSTIMSafeObject:@{@"colorHex":@(0x000000),@"alpha":@(1.0)} forKey:kOtherFontColor];
...
- (void)initNavBar{
    ...
    self.navigationItem.title = [NSBundle stimDB_localizedStringForKey:@"custom_color_title"];
}
...
- (void)initSettingView{
    if (_settingView == nil) {
        ...
        [_otherBubbleColorBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"custom_target_bubble_color"]
        ...
        [_otherFontColorBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"custom_target_font_color"]
                            forState:UIControlStateNormal];
        ...
        [_myBubbleColorBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"custom_myself_bubble_color"]
                           forState:UIControlStateNormal];
        ...
        [_myFontColorBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"custom_myself_font_color"]
                         forState:UIControlStateNormal];
        ...
        [_restoreColorsBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"custom_back_to_normal"]
                           forState:UIControlStateNormal];
    ...
    [_otherBubbleColorBtn setTitleColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kOtherFontColor][@"colorHex"] integerValue] alpha:alpha] forState:UIControlStateNormal];
    [_otherBubbleColorBtn setBackgroundColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kOtherBubbleColor][@"colorHex"] integerValue] alpha:[_chatColorInfo[kOtherBubbleColor][@"alpha"] floatValue]]];
    [_otherFontColorBtn setTitleColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kOtherFontColor][@"colorHex"] integerValue] alpha:alpha] forState:UIControlStateNormal];
    [_otherFontColorBtn setBackgroundColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kOtherBubbleColor][@"colorHex"] integerValue] alpha:[_chatColorInfo[kOtherBubbleColor][@"alpha"] floatValue]]];
    alpha = [_chatColorInfo[kMyFontColor][@"alpha"] floatValue];
    if (alpha == 0) {
        alpha = 0.05;
    }
    [_myBubbleColorBtn setTitleColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kMyFontColor][@"colorHex"] integerValue] alpha:alpha] forState:UIControlStateNormal];
    [_myBubbleColorBtn setBackgroundColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kMyBubbleColor][@"colorHex"] integerValue] alpha:[_chatColorInfo[kMyBubbleColor][@"alpha"] floatValue]]];
    [_myFontColorBtn setTitleColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kMyFontColor][@"colorHex"] integerValue] alpha:alpha] forState:UIControlStateNormal];
    [_myFontColorBtn setBackgroundColor:[UIColor stimDB_colorWithHex:[_chatColorInfo[kMyBubbleColor][@"colorHex"] integerValue] alpha:[_chatColorInfo[kMyBubbleColor][@"alpha"] floatValue]]];
}
...
- (void)initPlaceholderMsgs{
    ...
    msg.message = [NSBundle stimDB_localizedStringForKey:@"custom_bubble_preview"];
...
//    msg.nickName = [STIMKit getLastUserName];
    ...
    msg1.message = [NSBundle stimDB_localizedStringForKey:@"Custom_Color_Choose_Btn"];
...
//    msg1.nickName = [NSBundle stimDB_localizedStringForKey:@"qtalk_team"];
    ...
    msg2.message = [NSBundle stimDB_localizedStringForKey:@"Custom_Color_FeedBack"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    if (_isSaved == NO) {
        [[STIMKit sharedInstance] setUserObject:_lastChatColorInfo forKey:kChatColorInfo];
    }
    if (_isValueChange) {
        NSString *infoDicStr = [[STIMJSONSerializer sharedInstance] serializeObject:_chatColorInfo];
        [[STIMKit sharedInstance] updateRemoteClientConfigWithType:STIMClientConfigTypeKChatColorInfo WithSubKey:[[STIMKit sharedInstance] getLastJid] WithConfigValue:infoDicStr WithDel:NO];
...
- (void) changeBtnHandle:(UIButton *)sender{
    ...
    if (_picker == nil) {
        ...
        [RGBInputBtn setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"rgb_input"] forState:UIControlStateNormal];
...
//    KZColorPicker * cp = [self.view viewWithTag:kColorSelectViewTag];
    ...
    switch (_changeBtnType) {
        ...
        {
            [_chatColorInfo setSTIMSafeObject:@{@"colorHex":@(colorHex),@"alpha":@(sctAlpha)} forKey:kMyBubbleColor];
            
        }
            break;
        case ChangeBtnTypeMyFont:
        {
            [_chatColorInfo setSTIMSafeObject:@{@"colorHex":@(colorHex),@"alpha":@(sctAlpha)} forKey:kMyFontColor];
        }
            break;
        case ChangeBtnTypeOtherBubble:
        {
            [_chatColorInfo setSTIMSafeObject:@{@"colorHex":@(colorHex),@"alpha":@(sctAlpha)} forKey:kOtherBubbleColor];
        }
            break;
        case ChangeBtnTypeOtherFont:
        {
            [_chatColorInfo setSTIMSafeObject:@{@"colorHex":@(colorHex),@"alpha":@(sctAlpha)} forKey:kOtherFontColor];
        }
            break;
        default:
            break;
    }
    [[STIMKit sharedInstance] setUserObject:_chatColorInfo forKey:kChatColorInfo];
...
- (void)refresh{
    [self initSettingView];
    for (STIMMessageModel * msg in _placeholderMsgs) {
...
- (void)restoreChatColorInfo{
    NSMutableDictionary * infoDic = [NSMutableDictionary dictionaryWithCapacity:1];
    [infoDic setSTIMSafeObject:@{@"colorHex":@(0x568bdf),@"alpha":@(1.0)} forKey:kMyBubbleColor];
    [infoDic setSTIMSafeObject:@{@"colorHex":@(0xffffff),@"alpha":@(1.0)} forKey:kMyFontColor];
    [infoDic setSTIMSafeObject:@{@"colorHex":@(0xb2b2b2),@"alpha":@(1.0)} forKey:kOtherBubbleColor];
    [infoDic setSTIMSafeObject:@{@"colorHex":@(0x000000),@"alpha":@(1.0)} forKey:kOtherFontColor];
    _chatColorInfo = [NSMutableDictionary dictionaryWithDictionary:infoDic];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
   STIMMessageModel * message = [_placeholderMsgs objectAtIndex:indexPath.row];
...
- (void)showRGBColorColorPopView{
    _RgbPopView = [[STIMSTAlertView alloc] initWithTitle:@"请输入RGB颜色值"
                               message:@"RGB颜色值为6位16进制正整数"
                         textFieldHint:@"请输入RGB颜色值,如0xFFFFFF"
                        textFieldValue:nil
                     cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Back"]
                     otherButtonTitles:[NSBundle stimDB_localizedStringForKey:@"Confirm"]
                     ...
                     cancelButtonBlock:^{
                         ...
                         if (colorHex.length && [self isValidateColorHex:colorHex]) {
                             ...
                             _picker.selectedColor = [UIColor stimDB_colorWithHex:mac1 alpha:1.0];
                             _picker.oldColor = [UIColor stimDB_colorWithHex:mac1 alpha:1.0];
                         }else{
                             _RgbPopView = [[STIMSTAlertView alloc] initWithTitle:@"颜色设置出错"
