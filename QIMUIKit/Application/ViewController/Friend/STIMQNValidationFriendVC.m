...
@interface STIMQNValidationFriendVC ()<UITableViewDataSource,UITableViewDelegate>

@end

@implementation STIMQNValidationFriendVC{
...
- (void)onDoneClick{
    if (_answerTextField.text.length <= 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"请输入答案内容。" delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] otherButtonTitles:nil];
        [alertView show];
        return;
    }
    [[STIMKit sharedInstance] addFriendPresenceWithXmppId:self.xmppId WithAnswer:_answerTextField.text];
}

- (void)initWithNav{
    [self.navigationItem setTitle:@"好友验证"];
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_send"] style:UIBarButtonItemStyleDone target:self action:@selector(onDoneClick)];
