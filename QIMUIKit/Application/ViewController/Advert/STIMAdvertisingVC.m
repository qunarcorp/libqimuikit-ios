...
@property (nonatomic, weak) STIMAdvertisingVC *owner;
@end

@interface STIMAdvertisingVC ()<UIWebViewDelegate,UIScrollViewDelegate,UIGestureRecognizerDelegate>{
    UIScrollView *_scrollView;
    UIPageControl *_pageControl;
//    UIWebView *_webView;
    STIMImageView *_adImageView;
    UIButton *_loadingButton;
    int _currentLoadingTime;
    STIMAdvertItem *_currentAdvertItem;
    int _prepPageIndex;
...
@implementation STIMAdvertisingVC
...
- (void)onOpenWebView:(STIMTapGestureRecognizer *)tap {
...
- (void)onLoadUrl:(STIMTapGestureRecognizer *)tap{
...
- (void)onCloseClick{
    ...
    [[STIMKit sharedInstance] qimNav_clearAdvertSource];
    ...
    if ([[[UIApplication sharedApplication] delegate] respondsToSelector:sel]) {
        [[[STIMAppWindowManager sharedInstance] advertWindow] setHidden:YES];
        [[STIMAppWindowManager sharedInstance] setAdvertWindow:nil];
        [[[STIMAppWindowManager sharedInstance] advertWindow] resignKeyWindow];
    }
}
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    if ([[STIMKit sharedInstance] qimNav_AdCarousel] == NO && [[[request URL] description] isEqualToString:[_currentAdvertItem adLinkUrl]] == NO) {
        [self cancelLoadingState];
    } else if ([[STIMKit sharedInstance] qimNav_AdCarousel] && [[STIMKit sharedInstance] qimNav_AdItems].count > 1) {
        if (_pageControl.currentPage >=0 && _pageControl
            .currentPage < [[[STIMKit sharedInstance] qimNav_AdItems] count]) {
            STIMAdvertItem *adItem = [[STIMKit sharedInstance] qimNav_AdItems][_pageControl.currentPage];
...
- (void)onLoadingButtonClick{
    if ([[STIMKit sharedInstance] qimNav_AdAllowSkip]) {
        [self onCloseClick];
    } else {
        NSString *skipTips = [[STIMKit sharedInstance] qimNav_AdSkipTips];
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:skipTips delegate:nil cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"OK"] otherButtonTitles:nil];
        [alertView show];
    }
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    if ([[STIMKit sharedInstance] qimNav_AdItems].count > 1 && [[STIMKit sharedInstance] qimNav_AdCarouselDelay] > 0) {
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(carouselAdvert) object:nil];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if ([[STIMKit sharedInstance] qimNav_AdItems].count > 1 && [[STIMKit sharedInstance] qimNav_AdCarouselDelay] > 0) {
        [self performSelector:@selector(carouselAdvert) withObject:nil afterDelay:[[STIMKit sharedInstance] qimNav_AdCarouselDelay]];
    }
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
    if ([[STIMKit sharedInstance] qimNav_AdItems].count > 1 && [[STIMKit sharedInstance] qimNav_AdCarouselDelay] > 0 && decelerate) {
        [self performSelector:@selector(carouselAdvert) withObject:nil afterDelay:[[STIMKit sharedInstance] qimNav_AdCarouselDelay]];
    }
}
// 滚动显示
- (void)initCarouselUI{
    ...
    [_scrollView setPagingEnabled:[[STIMKit sharedInstance] qimNav_AdItems].count > 0];
    [self.view addSubview:_scrollView];
    int index = 1;
    for (STIMAdvertItem *adItem in [[STIMKit sharedInstance] qimNav_AdItems]) {
        switch ([adItem adType]) {
            ...
            {
                _adImageView = [[STIMImageView alloc] initWithFrame:CGRectMake(index * _scrollView.width, 0, _scrollView.width, _scrollView.height)];
                NSString *imgUrl = [adItem adImgUrl];
                NSString *filePath = [[STIMKit sharedInstance] qimNav_getAdvertImageFilePath];
                NSString *advertFileName = [[STIMKit sharedInstance] getFileNameFromUrl:imgUrl];
                filePath = [filePath stringByAppendingPathComponent:advertFileName];
                _adImageView.image = [STIMImage imageWithContentsOfFile:filePath];
                [_scrollView addSubview:_adImageView];
                if (adItem.adLinkUrl) {
                    STIMTapGestureRecognizer *tap = [[STIMTapGestureRecognizer alloc] initWithTarget:self action:@selector(onOpenWebView:)];
    ...
    if (_scrollView.pagingEnabled) {
        { //最前边 补充一个
            STIMAdvertItem *lastItem = [[[STIMKit sharedInstance] qimNav_AdItems] lastObject];
            switch ([lastItem adType]) {
                ...
                case AdvertType_Image:
                {
                    _adImageView = [[STIMImageView alloc] initWithFrame:CGRectMake(0, 0, _scrollView.width, _scrollView.height)];
        ...
        { //最后一个 补充一个
            STIMAdvertItem *firstItem = [[[STIMKit sharedInstance] qimNav_AdItems] firstObject];
            NSInteger count = [[[STIMKit sharedInstance] qimNav_AdItems] count] + 1;
            switch ([firstItem adType]) {
                ...
                case AdvertType_Image:
                {
                    _adImageView = [[STIMImageView alloc] initWithFrame:CGRectMake(count * _scrollView.width, 0, _scrollView.width, _scrollView.height)];
    ...
    [_loadingButton setBackgroundColor:[UIColor stimDB_colorWithHex:0x0 alpha:0.75]];
    ...
    if (_scrollView.pagingEnabled) {
        ...
        [_pageControl setNumberOfPages:[[STIMKit sharedInstance] qimNav_AdItems].count];
        _pageControl.pageIndicatorTintColor = [UIColor lightGrayColor];
        _pageControl.currentPageIndicatorTintColor = [UIColor redColor];
        _pageControl.currentPage   = 0;
        [self.view addSubview:_pageControl];
        
        [_scrollView setContentSize:CGSizeMake(_scrollView.width * ([[STIMKit sharedInstance] qimNav_AdItems].count+2), _scrollView.height)];
        [_scrollView setContentOffset:CGPointMake(_scrollView.width, 0)];
        if ([[STIMKit sharedInstance] qimNav_AdItems].count > 1 && [[STIMKit sharedInstance] qimNav_AdCarouselDelay] > 0) {
            [self performSelector:@selector(carouselAdvert) withObject:nil afterDelay:[[STIMKit sharedInstance] qimNav_AdCarouselDelay]];
        }
...
- (void)initAdvertUI{
    int index = 0;
    NSNumber *ciNum = [[STIMKit sharedInstance] userObjectForKey:@"AdvertPlayIndex"];
    if (ciNum) {
        index = ciNum.intValue + 1;
        if (index < [[STIMKit sharedInstance] qimNav_AdItems].count) {
        } else {
            index = 0;
        }
    } else {
    ...
    if (index >= 0 && index < [[STIMKit sharedInstance] qimNav_AdItems].count) {
        _currentAdvertItem = [[STIMKit sharedInstance] qimNav_AdItems][index];
    }
    if (_currentAdvertItem) {
        [[STIMKit sharedInstance] setUserObject:@(index) forKey:@"AdvertPlayIndex"];
        ...
        switch ([_currentAdvertItem adType]) {
            ...
            case AdvertType_Image:
            {
                _adImageView = [[STIMImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, self.view.height)];
                ...
                if (_currentAdvertItem.adLinkUrl) {
                    STIMTapGestureRecognizer *tap = [[STIMTapGestureRecognizer alloc] initWithTarget:self action:@selector(onLoadUrl:)];
        ...
        _currentLoadingTime = [[STIMKit sharedInstance] qimNav_AdSec];
...
- (void)viewDidLoad {
    ...
    if ([[STIMKit sharedInstance] qimNav_AdCarousel]) {
        [self initCarouselUI];
    } else {
        [self initAdvertUI]; 
    }
