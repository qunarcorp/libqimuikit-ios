...
@interface STIMMySignatureViewController ()<UITextViewDelegate>
{
...
@implementation STIMMySignatureViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    self.view.backgroundColor = [UIColor stimDB_colorWithHex:0xebebf1 alpha:1];
...
- (void)initNav{
    ...
    self.navigationItem.title = [NSBundle stimDB_localizedStringForKey:@"What's Up"];
...
- (void)onDoneClick{
    if (_textView.text.length > 0) {
        [[STIMKit sharedInstance] updateUserSignature:_textView.text withCallBack:^(BOOL successed) {
            
        }];
        [self.navigationController popViewControllerAnimated:YES];
    }else{
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"] message:@"您并没有写下任何东西~" delegate:nil cancelButtonTitle:@"俺 know~" otherButtonTitles:nil, nil];
        [alertView show];
    }
}
