...
@interface STIMSourceCodeCell()<STIMMenuImageViewDelegate>


@end

@implementation STIMSourceCodeCell{
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [_iconLabel setBackgroundColor:[UIColor stimDB_colorWithHex:0x0 alpha:0.75]];
...
{
    if (self.message.messageType == STIMMessageType_Markdown) {
        NSError  *error;
        ...
        if (self.owerViewController) {
            ...
            if (infoDic.count > 0) {
                STIMSourceCodeVC *codeVC = [[STIMSourceCodeVC alloc] init];
...
- (void)refreshUI{
    ...
    UIImage *image = [STIMFileIconTools getFileIconWithExtension:@"txt"];
    ...
    } else if (self.message.messageType == STIMMessageType_Markdown) {
