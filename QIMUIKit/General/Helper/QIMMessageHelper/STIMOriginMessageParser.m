...
@interface STIMOriginMessageParser ()

@property (nonatomic, strong) NSMutableDictionary *originMsgDict;

@end

@implementation STIMOriginMessageParser
...
static STIMOriginMessageParser *_message = nil;
+ (instancetype)shareParserOriginMessage {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _message = [[STIMOriginMessageParser alloc] init];
    });
    return _message;
}

- (NSString *)getOriginPBMessageWithMsgId:(NSString *)msgId {
    NSDictionary *msgDic = [[STIMKit sharedInstance] getMsgDictByMsgId:msgId];
    id msgRaw = msgDic[@"MsgRaw"];
    if (msgRaw) {
        NSDictionary *originMsgDic = [[STIMKit sharedInstance] parseOriginMessageByMsgRaw:msgRaw];
        return [[STIMJSONSerializer sharedInstance] serializeObject:originMsgDic];
    }
    return nil;
}
...
- (NSString *)getOriginMsgBackupInfoWithMsgId:(NSString *)msgId {
    ...
    if (!originMsgDic) {
        ...
        [self.originMsgDict setSTIMSafeObject:originMsgDic forKey:msgId];
    }
...
- (NSString *)getOriginMsgBackupInfoWithMsgRaw:(id)msgRaw WithMsgId:(NSString *)msgId {
    ...
    if (!memOriginMsg.length) {
        ...
        [self.originMsgDict setSTIMSafeObject:backupInfo forKey:msgId];
        return backupInfo;
    }
    return memOriginMsg;
}
