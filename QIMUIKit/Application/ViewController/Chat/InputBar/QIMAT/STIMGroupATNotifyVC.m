...
@interface STIMGroupATNotifyVC () <UITableViewDataSource,UITableViewDelegate> {
...
@interface STIMGroupATNotifyVC (Search)<SearchBarDelgt>

@end


@implementation STIMGroupATNotifyVC (Search)
...
- (void)searchBar:(SearchBar *)SectionSearchBar textDidChange:(NSString *)searchText
{
    ...
    if (searchText.length > 0 && [[self groupID] length] > 0) {
        [_searchResults addObjectsFromArray:[[STIMKit sharedInstance] searchGroupUserBySearchStr:searchText inGroup:[self groupID]]];
        STIMVerboseLog(@"_searchResults :%@", _searchResults);
    }
...
@implementation STIMGroupATNotifyVC
...
- (void)setupSearchBar {
    ...
    iconView.image = [STIMKit defaultGroupHeaderImage];
...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        self.dataSource = [NSMutableArray arrayWithArray:[[STIMKit sharedInstance] syncgroupMember:[self groupID]]];
        STIMVerboseLog(@"后备线程请求群组成员数据 : %@", self.dataSource);
        dispatch_async(dispatch_get_main_queue(), ^{
            STIMVerboseLog(@"刷新界面");
            [self.groupMemberListView reloadData];
        });
...
- (void)reloadGroup:(NSNotification *)notify {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([groupIds containsObject:self.groupID]) {
            NSArray *members = [[STIMKit sharedInstance] stIMDB_getGroupMember:[self groupID]];
...
- (void)loadGroupData {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray *roomMembers = [NSMutableArray arrayWithArray:[[STIMKit sharedInstance] getGroupMembersByGroupId:self.groupID]];
        STIMVerboseLog(@"roomMembers == %@", roomMembers);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [STIMAtUserTableViewCell getCellHeight];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMAtUserTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [[STIMAtUserTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    STIMAtUserTableViewCell *cell = (STIMAtUserTableViewCell *)[tableView cellForRowAtIndexPath:indexPath];
