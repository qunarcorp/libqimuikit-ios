...
#import "STIMAudioPlayer.h"
#import "STIMIconInfo.h"
#import "STIMContactSelectionViewController.h"

@interface STIMFilePreviewVC ()<UIWebViewDelegate,ASIProgressDelegate,ASIHTTPRequestDelegate>{
    ...
    STIMMoviePlayer *_videoPlayer;
    STIMAudioPlayer *_audioPlayer;
...
@implementation STIMFilePreviewVC

- (NSString *) fileNamefromUrl:(NSString *) fileUrl {
    NSString *fileName = nil;

    NSURL *url = [NSURL URLWithString:fileUrl];
    NSDictionary *queryComponents = [url.query stimDB_dictionaryFromParamComponents];
...
- (void)viewDidLoad {
    ...
    [_openOtherButton setImage:[UIImage qimIconWithInfo:[STIMIconInfo iconInfoWithText:@"\U0000f1cd" size:24 color:[UIColor colorWithRed:33/255.0 green:33/255.0 blue:33/255.0 alpha:1/1.0]]] forState:UIControlStateNormal];
    ...
    NSString *downLoad = [[STIMKit sharedInstance] getDownloadFilePath];
    ...
    
    {
        ...
        [iconView setImage:[STIMFileIconTools getFileIconWithExtension:_fileName.pathExtension]];
...
- (void)showFile {
    ...
    if ([STIMFileIconTools getFileTypeByFileExtension:_fileName.pathExtension] == FileType_Video) {
        _hasFile = YES;
        _openOtherButton.enabled = YES;
        _videoPlayer = [[STIMMoviePlayer alloc] initWithFrame:CGRectMake(0, 0, self.view.width, _bottomView.top)];
    ...
    } else if ([STIMFileIconTools getFileTypeByFileExtension:_fileName.pathExtension] == FileType_Audio){
        _hasFile = NO;
        _audioPlayer = [[STIMAudioPlayer alloc] initWithFrame:CGRectMake(0, 0, self.view.width, _bottomView.top)];
...
- (void)request:(ASIHTTPRequest *)request didReceiveResponseHeaders:(NSDictionary *)responseHeaders{
    STIMVerboseLog(@"didReceiveResponseHeaders %@",responseHeaders);
}
...
- (void)request:(ASIHTTPRequest *)request didReceiveData:(NSData *)data{
    if (request.responseStatusCode == 200) {
        ...
        STIMVerboseLog(@"FilePreViewVc didReceiveData : %@, StatusCode : %d", request, request.responseStatusCode);
    }
}

- (void)requestFinished:(ASIHTTPRequest *)request{
    if (request.responseStatusCode == 200) {
        STIMVerboseLog(@"finished");
        ...
        STIMVerboseLog(@"FilePreViewVc requestFinished : %@, StatusCode : %d", request, request.responseStatusCode);
    }
}

- (void)requestFailed:(ASIHTTPRequest *)request{
    STIMVerboseLog(@"requestFailed");
    [_downloadRequest setDelegate:nil];
    [_downloadRequest setDownloadProgressDelegate:nil];
    _downloadRequest = nil;
    [_fileHandle closeFile];
...
- (void)setProgress:(float)newProgress {
    STIMVerboseLog(@"Down File %f",newProgress);
}

// Called when the request receives some data - bytes is the length of that data
- (void)request:(ASIHTTPRequest *)request didReceiveBytes:(long long)bytes{
//    STIMVerboseLog(@"didReceiveBytes %lld", bytes);
...
- (void)request:(ASIHTTPRequest *)request didSendBytes:(long long)bytes{
    STIMVerboseLog(@"didSendBytes %lld", bytes);
//    _currentOffset = bytes;
}

// Called when a request needs to change the length of the content to download
- (void)request:(ASIHTTPRequest *)request incrementDownloadSizeBy:(long long)newLength{
    STIMVerboseLog(@"incrementDownloadSizeBy %lld", newLength);
...
- (void)request:(ASIHTTPRequest *)request incrementUploadSizeBy:(long long)newLength{
    STIMVerboseLog(@"incrementUploadSizeBy %lld", newLength);
    _requestLength += newLength;
}
#pragma mark - init ui
...
- (void)onRepeatButton:(UIButton *)sender{
    ...
    self.message.messageType = STIMMessageType_File;
...
- (void)initBottomView{
    ...
    [_repeatButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Forward"] forState:UIControlStateNormal];
    ...
    [_downLoadButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Download to this device"] forState:UIControlStateNormal];
    ...
    [_deleteButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Delete from this device"] forState:UIControlStateNormal];
    ...
    [_cancelDownButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"cancel_down"] forState:UIControlStateNormal];
    [_cancelDownButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"cancel_down_pressed"] forState:UIControlStateHighlighted];
...
- (void)loadFileError {
    ...
    UIImageView *iconView = [[UIImageView alloc] initWithFrame:CGRectMake((self.view.width - 60)/2.0, 100, 60, 60)];
    [iconView setAutoresizingMask:UIViewAutoresizingFlexibleBottomMargin|UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin];
    [iconView setImage:[STIMFileIconTools getFileIconWithExtension:fileName.pathExtension]];
    [_downloadView addSubview:iconView];
    ...
    [otherAppOpenBtn setTitle:[NSBundle stimDB_localizedStringForKey:@"Open in Other Apps"] forState:UIControlStateNormal];
    ...
    [infoLabel setTextAlignment:NSTextAlignmentCenter];
    [infoLabel setText:[NSString stringWithFormat:@"%@%@", [STIMKit getSTIMProjectTitleName], @"暂不支持打开此类文件 \n 可使用前其他应用打开并预览"]];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showUI)];
...
- (void)loadFilePath:(NSString*)filePath inView:(UIWebView*)webView {
    if (filePath.length >= 0) {
        ...
        STIMVerboseLog(@"WebView 预览文件: %@, MiMEType : %@", filePath, MIMEType);
