...
//            [weakSelf scrollToBottom_tableView];
        ...
        [[QIMKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:nil WihtLimit:kPageCount WithOffset:0 WithNeedReload:NO WihtComplete:^(NSArray *list) {
            [self.messageManager.dataSource addObjectsFromArray:list];
            [weakSelf.tableView reloadData];
//            [weakSelf scrollToBottom_tableView];
        }];
...
- (void)loadNewSystemMsgList {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if ([QIMKit getQIMProjectType] == QIMProjectTypeQTalk) {
            ...
            [[QIMKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:nil WihtLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count WithNeedReload:YES WihtComplete:^(NSArray *list) {
