...
@interface STIMXMenuItem ()

@property (nonatomic, assign) BOOL showSelected;
@end

@implementation STIMXMenuItem
...
+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image target:(id)target action:(SEL)action {
    STIMXMenuItem *item = [[STIMXMenuItem alloc] init:title image:image tag:0 userInfo:nil target:target action:action];
...
+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image tag:(NSInteger)tag userInfo:(NSDictionary *)userInfo {
    STIMXMenuItem *item = [[STIMXMenuItem alloc] init:title image:image tag:tag userInfo:userInfo target:nil action:NULL];
    item.alignment = NSTextAlignmentCenter;
...
+ (instancetype)menuTitle:(NSString *)title WithIcon:(UIImage *)icon {
    STIMXMenuItem *item = [[STIMXMenuItem alloc] init:title image:icon tag:0 userInfo:nil target:nil action:NULL];
