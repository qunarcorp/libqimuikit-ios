...
@interface STIMUIImagePickerBrowserVC ()<UIActionSheetDelegate,UIScrollViewDelegate,STIMImageEditViewControllerDelegate>{
...
@implementation STIMUIImagePickerBrowserVC
...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"common_save"] style:UIBarButtonItemStylePlain target:self action:@selector(saveBtnHandle:)];
    [self.navigationItem setRightBarButtonItem:rightItem];
    
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, [[STIMDeviceManager sharedInstance] getSTATUS_BAR_HEIGHT] - 20, self.view.frame.size.width, [UIScreen mainScreen].bounds.size.height - self.navigationController.navigationBar.height - 40 - 20 - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT])];
    ...
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - self.navigationController.navigationBar.height - 40 - 20 - [[STIMDeviceManager sharedInstance] getHOME_INDICATOR_HEIGHT], [UIScreen mainScreen].bounds.size.width, 46)];
    ...
    [_sdButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Standard Definition"] forState:UIControlStateNormal];
    [_sdButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"photo_browser_button_arrow_normal"] forState:UIControlStateNormal];
    [_sdButton setImage:[UIImage stimDB_imageNamedFromSTIMUIKitBundle:@"photo_browser_button_arrow_pressed"] forState:UIControlStateHighlighted];
    ...
    [imageEditButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Edit"] forState:UIControlStateNormal];
    ...
    [doneButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Confirm"] forState:UIControlStateNormal];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0) {
        ...
        _isSD = NO;
        [_sdButton setTitle:[NSBundle stimDB_localizedStringForKey:@"Full Image"] forState:UIControlStateNormal];
    }
}
...
{
    STIMImageEditViewController * imageEditVC = [[STIMImageEditViewController alloc] initWithImage:_sourceImage];
...
- (void)onPictureQualityClick:(UIButton *)sender{
    ...
    @autoreleasepool {
        ...
        sourceStr = [STIMStringTransformTools stimDB_CapacityTransformStrWithSize:data.length];
        UIImage *image = [_sourceImage stimDB_sdImage];
        data = UIImageJPEGRepresentation(image, 1);
        bqStr = [STIMStringTransformTools stimDB_CapacityTransformStrWithSize:data.length];
    }
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Select photo size"] delegate:self cancelButtonTitle:[NSBundle stimDB_localizedStringForKey:@"Cancel"] destructiveButtonTitle:nil otherButtonTitles:[NSString stringWithFormat:@"%@ (%@)", [NSBundle stimDB_localizedStringForKey:@"Standard Definition"], bqStr], [NSString stringWithFormat:@"%@ (%@)", [NSBundle stimDB_localizedStringForKey:@"Full Image"], sourceStr],nil];
...
- (void)onDoneClick:(UIButton *)sender{
    if ([self.delegate respondsToSelector:@selector(imagePickerBrowserDidFinish:)]) {
        if (_isSD) {
            [self setSourceImage:[_sourceImage stimDB_sdImage]];
        }
...
- (void)image: (UIImage *) image didFinishSavingWithError: (NSError *) error contextInfo: (void *) contextInfo
{
    ...
    if(error != NULL){
        ...
        msg = [NSBundle stimDB_localizedStringForKey:@"Saved_Success"] ;
    }
    
    alert = [[UIAlertView alloc] initWithTitle:[NSBundle stimDB_localizedStringForKey:@"Reminder"]
                                       message:msg
                                      delegate:self
