//
//  STIMPublicNumberOrderMsgCell.m
//  qunarChatIphone
//
...
- (void)refreshUI{
    ...
    for (NSDictionary *subDic in self.contentList) {
        ...
        [subTitleLabel setTextColor:[UIColor stimDB_colorWithHex:0x888888]];
        ...
        [subContentLabel setTextColor:[UIColor stimDB_colorWithHex:0x888888]];
...
@implementation STIMPublicNumberOrderMsgCell{
    
    STIMMenuImageView *_bgView;
...
+ (CGFloat)getCellHeightByContent:(NSString *)content{
    if (_screen_width == 0) {
        if ([[STIMKit sharedInstance] getIsIpad]) {
            _screen_width = [[UIScreen mainScreen] stimDB_rightWidth];
        } else {
            _screen_width = kScreenWidth;
        }
    }
    NSDictionary *dic = [[STIMJSONSerializer sharedInstance] deserializeObject:content error:nil];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _bgView = [[STIMMenuImageView alloc] initWithFrame:CGRectZero];
        [_bgView setUserInteractionEnabled:YES];
        [_bgView setBackgroundColor:[UIColor whiteColor]];
//        [_bgView.layer setCornerRadius:5];
//        [_bgView.layer setMasksToBounds:YES];
        [_bgView.layer setBorderColor:[UIColor stimDB_colorWithHex:0xE1E1E1].CGColor];
        [_bgView.layer setBorderWidth:1.0f];
        [self.contentView addSubview:_bgView];
        
        _linkUrlButton = [[PNOrderMsgButton alloc] initWithFrame:CGRectZero];
        [_linkUrlButton setBackgroundImage:[UIImage stimDB_imageFromColor:[UIColor qtalkTableDefaultColor]] forState:UIControlStateHighlighted];
        ...
        [_titleLabel setTextColor:[UIColor stimDB_colorWithHex:0x333333]];
        ...
        [_orderLabel setTextColor:[UIColor stimDB_colorWithHex:0x888888]];
        ...
        [_lineView setBackgroundColor:[UIColor stimDB_colorWithHex:0XEAEAEA]];
        ...
        [_promptLabel setTextColor:[UIColor stimDB_colorWithHex:0xEB524A]];
        ...
        [_operationLabel setTextColor:[UIColor stimDB_colorWithHex:0x00CABE]];
...
- (void)onLinkButtonClick:(PNOrderMsgButton *)sender{
    if ([self.delegate respondsToSelector:@selector(openWebUrl:)]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STIMKit sharedInstance] updateMessageReadStateWithMsgId:self.message.messageId];
        });
        [self.message setMessageReadState:STIMMessageRemoteReadStateDidOperated];
        if (self.message.messageReadState & STIMMessageRemoteReadStateDidOperated != STIMMessageRemoteReadStateDidOperated) {
            [_operationLabel setText:@"现在去处理"];
        } else {
...
- (void)refreshUI{
    ...
    NSDictionary *dic = [[STIMJSONSerializer sharedInstance] deserializeObject:self.message.message error:nil];
    if (self.message.messageReadState & STIMMessageRemoteReadStateDidOperated != STIMMessageRemoteReadStateDidOperated) {
        [_operationLabel setText:@"现在去处理"];
        [_operationLabel setTextColor:[UIColor stimDB_colorWithHex:0x00CABE]];
    } else {
        [_operationLabel setText:@"已处理"];
        [_operationLabel setTextColor:[UIColor stimDB_colorWithHex:0x999999]];
    }
    NSString *title = [dic objectForKey:@"title"];
    NSArray *contentList = [dic objectForKey:@"content"];
