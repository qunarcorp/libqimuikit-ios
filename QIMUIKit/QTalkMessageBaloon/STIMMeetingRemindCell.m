...
#define kCommonMeetingRemindCellWidth       IS_Ipad ? ([UIScreen mainScreen].stimDB_rightWidth  * 240 / 320) : ([UIScreen mainScreen].bounds.size.width * 5/7)

@interface STIMMeetingRemindCell () <STIMMenuImageViewDelegate>
...
@implementation STIMMeetingRemindCell
...
- (void)onClick {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([self.meetingUrl containsString:@"?"]) {
            url = [NSString stringWithFormat:@"%@username=%@&meeting_id=%@", self.meetingUrl, [STIMKit getLastUserName], self.meetingId];
        } else {
            url = [NSString stringWithFormat:@"%@?username=%@&meeting_id=%@", self.meetingUrl, [STIMKit getLastUserName], self.meetingId];
        }
...
- (void)refreshUI {
    ...
    CGFloat cellHeight = [STIMMeetingRemindCell getCellHeightWithMessage:self.message chatType:self.chatType];
    ...
    [messageAttriStr addAttribute:NSForegroundColorAttributeName value:[UIColor stimDB_colorWithHex:0x009ad6 alpha:1.0] range:NSMakeRange(0, 6)];
